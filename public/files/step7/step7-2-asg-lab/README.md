...
| Target Group       | `asg-lab-tg`              | HTTP:AppPort, Health Check: /    |
...
ALB (asg-lab-alb) → 두 서브넷에 걸쳐 배치
  └── Listener HTTP:80 → Target Group (asg-lab-tg, HTTP:AppPort)
...
3. **Upload a template file** → `step7-2-asg-prereq.yaml` 업로드
