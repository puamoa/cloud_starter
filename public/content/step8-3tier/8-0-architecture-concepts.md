...
learningObjectives:
  ...
  - CI/CD의 기본 개념과 GitHub Actions를 이용한 배포 흐름을 이해할 수 있습니다.
---
...
> **모놀리식**(Monolithic)은 모든 기능이 하나의 애플리케이션에 통합된 구조이고,  
> **마이크로서비스**(Microservices)는 각 기능을 독립된 서비스로 분리한 구조입니다.  
> 프로젝트 규모와 팀 구성에 따라 적합한 아키텍처가 다릅니다.
...
> **N-Tier 아키텍처**는 애플리케이션을 논리적 계층(Tier)으로 분리하는 설계 패턴입니다.  
> 각 계층은 특정 역할을 담당하며, 계층 간 명확한 인터페이스를 통해 통신합니다. 분리를 통해 독립적 확장과 유지보수가 가능해집니다.
...
> **프론트엔드/백엔드 분리**는 UI(프론트엔드)와 비즈니스 로직(백엔드)을 독립된 프로젝트로 개발·배포하는 아키텍처입니다.  
> API를 통해 통신하며, 각각 최적의 기술과 인프라를 선택할 수 있습니다.
...
> **REST**(Representational State Transfer)는 HTTP 프로토콜을 활용한 API 설계 아키텍처 스타일입니다.  
> 리소스를 URL로 식별하고, HTTP 메서드로 행위를 표현하며, 상태를 서버에 저장하지 않는(Stateless) 것이 핵심입니다.
...
> **Stateless**(무상태)는 서버가 클라이언트의 상태를 저장하지 않는 방식이고,  
> **Stateful**(상태 유지)은 서버가 세션 정보를 유지하는 방식입니다.  
> 수평 확장을 위해서는 Stateless 설계가 필수적입니다.
...
> [!CONCEPT] 다계층 캐싱 전략
> **캐싱**은 자주 요청되는 데이터를 빠른 저장소에 보관하여 응답 시간을 단축하는 전략입니다.  
> 웹 아키텍처에서는 CDN(정적 콘텐츠), Redis(동적 데이터), 브라우저 캐시 등 여러 계층에서 캐싱을 적용합니다.
...
> **비동기 처리**는 시간이 오래 걸리는 작업을 즉시 응답하고 백그라운드에서 처리하는 패턴입니다.  
> **메시지 큐**는 생산자(Producer)와 소비자(Consumer) 사이에서 메시지를 임시 저장하여 비동기 통신을 가능하게 합니다.
...
> [!CONCEPT] CI/CD란?
> **CI**(Continuous Integration)는 코드 변경을 자주 통합하고 자동으로 빌드·테스트하는 관행이고,  
> **CD**(Continuous Delivery/Deployment)는 검증된 코드를 자동으로 스테이징/프로덕션에 배포하는 관행입니다.
>
> Step 8-2와 8-3에서 **GitHub Actions**를 사용하여 프론트엔드/백엔드 배포를 자동화합니다.  
> CI/CD의 자세한 이론(배포 전략, Git Flow, 컨테이너 등)은 **Step 9**에서 다룹니다.

### 이 실습에서 사용하는 CI/CD 구조
...
GitHub Actions (자동 트리거)
    │
    ├── 프론트엔드: npm build → S3 업로드 → CloudFront 무효화
    │
    └── 백엔드: Gradle build → S3 업로드 → SSM Run Command → EC2 배포
```

| 용어               | 설명                                                    |
| ------------------ | ------------------------------------------------------- |
| **GitHub Actions** | GitHub에 내장된 CI/CD 자동화 도구. YAML로 워크플로 정의 |
| **워크플로**       | `.github/workflows/*.yml`에 정의된 자동화 파이프라인    |
| **트리거**         | 워크플로를 실행하는 이벤트 (push, PR 등)                |
| **아티팩트**       | 빌드 결과물 (JAR, dist 폴더 등)                         |

> [!NOTE]
> CI/CD의 상세 이론(Rolling/Blue-Green/Canary 배포 전략, 컨테이너, GitOps 등)은 Step 9에서 학습합니다.
...
| CI/CD          | 코드 Push → 자동 빌드·테스트·배포 (Step 9에서 심화) |
