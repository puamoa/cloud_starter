...
AWS 배포를 위해 아래 항목을 확인하고 수정한 후, **태스크 3**으로 이동하세요.

### A-1. API URL 환경 변수 설정
...
> [!TIP]
> UI 프레임워크가 필요하면 함께 설치할 수 있습니다:
>
> ```bash
> npm install bootstrap@5       # Bootstrap 5
> npm install vuetify            # Vuetify (Material Design)
...
> [!TIP]
> **인터셉터 없이 간단하게 사용하는 경우:**
>
> ```javascript
> // src/api/index.js (심플 버전)
> import axios from 'axios';
>
> const apiClient = axios.create({
>   baseURL: import.meta.env.VITE_API_URL,
...
> ```
>
> 핵심은 `baseURL`에 환경 변수를 설정하는 것입니다. 인터셉터는 선택 사항입니다.  
> 기존 프로젝트에서 `axios.get('/api/items')`처럼 상대 경로로 호출하고 있다면,  
> `baseURL`이 설정된 인스턴스를 사용하도록 변경하거나 호출 시 전체 URL을 포함해야 합니다.
...
26. **Domain** 섹션: 비워둡니다 (커스텀 도메인은 태스크 7에서 설정).
...
30. **Origin** 필드에 태스크 3에서 확인한 S3 웹사이트 엔드포인트를 직접 입력합니다:
...
> 범위가 넓을수록 더 많은 지역에서 빠르지만 요청당 단가가 높습니다.  
...
> [!TIP]
> **Error caching minimum TTL** 필드는 기본값(10초)으로 두면 됩니다.  
> 이 값은 에러 응답을 엣지에서 캐싱하는 시간입니다.  
> SPA에서는 모든 에러를 `index.html`(200)로 반환하므로 실질적 영향이 없지만,  
> 배포 직후 즉시 반영이 필요하면 `0`으로 설정할 수 있습니다.
...
57. 주소창에 `https://d1234abcdef.cloudfront.net/<본인 프로젝트의 하위 경로>`를 직접 입력하여 SPA 라우팅이 동작하는지 확인합니다 (예: `/items`, `/board`, `/about` 등).
...
> [!TIP]
> 자동 배포가 정상 동작하는지 눈으로 확인하고 싶다면, 워크플로우 작성 후 push 전에 화면 텍스트를 한 군데 수정해두세요 (예: `HomeView.vue`의 제목 변경).  
> push 후 Amazon CloudFront URL에서 변경된 텍스트가 보이면 자동 배포가 성공한 것입니다.
...
> [!NOTE]
> 이 사용자는 GitHub Actions에서 프로그래밍 전용(API 접근)으로만 사용합니다.  
> 콘솔 로그인 권한은 불필요하므로 체크하지 않습니다.
...
> 기존 프로젝트(방법 A)에서 `VITE_KAKAO_KEY` 등 추가 환경변수가 있다면:
>
> 1. `.gitignore`에 추가합니다:
>    ```gitignore
>    .env.production
  ...
  deploy:
    ...
    steps:
      ...
      - name: Build for production
        ...
        env:
          VITE_API_URL: ${{ secrets.VITE_API_URL }}
          # 추가 환경변수가 있다면 여기에 등록 (81번에서 등록한 Secret과 동일)
...
> [!TIP]
> push 시 인증 에러가 발생하면 Step 8-1 태스크 2에서 설정한 GitHub 인증(`gh auth login` 또는 remote URL 토큰)을 확인하세요.
...
> [!NOTE]
> 스텝이 ❌ 실패한 경우, 실패한 스텝을 클릭하면 상세 로그를 확인할 수 있습니다.  
> 에러 메시지를 확인하고 아래 TROUBLESHOOTING 표를 참고하세요.
...
92. Distributions 목록에서 태스크 5에서 생성한 배포를 클릭합니다.
...
> [!TIP]
> 서브도메인은 `app` 외에 `front`, `www` 등 원하는 이름을 사용해도 됩니다.  
> 단, 이후 Amazon Route 53 레코드와 동일한 이름을 사용해야 합니다.
...
> [!WARNING]
> Amazon CloudFront에 CNAME을 등록하는 것만으로는 커스텀 도메인 접속이 되지 않습니다.  
> Amazon Route 53에 해당 도메인의 A 레코드(Alias)를 추가해야 DNS가 Amazon CloudFront로 연결됩니다.
...
> [!TIP]
> **Routing policy**는 기본값 `Simple routing`을 사용합니다.  
> Amazon Route 53은 다양한 라우팅 정책을 제공합니다:
>
> - **Simple**: 단일 대상으로 라우팅 (이 실습에서 사용)
> - **Weighted**: 비율 기반 분산 (A/B 테스트, 카나리 배포)
> - **Latency**: 가장 낮은 지연 시간의 리전으로 라우팅
> - **Failover**: 장애 시 대체 대상으로 전환
