...
23. Amazon S3 웹사이트 엔드포인트로 접속합니다 (태스크 2의 17번에서 복사한 본인 URL 사용):
...
40. **Allowed HTTP methods**: `GET, HEAD` (기본값 유지)
41. **Cache policy**: `CachingOptimized` (기본값 — "Recommended for S3" 표시)
42. **Origin request policy**: 비워둡니다 (Select origin policy 그대로).
43. **Response headers policy**: 비워둡니다.
44. [[Next]]를 클릭합니다.
