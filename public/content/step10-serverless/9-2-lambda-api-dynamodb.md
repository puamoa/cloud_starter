...
1. 상단 검색창에 `Lambda`를 입력하고 선택합니다.
2. [[Create function]]을 클릭합니다.
3. **Author from scratch**를 선택합니다.
4. 다음을 설정합니다:
   - **Function name**: `starter-dynamodb-api`
   - **Runtime**: **Node.js 20.x**
   - **Architecture**: x86_64
5. **Permissions** 섹션을 펼칩니다:
...
7. 함수 상세 페이지에서 **Configuration** 탭을 클릭합니다.
8. 왼쪽 메뉴에서 **Permissions**를 클릭합니다.
9. **Execution role** 섹션에서 Role name 링크를 클릭합니다 (IAM 콘솔로 이동).
10. IAM Role 페이지에서 [[Add permissions]] → [[Attach policies]]를 클릭합니다.
11. 검색창에 `DynamoDB`를 입력합니다.
12. **AmazonDynamoDBFullAccess**를 체크합니다.
13. [[Add permissions]]를 클릭합니다.
...
1. Lambda 함수 페이지에서 **Code** 탭을 선택합니다.
2. `index.mjs` 파일의 내용을 아래 코드로 교체합니다:
...
1. Lambda 함수 페이지에서 **Test** 탭을 선택합니다.
2. [[Create new event]]를 클릭합니다.
3. 다음을 설정합니다:
...
1. 상단 검색창에 `API Gateway`를 입력하고 선택합니다.
2. **REST API** 섹션에서 [[Build]]를 클릭합니다.
...
4. [[Create API]]를 클릭합니다.

### 리소스 생성 (/items)

5. `/` (루트)를 선택한 상태에서 [[Create resource]]를 클릭합니다.
6. 설정:
   - **Resource name**: `items`
   - **Resource path**: `/items` (자동)
   - ✅ **CORS (Cross Origin Resource Sharing)** 체크
7. [[Create resource]]를 클릭합니다.

### 하위 리소스 생성 (/items/{id})

8. `/items` 리소스를 선택합니다.
9. [[Create resource]]를 클릭합니다.
10. 설정:
    - **Resource name**: `{id}`
    - **Resource path**: `/items/{id}`
    - ✅ **CORS** 체크
11. [[Create resource]]를 클릭합니다.

### 메서드 생성 (GET /items)

12. `/items` 리소스를 선택합니다.
13. [[Create method]]를 클릭합니다.
14. 설정:
    - **Method type**: GET
    - **Integration type**: Lambda Function
    - ✅ **Lambda proxy integration** 체크
    - **Lambda function**: `starter-dynamodb-api` 선택
15. [[Create method]]를 클릭합니다.

### 메서드 생성 (POST /items)

16. `/items` 리소스를 선택합니다.
17. [[Create method]]를 클릭합니다.
18. 설정:
    - **Method type**: POST
    - **Integration type**: Lambda Function
    - ✅ **Lambda proxy integration** 체크
    - **Lambda function**: `starter-dynamodb-api`
19. [[Create method]]를 클릭합니다.

### 메서드 생성 (GET /items/{id})

20. `/items/{id}` 리소스를 선택합니다.
21. [[Create method]]를 클릭합니다.
22. 설정:
    - **Method type**: GET
    - **Integration type**: Lambda Function
    - ✅ **Lambda proxy integration** 체크
    - **Lambda function**: `starter-dynamodb-api`
23. [[Create method]]를 클릭합니다.

### 메서드 생성 (DELETE /items/{id})

24. `/items/{id}` 리소스를 선택합니다.
25. [[Create method]]를 클릭합니다.
26. 설정:
    - **Method type**: DELETE
    - **Integration type**: Lambda Function
    - ✅ **Lambda proxy integration** 체크
    - **Lambda function**: `starter-dynamodb-api`
27. [[Create method]]를 클릭합니다.
...
1. API Gateway 콘솔에서 `starter-serverless-api`를 선택합니다.
2. [[Deploy API]]를 클릭합니다.
3. 설정:
   - **Stage**: **\*New Stage\***
   - **Stage name**: `prod`
4. [[Deploy]]를 클릭합니다.
...
5. **Stages** → `prod`를 선택합니다.
6. 상단에 **Invoke URL**이 표시됩니다:
...
2. 왼쪽 메뉴에서 **Logs** → **Log groups**를 클릭합니다.
3. `/aws/lambda/starter-dynamodb-api` 로그 그룹을 클릭합니다.
4. 최신 **Log stream**을 클릭합니다.
...
1. API Gateway 콘솔에서 `starter-serverless-api`를 선택합니다.
2. **Actions** → [[Delete API]]를 클릭합니다.
3. 확인 입력란에 API 이름을 입력합니다.
4. [[Delete]]를 클릭합니다.
...
1. Lambda 콘솔에서 `starter-dynamodb-api` 함수를 선택합니다.
2. **Actions** → [[Delete function]]을 클릭합니다.
3. 확인 대화상자에서 [[Delete]]를 클릭합니다.
...
1. DynamoDB 콘솔에서 `Items` 테이블을 선택합니다.
2. [[Delete]]를 클릭합니다.
...
2. `/aws/lambda/starter-dynamodb-api` 로그 그룹을 선택합니다.
3. **Actions** → [[Delete log group(s)]]를 클릭합니다.
4. [[Delete]]를 클릭합니다.
...
1. IAM 콘솔 → **Roles** → `starter-dynamodb-api-role-` 로 시작하는 Role 검색
2. 선택 후 [[Delete]]를 클릭합니다.
3. 확인 입력란에 Role 이름을 입력하고 [[Delete]]를 클릭합니다.
