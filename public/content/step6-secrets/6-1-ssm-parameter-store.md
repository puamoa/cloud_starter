...
> | 증상 | 원인 | 해결 방법 |
> |------|------|-----------|
> | `Unable to locate credentials` | IAM Role이 EC2에 연결되지 않음 | EC2 → Actions → Security → Modify IAM role에서 Role 연결 확인 |
> | `AccessDeniedException` on SSM | 정책의 Resource ARN 불일치 | 정책의 `Resource`가 `parameter/starter/*`와 일치하는지 확인 |
> | `AccessDeniedException` on KMS | KMS 복호화 권한 누락 | 정책에 `kms:Decrypt` Action이 있는지 확인 |
> | Role 변경 후에도 권한 에러 | IAM 전파 지연 (수 초~수 분) | 1~2분 대기 후 재시도 |
> | `Could not connect to the endpoint URL` | 리전 설정 오류 | `--region ap-northeast-2` 옵션 추가 |
