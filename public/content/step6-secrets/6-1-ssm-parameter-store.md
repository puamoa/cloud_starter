...
13. **Value** 필드에 `jdbc:mysql://localhost:3306/starter_db`를 입력합니다.

> [!TIP]
> `starter_db` 부분은 본인 프로젝트의 데이터베이스 이름으로 변경해도 됩니다. (예: `scoula_db`, `myapp_db`)  
> Amazon RDS를 사용 중이라면 `localhost` 대신 RDS 엔드포인트를 입력하세요.  
> 예: `jdbc:mysql://my-rds-mysql.xxxx.ap-northeast-2.rds.amazonaws.com:3306/scoula_db`  
...
41. **Value** 필드에 본인의 Amazon S3 버킷 이름을 입력합니다. (예: `hong-starter-app`)
...
47. [[Show]] 버튼을 클릭하여 복호화된 값(`MyPassword123!`)이 표시되는지 확인합니다.
...
49. AWS CloudShell을 엽니다 (콘솔 상단 우측의 `>_` 아이콘 클릭).
...
58. `build.gradle` 파일을 열고 다음 의존성을 추가합니다 (본인 프로젝트에 맞는 방법 선택):
...
> [!WARNING]
> **기존 프로젝트에 DataSource Bean이 이미 있는 경우:**
>
> - **Spring Boot**: `application.properties`에 `spring.datasource.*` 설정이 있으면 Boot가 자동으로 DataSource를 생성합니다. `@Profile("aws")`를 붙였으므로 로컬에서는 기존 설정이 사용되고, `aws` 프로필일 때만 이 Bean이 활성화됩니다. 충돌하지 않습니다.
> - **Spring MVC 레거시**: 기존 `RootConfig.java`에 DataSource Bean이 있다면, 기존 Bean에 `@Profile("local")`을 추가하세요. 그러면 새 `DataSourceConfig`의 `@Profile("aws")`와 분리되어 충돌하지 않습니다. (아래 TIP 참조)

> [!TIP]
> **레거시 프로젝트에서의 등록 위치:**
>
> 기존 `RootConfig.java`에 DataSource가 이미 있다면, 기존 코드를 건드리지 않고 **Profile로 분리**할 수 있습니다:
>
> ```java
> // 기존 RootConfig.java — 기존 DataSource에 @Profile("local") 추가
> @Bean
> @Profile("local")  // ← 이 줄만 추가하면 로컬에서만 활성화
...
>
> ```java
> // 새 DataSourceConfig.java — @Profile("aws")로 배포 시에만 활성화
> @Configuration
> @Profile("aws")
...
>
> **실행 방법:**
> - 로컬: `-Dspring.profiles.active=local` (기존과 동일하게 동작)
> - EC2 배포: `-Dspring.profiles.active=aws` (Parameter Store에서 조회)
>
> `@ComponentScan`에 `config` 패키지가 포함되어 있는지 확인하세요.
...
✅ **태스크 완료** — Spring에서 Parameter Store 값을 조회하여 DataSource를 설정했습니다.

> [!CONCEPT] 로컬 개발 vs 배포 환경 분리
> 위 코드를 그대로 적용하면 **로컬에서도 AWS 자격 증명이 필요**합니다.  
> 실무에서는 Spring Profile로 환경을 분리하여, 로컬에서는 기존 설정 파일을 사용하고, 배포 시에만 Parameter Store를 사용합니다.
>
> **핵심**: `DataSourceConfig` 클래스에 `@Profile("aws")` 어노테이션을 추가합니다.
>
> ```java
> @Configuration
> @Profile("aws")  // ← 이 줄 추가: "aws" 프로필일 때만 활성화
> @RequiredArgsConstructor
> public class DataSourceConfig { ... }
> ```
>
> **실행 방법:**
>
> - 로컬 개발: 기존 `application.properties`(또는 `.yml`)의 DB 설정 사용 (변경 없음)
> - EC2 배포: `java -jar app.jar --spring.profiles.active=aws`로 실행
>
> 이렇게 하면 **기존 코드를 건드리지 않고** Parameter Store 연동을 추가할 수 있습니다.

> [!TIP]
> **Spring Boot — `application.properties` 방식:**
>
> ```properties
> # application.properties (로컬용 — 기존 그대로 유지)
...
> ```
>
> **Spring Boot — `application.yml` 방식:**
>
> ```yaml
> # application.yml (로컬용 — 기존 그대로 유지)
> spring:
>   datasource:
>     url: jdbc:mysql://localhost:3306/starter_db
>     username: admin
>     password: MyPassword123!
>     driver-class-name: com.mysql.cj.jdbc.Driver
> ```
>
> 두 방식 모두 동일하게 동작합니다. 본인 프로젝트에서 사용하는 형식을 유지하세요.  
> `--spring.profiles.active=aws`로 실행하면 `DataSourceConfig` (`@Profile("aws")`)가 활성화되어 위 설정 대신 Parameter Store 값을 사용합니다.
...
> ```
>
> MVC 레거시에서는 Profile 대신 **환경 변수**로 분기할 수 있습니다:  
> - 로컬: 기존 `application.properties`에서 직접 읽기
> - EC2: `ParameterStoreService`를 `@Autowired`로 주입하여 사용
>
> 또는 `ParameterStoreService`의 `@PostConstruct`에서 AWS 접근 실패 시 기존 properties로 폴백하는 방식도 가능합니다.
...
>
> 1. **Access Key 직접 설정** (❌ 비권장): Amazon EC2에 Access Key를 저장 → 키 유출 위험
> 2. **IAM Role 연결** (✅ 권장): Amazon EC2에 Role을 부여 → 임시 자격 증명 자동 발급, 키 관리 불필요
>
...
> [!TIP]
> 이미 Amazon EC2에 연결된 Role이 있다면 새 Role을 생성하지 않고, 기존 Role에 정책을 추가할 수 있습니다:
>
...
90. IAM Role을 연결할 Amazon EC2 인스턴스의 체크박스를 클릭합니다.
...
> [!NOTE]
> Amazon EC2 인스턴스가 아직 없다면 이 단계는 건너뛰어도 됩니다.
> Step 2에서 생성한 Amazon EC2가 있다면 해당 인스턴스에 Role을 연결하세요.
...
95. Amazon EC2 인스턴스에 SSH로 접속합니다.
...
96. SSH 접속 후, **Amazon EC2 안에서** 다음 명령어를 실행합니다 (String 파라미터 조회):
