...
63. 기존 `RootConfig.java`에서 DataSource 관련 코드를 제거하고, 파라미터 주입으로 변경합니다:
...
64. `config` 패키지에 `LocalDataSourceConfig.java`를 생성합니다:
...
65. 같은 `config` 패키지에 `AwsDataSourceConfig.java`를 생성합니다:
...
✅ **태스크 완료** — Spring에서 Parameter Store 값을 조회하여 환경별 DataSource를 설정했습니다.
