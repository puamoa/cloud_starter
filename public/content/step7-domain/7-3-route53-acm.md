...
> | 리소스                | 비용               | 비고                      |
> | --------------------- | ------------------ | ------------------------- |
> | Route53 Hosted Zone   | 월 $0.50           | 삭제하지 않으면 매월 과금 |
> | 도메인 구매 (Route53) | 연 $11~39          | TLD에 따라 다름           |
...
24. 기존 네임서버를 삭제하고, Route53의 NS 레코드 4개를 입력합니다:
...
> | 증상 | 원인 | 해결 방법 |
> |------|------|-----------|
> | `dig NS` 결과에 Route53 네임서버 미표시 | DNS 전파 미완료 | 최대 48시간 대기, `dig @8.8.8.8 NS` 로 확인 |
...
> | 증상 | 원인 | 해결 방법 |
> |------|------|-----------|
> | 인증서가 30분 이상 `Pending validation` | DNS 검증 레코드 미생성 또는 전파 미완료 | Route53에서 CNAME 레코드 존재 확인, 네임서버 전파 확인 |
> | "Create records in Route 53" 버튼 비활성 | Hosted Zone이 없거나 다른 계정 | Route53에 해당 도메인의 Hosted Zone 존재 확인 |
