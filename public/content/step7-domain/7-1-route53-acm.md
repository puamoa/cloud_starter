...
> 스크린샷과 가이드도 cafe24 기준이며, Route 53에서 구매한 경우 태스크 2/3을 건너뛸 수 있습니다.
...
Route 53에서도 도메인을 직접 구매할 수 있습니다. 네임서버 설정이 자동으로 처리되어 편리합니다.

> [!NOTE]
> 이 가이드의 스크린샷은 **cafe24에서 구매한 경우**를 기준으로 작성되었습니다.  
> Route 53에서 구매한 경우 Hosted Zone이 자동 생성되므로 태스크 2(Hosted Zone 생성)와 태스크 3(네임서버 변경)을 건너뛸 수 있습니다.
...
>   도메인 등록기관(cafe24/가비아)에 이 4개의 네임서버를 등록하면, 해당 도메인의 모든 DNS 요청이 Route 53으로 전달됩니다.
>   ```
>   ns-1234.awsdns-12.org.
...
## 태스크 3: 네임서버 변경 (cafe24 → Route 53)

cafe24에서 도메인을 구매한 경우, 네임서버를 Route 53으로 변경해야 합니다.  
...
19. [cafe24 나의서비스관리](https://hosting.cafe24.com/)에 로그인합니다.
20. 좌측 메뉴에서 **도메인관리** → **네임서버 변경**을 클릭합니다.
21. 도메인 목록에서 해당 도메인의 [[네임서버 변경하기]]를 클릭합니다.
    - 현재 네임서버가 `ns1.cafe24.com` ~ `ns4.cafe24.co.kr`로 설정되어 있습니다.
22. **본인인증** 화면이 나타나면 휴대폰 또는 이메일로 인증을 완료합니다.
    - [[인증번호받기]]를 클릭하고, 받은 인증번호를 입력 후 [[확인]]을 클릭합니다.
23. **네임서버 변경** 팝업에서 **변경할 네임서버 선택**: **다른 네임서버**를 선택합니다.
24. Route 53 Hosted Zone의 NS 레코드 4개를 입력합니다:
    - **1차 네임서버**: `ns-xxx.awsdns-xx.org` (Route 53에서 복사)
...
25. [[변경하기]]를 클릭합니다.
26. "네임서버 변경신청이 정상적으로 접수되었습니다. 적용되기까지 24시간~48시간이 소요됩니다." 메시지를 확인합니다.
...
>
> `dig @8.8.8.8`에서 Route 53이 나오는데 로컬 `dig`에서 안 나오면, ISP DNS 캐시가 아직 갱신되지 않은 것입니다.
>
...
✅ **태스크 완료** — cafe24(또는 가비아)에서 Route 53으로 네임서버를 변경하고 전파를 확인했습니다.

> [!NOTE]
> NS 전파가 완료되어야 이후 태스크(A 레코드 접속 테스트, ACM DNS 검증)가 정상 동작합니다.  
> `nslookup` 또는 `dig`로 NS가 Route 53으로 바뀐 것을 확인한 후 다음 태스크를 진행하세요.  
...
26. 상단 검색창에 `EC2`를 입력하고 선택합니다.
27. 왼쪽 메뉴에서 **Instances**를 클릭합니다.
28. [[Launch instances]]를 클릭합니다.
29. 다음을 설정합니다:
...
30. **Network settings** — [[Edit]]를 클릭하고 다음을 설정합니다:
    ...
    - **Description**: `Allow SSH and HTTP for Route 53 test`
...
31. **Advanced details**를 펼치고 맨 아래 **User Data**에 다음 스크립트를 입력합니다:
...
<!DOCTYPE html>
<html>
<head><title>Route 53 Test</title>
<style>
...
30. [[Launch instance]]를 클릭합니다.
31. 인스턴스가 **Running** 상태가 되면 **Public IPv4 address**를 복사합니다.
...
36. [[Next]]를 클릭합니다.
37. **Request public certificate** 페이지에서 다음을 설정합니다:
    - **Domain names**:
...
- 도메인을 구매하고(또는 기존 도메인을 사용하고) Route 53 Hosted Zone에 연결했습니다.
- 네임서버를 변경하여 Route 53이 도메인의 DNS를 관리하도록 설정했습니다.
...
> [!WARNING]
> Route 53 Hosted Zone은 월 $0.50이 과금됩니다.  
...
> - 도메인 등록기관(cafe24/가비아/Route 53)에서 네임서버를 다시 변경해야 합니다.
...
> [!TIP]
> Route 53 Hosted Zone과 ACM 인증서는 Tag Editor에서 검색되지 않을 수 있습니다.  
...
5. EC2 콘솔 → **Instances**에서 `route53-test`를 선택합니다.
6. **Instance state** → **Terminate instance**를 클릭합니다.
7. 확인 팝업에서 [[Terminate]]를 클릭합니다.
8. **Security Groups**에서 `route53-test-sg`를 선택하고 삭제합니다.
...
9. Route 53 콘솔 → **Hosted zones** → 도메인 선택
10. A 레코드, CNAME 레코드 등 직접 생성한 레코드를 모두 선택합니다.
11. [[Delete]]를 클릭합니다.
...
12. Route 53 콘솔 → **Hosted zones** → 도메인을 선택합니다.
13. [[Delete hosted zone]]을 클릭합니다.
14. 확인 입력란에 `delete`를 입력합니다.
15. [[Delete]]를 클릭합니다.
...
16. 상단 검색창에 `Certificate Manager`를 입력하고 선택합니다.
17. 삭제할 인증서를 체크합니다.
18. [[More actions]] 드롭다운 → [[Delete]]를 클릭합니다.
19. 확인 입력란에 `delete`를 입력합니다.
20. [[Delete]]를 클릭합니다.
...
21. [cafe24 나의서비스관리](https://hosting.cafe24.com/)에 로그인합니다.
22. 좌측 메뉴에서 **도메인관리** → **기본관리**를 클릭합니다.
23. 해당 도메인의 **자동 연장** 설정을 **해제**합니다.

**가비아에서 구매한 경우:**

24. [가비아](https://www.gabia.com)에 로그인합니다.
25. **My 가비아** → **도메인** → **도메인 관리**로 이동합니다.
26. 해당 도메인의 **자동 연장** 설정을 **해제**합니다.

**Route 53에서 구매한 경우:**

27. Route 53 콘솔 → **Registered domains** → 도메인을 선택합니다.
28. **Auto-renew** 옆의 [[Disable]]을 클릭합니다.
...
29. 상단 검색창에 `Resource Groups & Tag Editor`를 입력하고 선택합니다.
30. 왼쪽 메뉴에서 **Tag Editor**를 선택합니다.
31. 다음 조건으로 검색합니다:
    - **Regions**: `ap-northeast-2`
    - **Tag key**: `Session`, **Tag value**: `7-1`
32. [[Search resources]]를 클릭합니다.
33. 검색 결과가 없으면 모든 태그된 리소스가 정리된 것입니다.
