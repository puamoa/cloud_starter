...
>
> - Route 53 Hosted Zone에 도메인이 등록됨
> - ACM 인증서 상태가 `Issued` (발급 완료)
> - NS 전파가 완료됨 (도메인으로 접속 가능)

### ALB에 HTTPS Listener 추가

85. EC2 콘솔 → **Load Balancing** → **Load Balancers**로 이동합니다.
86. `starter-alb`를 선택합니다.
87. **Listeners** 탭에서 [[Add listener]]를 클릭합니다.
88. 다음을 설정합니다:
    - **Protocol**: `HTTPS`
    - **Port**: `443`
    - **Default actions**: Forward to → `starter-app-tg`
    - **Security policy**: 기본값 유지
    - **Default SSL/TLS server certificate**: **From ACM** → 발급받은 인증서 선택
89. [[Add]]를 클릭합니다.

> [!TIP]
> HTTP(80) Listener도 유지하면서, HTTP → HTTPS 리다이렉트를 설정할 수 있습니다:
>
> - 기존 HTTP:80 Listener 선택 → [[Edit]] → Default actions를 **Redirect to** → HTTPS, 443으로 변경

### Route 53 A 레코드를 ALB Alias로 변경

90. Route 53 콘솔 → **Hosted zones** → 도메인을 클릭합니다.
91. 기존 A 레코드(EC2 IP)를 선택하고 [[Edit record]]를 클릭합니다.
92. 다음과 같이 변경합니다:
    - **Alias** 토글: ON
...
> [!CONCEPT] A 레코드 vs Alias 레코드
>
> - **A 레코드 (IP)**: EC2 Public IP를 직접 지정. IP 변경 시 수동 업데이트 필요.
> - **Alias 레코드 (ALB)**: ALB DNS를 가리킴. ALB IP가 변경되어도 자동 반영. 루트 도메인에서도 사용 가능.
>
> ALB를 사용할 때는 항상 Alias 레코드를 사용합니다.

### 도메인 HTTPS 접속 확인

94. 브라우저에서 `https://내도메인.shop`으로 접속합니다.
    - 🔒 자물쇠 아이콘이 표시되면 HTTPS 적용 성공입니다.
95. `http://내도메인.shop`으로도 접속하여 HTTPS로 리다이렉트되는지 확인합니다.
...
> | 도메인 접속 안 됨 | A 레코드 미설정 또는 NS 미전파 | Route 53 레코드 확인 + `dig @8.8.8.8` |
