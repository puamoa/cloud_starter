...
### 7-2 아키텍처: ALB + Target Group

<img src="/images/step7/7-2-architecture.png" alt="7-2 ALB + Target Group 아키텍처" class="guide-img-md" />

> [!TIP]
> 이번 실습에서는 ALB를 생성하고 Target Group에 EC2를 수동 등록합니다.  
> Security Group으로 EC2 직접 접근을 차단하고, ACM 인증서를 ALB에 연결하여 HTTPS를 적용합니다.  
> ※ 다이어그램의 Private Subnet은 이 실습에서 직접 사용하지 않지만, 전체 VPC 구조를 보여주기 위해 포함됩니다.
...
```
[선행 리소스 확인] → [ALB 개념] → [Target Group 생성] → [EC2 등록] → [ALB 생성] → [동작 확인] → [보안 강화] → [도메인 + HTTPS 적용]
```
...
> [!TIP]
> 일부 AWS 서비스(IAM, CloudFront, Amazon Route 53 등)는 **글로벌 서비스**이므로 리전 선택 드롭다운이 비활성화되거나 "Global"로 표시됩니다.  
...
4. [[Create stack]] 드롭다운을 클릭한 후 **With new resources (standard)**를 선택합니다.
   <img src="/images/step7/7-2-step4-cf-create-stack.png" alt="Create stack" class="guide-img-sm" />
5. **Prerequisite - Prepare template**에서 `Choose an existing template`을 선택합니다.
6. **Specify template**에서 `Upload a template file`을 선택합니다.
   <img src="/images/step7/7-2-step6-upload-template.png" alt="템플릿 업로드" class="guide-img-sm" />
...
10. **Parameters** 섹션에서 다음을 설정합니다:
    ...
    - 나머지 파라미터: 기본값 유지
      <img src="/images/step7/7-2-step10-parameters.png" alt="Parameters 설정" class="guide-img-sm" />
...
13. **Review and create** 페이지에서 설정 내용을 확인합니다.
    <img src="/images/step7/7-2-step13-review.png" alt="Review and create" class="guide-img-sm" />
14. [[Submit]] 버튼을 클릭합니다.
    <img src="/images/step7/7-2-step14-submit.png" alt="Submit 클릭" class="guide-img-sm" />
15. 스택 상태가 `CREATE_COMPLETE`가 될 때까지 기다립니다 (약 2~3분).
    <img src="/images/step7/7-2-step15-create-complete.png" alt="CREATE_COMPLETE" class="guide-img-sm" />
...
18. 브라우저에서 `http://<복사한 IP>`로 접속합니다.
    ...
    - 이것이 현재 상태 — 사용자가 EC2 Public IP로 직접 접속하는 구성입니다.
      <img src="/images/step7/7-2-step18-ec2-access.png" alt="EC2 직접 접속 확인" class="guide-img-sm" />

> [!TIP]
> 브라우저에서 "이 사이트는 안전하지 않습니다" 또는 HTTP 사이트 이동 경고가 나타나면 **계속 이동**을 클릭하세요.  
> 아직 HTTPS를 설정하지 않았으므로 HTTP로 접속하는 것이 정상입니다.
>
> <img src="/images/step7/7-2-step18-http-alert.png" alt="HTTP 사이트 이동 알럿" class="guide-img-sm" />
...
20. 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
    - 왼쪽 메뉴가 보이지 않으면 좌측 상단의 햄버거 버튼(☰)을 클릭하여 메뉴를 펼칩니다.
      <img src="/images/step7/7-2-step20-target-groups.png" alt="Target Groups 메뉴" class="guide-img-sm" />
...
22. **Choose a target type** — 타겟의 종류를 선택합니다:
    ...
    - 다른 옵션(IP addresses, Lambda function, Application Load Balancer)은 이 실습에서 사용하지 않습니다.
      <img src="/images/step7/7-2-step22-basic-config.png" alt="Target type 선택" class="guide-img-sm" />
23. **Target group name** — Target Group의 이름을 입력합니다:
    - `starter-app-tg`를 입력합니다.
      <img src="/images/step7/7-2-step23-target-group-name.png" alt="Target group name 입력" class="guide-img-sm" />
...
26. **VPC** — Target Group이 속할 VPC를 선택합니다:
    ...
    - 드롭다운에 여러 VPC가 표시되면, 이름 태그를 확인하여 올바른 VPC를 선택합니다.
      <img src="/images/step7/7-2-step26-vpc-select.png" alt="VPC 선택" class="guide-img-sm" />
...
28. **Health check protocol**: `HTTP`를 선택합니다.
    <img src="/images/step7/7-2-step28-health-check.png" alt="Health check 설정" class="guide-img-sm" />
...
31. **Tags – optional** 섹션을 펼치고 [[Add new tag]]를 클릭하여 태그를 추가합니다:
    ...
    - **Key**: `Session`, **Value**: `7-1`
      <img src="/images/step7/7-2-step31-tags.png" alt="Tags 추가" class="guide-img-sm" />
...
33. 이 단계에서는 타겟을 등록하지 않고 [[Next]]를 클릭합니다.
    <img src="/images/step7/7-2-step33-skip-register.png" alt="Register targets 건너뛰기" class="guide-img-sm" />
...
34. 설정 내용을 확인합니다:
    ...
    - Register targets: Targets (0) — 등록하지 않았으므로 0개 정상
      <img src="/images/step7/7-2-step34-review.png" alt="Review and create" class="guide-img-sm" />
35. [[Create target group]]을 클릭합니다.
    - "Feedback for ELB Create Target Group" 팝업이 나타나면 ✕ 버튼으로 닫습니다.
      <img src="/images/step7/7-2-step35-created1.png" alt="Target Group 생성 1" class="guide-img-sm" />
      <img src="/images/step7/7-2-step35-created2.png" alt="Target Group 생성 2" class="guide-img-sm" />
...
37. `starter-app-tg`를 클릭하여 상세 페이지로 이동합니다.
    <img src="/images/step7/7-2-step37-tg-detail.png" alt="Target Group 상세 페이지" class="guide-img-sm" />
38. **Targets** 탭을 클릭합니다.
    <img src="/images/step7/7-2-step38-targets-tab.png" alt="Targets 탭" class="guide-img-sm" />
...
41. **Ports for the selected instances**: 애플리케이션 포트를 확인합니다.
    ...
    - 변경할 필요가 없으면 그대로 둡니다.
      <img src="/images/step7/7-2-step41-ports-confirm.png" alt="포트 확인 및 Include as pending" class="guide-img-sm" />
...
43. 하단 **Review targets**에서 등록할 인스턴스와 포트가 올바른지 확인합니다.
    <img src="/images/step7/7-2-step43-review-targets.png" alt="Review targets 확인" class="guide-img-sm" />
44. [[Register pending targets]]를 클릭합니다.
    <img src="/images/step7/7-2-step44-register-pending.png" alt="Register pending targets" class="guide-img-sm" />
...
46. EC2 콘솔 왼쪽 메뉴에서 **Network & Security** → **Security Groups**를 클릭합니다.
    <img src="/images/step7/7-2-step46-security-groups.png" alt="Security Groups 메뉴" class="guide-img-sm" />
...
48. **Basic details** 섹션을 설정합니다:
    ...
    - **VPC**: 실습용 VPC를 선택합니다 (예: `alb-lab-vpc`).
      - ⚠️ 반드시 Target Group과 같은 VPC를 선택해야 합니다.
        <img src="/images/step7/7-2-step48-sg-basic-details.png" alt="Security Group Basic details" class="guide-img-sm" />
...
<img src="/images/step7/7-2-step49-inbound-rules.png" alt="Inbound rules 설정" class="guide-img-sm" />

50. **Outbound rules** 섹션:
    - 기본값(All traffic, Destination: 0.0.0.0/0)을 유지합니다.
      <img src="/images/step7/7-2-step50-outbound-rules.png" alt="Outbound rules" class="guide-img-sm" />
51. **Tags** 섹션에서 [[Add new tag]]를 클릭하여 태그를 추가합니다:
    - **Key**: `Name`, **Value**: `alb-sg`
    - **Key**: `CreatedBy`, **Value**: `admin-user`
    - **Key**: `Step`, **Value**: `step7`
    - **Key**: `Session`, **Value**: `7-1`
      <img src="/images/step7/7-2-step51-sg-tags.png" alt="Security Group Tags" class="guide-img-sm" />
...
52. [[Create security group]]을 클릭합니다.
    <img src="/images/step7/7-2-step52-sg-created1.png" alt="Security Group 생성 1" class="guide-img-sm" />
    <img src="/images/step7/7-2-step52-sg-created2.png" alt="Security Group 생성 2" class="guide-img-sm" />
...
53. EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Load Balancers**를 클릭합니다.
    <img src="/images/step7/7-2-step53-load-balancers.png" alt="Load Balancers 메뉴" class="guide-img-sm" />
...
55. 로드밸런서 유형 선택 화면에서 **Application Load Balancer** 섹션의 [[Create]]를 클릭합니다.
    - Network Load Balancer, Gateway Load Balancer는 이 실습에서 사용하지 않습니다.
      <img src="/images/step7/7-2-step55-alb-type.png" alt="ALB 유형 선택" class="guide-img-sm" />

### Basic configuration

56. **Load balancer name**: `starter-alb`를 입력합니다.
    - 이름은 영문, 숫자, 하이픈만 사용할 수 있습니다.
      <img src="/images/step7/7-2-step56-basic-config.png" alt="ALB Basic configuration" class="guide-img-sm" />
...
59. **VPC** — ALB가 배치될 VPC를 선택합니다:
    - 드롭다운에서 실습용 VPC를 선택합니다 (예: `vpc-xxx (alb-lab-vpc)`).
      <img src="/images/step7/7-2-step59-network-mapping.png" alt="Network mapping" class="guide-img-sm" />
...
62. 드롭다운에서 앞서 생성한 `alb-sg`를 선택합니다.
    <img src="/images/step7/7-2-step62-sg-select1.png" alt="ALB Security Group 선택 1" class="guide-img-sm" />
    <img src="/images/step7/7-2-step62-sg-select2.png" alt="ALB Security Group 선택 2" class="guide-img-sm" />
...
64. **Default action** 섹션:
    ...
    - **Target group stickiness**: 체크하지 않습니다 (기본값).
      <img src="/images/step7/7-2-step63-listener-routing.png" alt="Listeners and routing 설정" class="guide-img-sm" />
...
65. **Load balancer tags – optional** 섹션을 펼치고 태그를 추가합니다:
    ...
    - **Key**: `Step`, **Value**: `step7`
    - **Key**: `Session`, **Value**: `7-1`
      <img src="/images/step7/7-2-step65-alb-tags.png" alt="ALB Tags" class="guide-img-sm" />
...
66. **Optimize with service integrations – optional** 섹션:
    ...
    - 이 실습에서는 모두 체크하지 않고 넘어갑니다 (추가 비용 발생).
      <img src="/images/step7/7-2-step66-service-integrations.png" alt="Service integrations" class="guide-img-sm" />
...
67. 하단의 **Review** 섹션에서 **Summary**를 확인합니다:
    <img src="/images/step7/7-2-step67-summary.png" alt="ALB Summary" class="guide-img-sm" />
...
68. [[Create load balancer]]를 클릭합니다.
    <img src="/images/step7/7-2-step68-create-alb.png" alt="Create load balancer" class="guide-img-sm" />
...
69. Load Balancers 목록에서 `starter-alb`의 **State**를 확인합니다.
    ...
    - State가 Active가 되면 ALB가 트래픽을 받을 준비가 완료된 것입니다.
      <img src="/images/step7/7-2-step69-alb-active.png" alt="ALB Active 상태" class="guide-img-sm" />
      <img src="/images/step7/7-2-step69-alb-detail.png" alt="ALB 상세 정보" class="guide-img-sm" />
...
- 이것이 ALB의 **트래픽 분산(로드밸런싱)**이 동작하는 것입니다.
    ...
    <img src="/images/step7/7-2-step72-http-alert.png" alt="HTTP 사이트 이동 알럿" class="guide-img-sm" />
    <img src="/images/step7/7-2-step72-alb-access.png" alt="ALB를 통한 접속 확인" class="guide-img-sm" />
    <img src="/images/step7/7-2-step72-alb-access2.png" alt="ALB를 통한 접속 확인 - 다른 인스턴스" class="guide-img-sm" />
...
74. **Targets** 탭에서 등록된 인스턴스의 **Health status**가 `healthy`인지 확인합니다.
    <img src="/images/step7/7-2-step74-health-status.png" alt="Health status 확인" class="guide-img-sm" />
...
<img src="/images/step7/7-2-step75-cli-health.png" alt="CLI Health Check 결과" class="guide-img-sm" />
...
### Health Check 동작 확인 — EC2 Stop 테스트

ALB의 Health Check가 비정상 인스턴스를 자동으로 제외하는지 확인합니다.

76. EC2 콘솔 → **Instances**에서 `alb-lab-ec2-1`을 선택합니다.
77. **Instance state** → **Stop instance**를 클릭합니다.
    <img src="/images/step7/7-2-step77-stop-instance.png" alt="Stop instance" class="guide-img-sm" />
78. 확인 팝업에서 [[Stop]]을 클릭합니다.
    <img src="/images/step7/7-2-step78-stop-confirm1.png" alt="Stop 확인 1" class="guide-img-sm" />
    <img src="/images/step7/7-2-step78-stop-confirm2.png" alt="Stop 확인 2" class="guide-img-sm" />
79. EC2 콘솔 → **Target Groups** → `starter-app-tg` → **Targets** 탭을 확인합니다.
    - `alb-lab-ec2-1`의 Health status가 **Unused** ("Target is in the stopped state")로 변경됩니다.
    - ALB는 Unused 상태의 타겟에 트래픽을 보내지 않습니다.
      <img src="/images/step7/7-2-step79-unused-status1.png" alt="Unused 상태 확인 1" class="guide-img-sm" />
      <img src="/images/step7/7-2-step79-unused-status2.png" alt="Unused 상태 확인 2" class="guide-img-sm" />
80. 브라우저에서 ALB DNS Name으로 접속합니다.
    - 새로고침해도 `alb-lab-ec2-2`만 응답합니다 (초록색 배경만 표시).
    - ALB가 정지된 인스턴스를 자동으로 트래픽에서 제외한 것입니다.
...
> ├─────────────────┼──────┼──────────────────────────────────────────────┤
> │ alb-lab-ec2-1   │ 80   │ ⊘ Unused (Target is in the stopped state)   │
> │ alb-lab-ec2-2   │ 80   │ ✅ Healthy                                   │
> └─────────────────┴──────┴──────────────────────────────────────────────┘
> ```

> [!TIP]
> ALB의 **Resource map** 탭에서도 각 타겟의 상태를 시각적으로 확인할 수 있습니다.

### EC2 재시작 → healthy 복귀

81. EC2 콘솔 → `alb-lab-ec2-1` 선택 → **Instance state** → **Start instance**를 클릭합니다.
    <img src="/images/step7/7-2-step81-start-instance.png" alt="Start instance" class="guide-img-sm" />
82. Target Group → **Targets** 탭에서 약 90초(30초 × 3) 후 **Healthy**로 복귀하는지 확인합니다.
    <img src="/images/step7/7-2-step82-healthy-again.png" alt="Healthy 복귀 확인" class="guide-img-sm" />
83. 브라우저에서 새로고침하면 다시 두 인스턴스가 번갈아 응답합니다.

### Nginx 중지 → unhealthy 확인

이번에는 EC2는 실행 중인 상태에서 **애플리케이션만 중지**하여 Health Check 실패를 유도합니다.

84. EC2 콘솔에서 `alb-lab-ec2-1`의 Public IP를 확인합니다.
85. 터미널에서 SSH로 접속합니다:

```bash
ssh -i <키페어.pem> ec2-user@<EC2-1 Public IP>
```
...
<img src="/images/step7/7-2-step86-nginx-stop.png" alt="Nginx 중지" class="guide-img-sm" />

87. Target Group → **Targets** 탭을 확인합니다.
    - 약 60초(30초 × Unhealthy threshold 2) 후 `alb-lab-ec2-1`이 **Unhealthy** ("Health checks failed")로 변경됩니다.
      <img src="/images/step7/7-2-step87-unhealthy.png" alt="Unhealthy 상태" class="guide-img-sm" />
88. 브라우저에서 ALB DNS Name으로 접속합니다.
    - `alb-lab-ec2-2`만 응답합니다 — ALB가 Health Check 실패 인스턴스를 자동 제외했습니다.
      <img src="/images/step7/7-2-step88-only-ec2-2.png" alt="EC2 #2만 응답" class="guide-img-sm" />
      <img src="/images/step7/7-2-step88-alb-access.png" alt="ALB 접속 확인" class="guide-img-sm" />

### Nginx 재시작 → healthy 복귀

89. SSH 세션에서 Nginx를 다시 시작합니다:

```bash
sudo systemctl start nginx
```

<img src="/images/step7/7-2-step89-nginx-start.png" alt="Nginx 재시작" class="guide-img-sm" />

90. Target Group → **Targets** 탭에서 약 90초 후 **Healthy**로 복귀하는지 확인합니다.
    <img src="/images/step7/7-2-step90-healthy-restored.png" alt="Healthy 복귀" class="guide-img-sm" />
91. 브라우저에서 다시 두 인스턴스가 번갈아 응답합니다.

> [!CONCEPT] Health Check가 서비스 안정성을 보장하는 방식
>
> - ALB는 주기적으로(30초마다) 각 타겟에 Health Check 요청을 보냅니다.
> - EC2 자체가 정지(Stop)되면 → **Unused** (트래픽 즉시 제외)
> - EC2는 실행 중이나 앱이 응답 안 하면 → **Unhealthy** (Threshold만큼 실패 후 제외)
> - 타겟이 복구되면(Healthy threshold만큼 연속 성공) 자동으로 트래픽을 다시 받습니다.
> - 사람의 개입 없이 자동으로 장애를 격리하고 복구합니다.

✅ **태스크 완료** — 트래픽 분산, EC2 Stop(Unused), Nginx 중지(Unhealthy), 자동 복구까지 ALB의 핵심 동작을 모두 확인했습니다.
...
92. EC2 콘솔 왼쪽 메뉴에서 **Network & Security** → **Security Groups**를 클릭합니다.
    <img src="/images/step7/7-2-step92-ec2-sg-menu.png" alt="Security Groups 메뉴" class="guide-img-sm" />
93. EC2에 연결된 Security Group을 선택합니다 (예: `alb-lab-ec2-sg`).
    - 여러 SG가 있으면 **Name** 태그를 확인합니다.
94. **Inbound rules** 탭을 클릭합니다.
    <img src="/images/step7/7-2-step94-inbound-tab.png" alt="Inbound rules 탭" class="guide-img-sm" />
95. [[Edit inbound rules]]를 클릭합니다.
96. 기존 **애플리케이션 포트 규칙**(Source: `0.0.0.0/0`, Port: 8080 또는 80)을 찾아 우측 [[Delete]] 버튼으로 삭제합니다.
    - SSH(22번 포트) 규칙은 **삭제하지 않습니다**.
      <img src="/images/step7/7-2-step96-delete-rule.png" alt="기존 규칙 삭제" class="guide-img-sm" />
97. [[Add rule]]을 클릭하고 새 규칙을 추가합니다:
    - **Type**: `Custom TCP`
    ...
    - **Description**: `Allow from ALB only`
      <img src="/images/step7/7-2-step97-add-alb-sg.png" alt="ALB SG를 Source로 추가" class="guide-img-sm" />
...
98. 최종 Inbound rules가 다음과 같은지 확인합니다:
    <img src="/images/step7/7-2-step98-final-rules.png" alt="최종 Inbound rules" class="guide-img-sm" />
...
99. [[Save rules]]를 클릭합니다.
    <img src="/images/step7/7-2-step99-save-rules.png" alt="Save rules" class="guide-img-sm" />

### 변경 후 확인

100. 브라우저에서 **EC2 Public IP:포트**로 직접 접속을 시도합니다:
...
- ❌ 연결 시간 초과 (접근 차단됨) — 정상입니다.
     ...
     <img src="/images/step7/7-2-step100-direct-blocked1.png" alt="직접 접근 차단 1" class="guide-img-sm" />
     <img src="/images/step7/7-2-step100-direct-blocked2.png" alt="직접 접근 차단 2" class="guide-img-sm" />
     <img src="/images/step7/7-2-step100-direct-blocked3.png" alt="직접 접근 차단 3" class="guide-img-sm" />

101. 브라우저에서 **ALB DNS Name**으로 접속합니다:
...
- ✅ 정상 접속 (ALB를 통한 접근은 허용)
     ...
     <img src="/images/step7/7-2-step101-alb-ok.png" alt="ALB 접속 정상" class="guide-img-sm" />
...
## 태스크 7: ALB에 도메인 + HTTPS 적용

Step 7-1에서 발급한 ACM 인증서를 ALB에 연결하고, A 레코드를 ALB Alias로 변경하여 도메인으로 HTTPS 접속할 수 있도록 합니다.

> [!NOTE]
> 이 태스크는 Step 7-1을 완료하고 다음 조건이 충족된 경우에 진행합니다:
>
> - Amazon Route 53 Hosted Zone에 도메인이 등록됨
> - ACM 인증서 상태가 `Issued` (발급 완료)
> - NS 전파가 완료됨 (도메인으로 접속 가능)

### ALB에 HTTPS Listener 추가

102. EC2 콘솔 → **Load Balancing** → **Load Balancers**로 이동합니다.
103. `starter-alb`를 선택합니다.
104. **Listeners and rules** 탭에서 [[Add listener]]를 클릭합니다.
     <img src="/images/step7/7-2-step104-add-listener.png" alt="Add listener" class="guide-img-sm" />
105. **Listener: HTTPS:443** 설정:
     - **Protocol**: `HTTPS`
     - **Port**: `443`

     <img src="/images/step7/7-2-step105-https-listener.png" alt="HTTPS Listener 설정" class="guide-img-sm" />

106. **Default action** 섹션:
     - **Pre-routing action**: `No pre-routing action` (기본값)
     - **Routing action**: `Forward to target groups` (기본값)
     - **Target group**: `starter-app-tg` 선택
     - **Target group stickiness**: 체크하지 않음 (기본값)

107. **Secure listener settings** 섹션:
     - **Security policy**: `ELBSecurityPolicy-TLS13-1-2-Res-PQ-2025-09 (Recommended)` (기본값 유지)
     - **Default SSL/TLS server certificate**:
     - **Certificate source**: `From ACM` 선택
     - **Certificate (from ACM)**: 드롭다운에서 발급받은 인증서 선택 (예: `my-starter-app.shop`)
     - **Client certificate handling**: `Mutual authentication (mTLS)` 체크하지 않음 (기본값)

     <img src="/images/step7/7-2-step107-ssl-cert.png" alt="SSL 인증서 설정" class="guide-img-sm" />

108. **Listener tags – optional** 섹션을 펼치고 태그를 추가합니다:
     - **Key**: `CreatedBy`, **Value**: `admin-user`
     - **Key**: `Step`, **Value**: `step7`
     - **Key**: `Session`, **Value**: `7-2`

     <img src="/images/step7/7-2-step108-listener-tags.png" alt="Listener tags" class="guide-img-sm" />

109. [[Add listener]]를 클릭합니다.
     <img src="/images/step7/7-2-step109-listener-created.png" alt="Listener 생성 완료" class="guide-img-sm" />

> [!NOTE]
> ALB Security Group(`alb-sg`)에 HTTPS(443) Inbound 규칙이 이미 있는지 확인하세요.  
> 태스크 4에서 HTTP(80) + HTTPS(443)를 모두 추가했으므로 추가 설정은 불필요합니다.

> [!TIP]
> **HTTP(80) → HTTPS(443) 리다이렉트 설정:**
>
> 1. **Listeners and rules** 탭에서 `HTTP:80`을 체크합니다.
> 2. **Manage listener** 드롭다운 → **Edit listener**를 클릭합니다.
> 3. **Routing action**을 `Redirect to URL`로 변경합니다.
> 4. **Protocol**: `HTTPS`, **Port**: `443`, **Status code**: `301 - Permanently moved`
> 5. [[Save changes]]를 클릭합니다.
>
> 이렇게 하면 `http://내도메인`으로 접속해도 자동으로 `https://내도메인`으로 전환됩니다.
>
> <img src="/images/step7/7-2-step109-redirect1.png" alt="HTTP→HTTPS 리다이렉트 1" class="guide-img-sm" />
> <img src="/images/step7/7-2-step109-redirect2.png" alt="HTTP→HTTPS 리다이렉트 2" class="guide-img-sm" />
> <img src="/images/step7/7-2-step109-redirect3.png" alt="HTTP→HTTPS 리다이렉트 3" class="guide-img-sm" />
> <img src="/images/step7/7-2-step109-redirect4.png" alt="HTTP→HTTPS 리다이렉트 4" class="guide-img-sm" />

### Amazon Route 53 A 레코드를 ALB Alias로 변경

110. Amazon Route 53 콘솔 → **Hosted zones** → 도메인을 클릭합니다.
111. 기존 A 레코드(EC2 IP)를 선택하고 [[Edit record]]를 클릭합니다.
     <img src="/images/step7/7-2-step111-edit-record.png" alt="Edit record" class="guide-img-sm" />
112. 다음과 같이 변경합니다:
     - **Alias** 토글: ON
     - **Route traffic to**: **Alias to Application and Classic Load Balancer**
     - **Region**: `ap-northeast-2`
     - **Load balancer**: `starter-alb` 선택

     <img src="/images/step7/7-2-step112-alias-alb.png" alt="Alias to ALB 설정" class="guide-img-sm" />

113. [[Save]]를 클릭합니다.
     <img src="/images/step7/7-2-step113-save-record.png" alt="Save record" class="guide-img-sm" />

> [!CONCEPT] A 레코드 vs CNAME vs Alias 레코드
>
> | 타입                             | 값                                    | 루트 도메인 사용 | 비고                               |
> | -------------------------------- | ------------------------------------- | :--------------: | ---------------------------------- |
> | A 레코드                         | IP 주소 (`3.35.x.x`)                  |        ✅        | IP 변경 시 수동 업데이트 필요      |
> | CNAME                            | 다른 도메인 (`xxx.elb.amazonaws.com`) |        ❌        | DNS 표준상 루트 도메인에 사용 불가 |
> | A + Alias (Amazon Route 53 전용) | AWS 리소스 (ALB, CloudFront 등)       |        ✅        | IP 자동 반영, 쿼리 비용 무료       |
>
> - **루트 도메인**(`my-starter-app.shop`)에는 CNAME을 설정할 수 없습니다 — DNS RFC 규약 제한입니다.
> - Amazon Route 53의 **Alias**는 A 레코드 타입이면서 내부적으로 ALB의 IP를 자동 반환하므로 루트 도메인에서도 사용 가능합니다.
> - ALB를 사용할 때는 항상 **A 레코드 + Alias ON**으로 설정합니다.

### 도메인 HTTPS 접속 확인

114. 브라우저에서 `https://내도메인.shop`으로 접속합니다.
     - 🔒 자물쇠 아이콘이 표시되면 HTTPS 적용 성공입니다.

     <img src="/images/step7/7-2-step114-https-success.png" alt="HTTPS 접속 성공" class="guide-img-sm" />

115. `http://내도메인.shop`으로도 접속하여 HTTPS로 리다이렉트되는지 확인합니다.
     <img src="/images/step7/7-2-step115-http-redirect.png" alt="HTTP→HTTPS 리다이렉트 확인" class="guide-img-sm" />

> [!TROUBLESHOOTING]
> | 증상 | 원인 | 해결 |
> |------|------|------|
> | ERR_SSL_PROTOCOL_ERROR | HTTPS Listener 미설정 | Listeners 탭에서 443 확인 |
> | 인증서 선택 불가 | 리전 불일치 | ALB와 같은 리전(ap-northeast-2)에서 발급한 인증서인지 확인 |
> | "Your connection is not private" | 인증서 도메인 불일치 | ACM 인증서의 도메인이 접속 도메인과 일치하는지 확인 |
> | 도메인 접속 안 됨 | A 레코드 미설정 또는 NS 미전파 | Amazon Route 53 레코드 확인 + `dig @8.8.8.8` |

✅ **태스크 완료** — ALB에 HTTPS를 적용하고 도메인으로 안전하게 접속할 수 있습니다.
...
4. [[Search resources]]를 클릭합니다.
   <img src="/images/step7/7-2-cleanup4-tag-editor.png" alt="Tag Editor 검색" class="guide-img-sm" />
...
8. **Actions** 드롭다운 → **Delete load balancer**를 클릭합니다.
   <img src="/images/step7/7-2-cleanup8-alb-actions.png" alt="ALB Delete 선택" class="guide-img-sm" />
9. 확인 입력란에 `confirm`을 입력합니다.
   <img src="/images/step7/7-2-cleanup9-alb-confirm.png" alt="ALB 삭제 확인" class="guide-img-sm" />
10. [[Delete]]를 클릭합니다.
    <img src="/images/step7/7-2-cleanup10-alb-deleted.png" alt="ALB 삭제 완료" class="guide-img-sm" />
...
14. **Actions** 드롭다운 → **Delete**를 클릭합니다.
    <img src="/images/step7/7-2-cleanup14-tg-actions.png" alt="Target Group Delete" class="guide-img-sm" />
15. 확인 팝업에서 [[Delete]]를 클릭합니다.
    <img src="/images/step7/7-2-cleanup15-tg-confirm1.png" alt="Target Group 삭제 확인 1" class="guide-img-sm" />
    <img src="/images/step7/7-2-cleanup15-tg-confirm2.png" alt="Target Group 삭제 확인 2" class="guide-img-sm" />
...
17. EC2에 연결된 Security Group을 선택합니다 (예: `alb-lab-ec2-sg`).
    <img src="/images/step7/7-2-cleanup17-ec2-sg.png" alt="EC2 Security Group 선택" class="guide-img-sm" />
18. **Inbound rules** 탭 → [[Edit inbound rules]]를 클릭합니다.
19. Source가 `alb-sg`(sg-xxx)로 설정된 규칙을 [[Delete]] 버튼으로 삭제합니다.
    <img src="/images/step7/7-2-cleanup19-delete-rule.png" alt="ALB SG 참조 규칙 삭제" class="guide-img-sm" />
20. [[Save rules]]를 클릭합니다.
    <img src="/images/step7/7-2-cleanup20-save-rules.png" alt="Save rules" class="guide-img-sm" />
...
22. **Actions** 드롭다운 → **Delete security groups**를 클릭합니다.
    <img src="/images/step7/7-2-cleanup22-alb-sg-actions.png" alt="ALB SG 삭제" class="guide-img-sm" />
23. 확인 팝업에서 [[Delete]]를 클릭합니다.
    <img src="/images/step7/7-2-cleanup23-alb-sg-confirm1.png" alt="ALB SG 삭제 확인 1" class="guide-img-sm" />
    <img src="/images/step7/7-2-cleanup23-alb-sg-confirm2.png" alt="ALB SG 삭제 확인 2" class="guide-img-sm" />
...
26. [[Delete]]를 클릭합니다.
27. 확인 팝업에서 스택 이름 `step7-2-alb-prereq`를 입력하고 [[Delete stack]]을 클릭합니다.
    <img src="/images/step7/7-2-cleanup27-cfn-confirm.png" alt="CloudFormation 스택 삭제 확인" class="guide-img-sm" />
28. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 2~3분).
    <img src="/images/step7/7-2-cleanup28-cfn-deleted.png" alt="CloudFormation 스택 삭제 완료" class="guide-img-sm" />
...
32. Tag Editor에서 최종 확인합니다:
    ...
    - 검색 결과가 없으면 모든 태그된 리소스가 정리된 것입니다.
      <img src="/images/step7/7-2-cleanup32-final-check.png" alt="Tag Editor 최종 확인" class="guide-img-sm" />
