...
Amazon EC2 인스턴스를 자동으로 확장/축소하는 방법을 학습합니다.
...
2. AWS CloudFormation 콘솔에서 [[Create stack]] → **With new resources (standard)**를 선택합니다.
...
```
수동 확장:
7. 모니터링 → CPU 90% 감지 (사람이 확인)
8. EC2 콘솔 접속 → 인스턴스 생성 (5~10분)
9. 앱 배포 + 설정 (10~20분)
10. Target Group에 등록 (수동)
11. 트래픽 감소 → 인스턴스 종료 (잊어버리면 비용 낭비)
...
```
자동 확장:
12. Amazon CloudWatch → CPU 70% 초과 감지 (자동)
13. ASG → Launch Template으로 인스턴스 자동 생성 (2~3분)
14. User Data로 앱 자동 배포 (자동)
15. Target Group에 자동 등록 (자동)
16. 트래픽 감소 → 인스턴스 자동 종료 (자동)
...
17. EC2 콘솔 → **Instances** → **Launch Templates**로 이동합니다.
18. [[Create launch template]]을 클릭합니다.

### Launch template name and description

19. **Launch template name**: `starter-app-lt`
20. **Template version description**: `v1 - Spring Boot app with auto start`
21. ☑️ **Auto Scaling guidance** 체크 (ASG에서 사용할 수 있도록 가이드 제공)

### Application and OS Images (AMI)

22. **Quick Start** → **Amazon Linux** 선택
23. **Amazon Linux 2023 AMI** 선택

> [!NOTE]
> AMI는 인스턴스의 OS 이미지입니다. Amazon Linux 2023은 최신 보안 패치가
> 적용된 AWS 최적화 Linux입니다.

### Instance type

24. **Instance type**: `t3.micro`

### Key pair

25. **Key pair**: 기존 Key Pair를 선택합니다.
...
26. **Security groups**: EC2용 Security Group을 선택합니다.
    - Step 7-2에서 생성한 Security Group (ALB에서 8080 허용)
...
39. **Load balancing**: **Attach to an existing load balancer**를 선택합니다.
40. **Choose from your load balancer target groups**를 선택합니다.
41. **Existing load balancer target groups**: `starter-app-tg | HTTP` 선택
...
42. **Health check type**:
    - ✅ EC2 (기본)
    - ✅ ELB (추가 체크 — ALB Health Check도 활용)
43. **Health check grace period**: `300` seconds (5분)
...
46. **Scaling policies**: **Target tracking scaling policy**를 선택합니다.
...
>
> AWS가 자동으로 Amazon CloudWatch 알람을 생성하고 관리합니다.
> 가장 간단하고 권장되는 스케일링 방식입니다.

47. **Instance scale-in protection**: 체크하지 않음 (기본값)
48. [[Next]]를 클릭합니다.

### Step 5: Add notifications (선택)

49. 알림이 필요하면 SNS Topic을 설정합니다. 이 실습에서는 건너뜁니다.
50. [[Next]]를 클릭합니다.
...
54. [[Create Auto Scaling group]]을 클릭합니다.
...
### Amazon CloudWatch 알람 자동 생성 확인

55. Amazon CloudWatch 콘솔로 이동합니다.
56. 왼쪽 메뉴에서 **Alarms** → **All alarms**를 클릭합니다.
57. ASG가 자동으로 생성한 알람 2개를 확인합니다:
...
> [!NOTE]
> Target Tracking 정책은 Amazon CloudWatch 알람을 자동으로 생성하고 관리합니다.
> 이 알람을 수동으로 수정하거나 삭제하지 마세요. ASG가 자동으로 관리합니다.
> Scale In 임계값(63%)은 AWS가 자동으로 계산합니다 (목표값의 약 90%).

### ASG Activity 확인

58. EC2 콘솔 → **Auto Scaling Groups** → `starter-app-asg` 클릭
59. **Activity** 탭을 클릭합니다.
60. 인스턴스 생성 활동을 확인합니다:
...
61. **Automatic scaling** 탭을 클릭합니다.
62. `cpu-target-tracking` 정책을 확인합니다:
...
63. EC2 콘솔 → **Instances**로 이동합니다.
64. `starter-app-asg-instance` 이름의 인스턴스 2개가 Running 상태인지 확인합니다.
65. 각 인스턴스가 서로 다른 AZ에 배치되었는지 확인합니다:
...
66. EC2 콘솔 → **Target Groups** → `starter-app-tg` 클릭
67. **Targets** 탭에서 등록된 인스턴스를 확인합니다:
...
68. Load Balancers → `starter-alb` (또는 `asg-lab-alb`) → DNS name 복사
69. 브라우저에서 접속합니다:
...
70. 여러 번 새로고침하여 트래픽이 분산되는지 확인합니다.
...
71. EC2 콘솔에서 ASG가 생성한 인스턴스 1개를 **수동으로 종료**합니다.

- 인스턴스 선택 → **Instance state** → [[Terminate instance]]

72. 1~2분 후 ASG가 새 인스턴스를 자동으로 생성하는지 확인합니다.
...
73. EC2 콘솔 → **Instances**에서 ASG가 생성한 인스턴스 중 하나를 선택합니다.
74. **Actions** → **Image and templates** → **Create image**를 클릭합니다.
75. 다음을 설정합니다:
    - **Image name**: `starter-app-ami-v1`
    - **Image description**: (비워둡니다)
    - **No reboot**: ✅ 체크 (인스턴스를 재부팅하지 않고 이미지 생성)
    - 나머지: 기본값 유지
76. [[Create image]]를 클릭합니다.
77. 왼쪽 메뉴 **Images** → **AMIs**에서 생성 상태를 확인합니다.
...
78. EC2 콘솔 → **Instances** → **Launch Templates**로 이동합니다.
79. `starter-app-lt`를 선택하고 **Actions** → **Modify template (Create new version)**을 클릭합니다.
80. **Source template version**: 기존 버전을 선택합니다.
81. **Application and OS Images (AMI)** 섹션에서:
    - **My AMIs** 탭을 클릭합니다.
    - `starter-app-ami-v1`을 선택합니다.
82. **Advanced details** → **User Data**: 내용을 비웁니다 (AMI에 이미 포함되어 있으므로 불필요).
83. [[Create template version]]을 클릭합니다.

### ASG에 새 버전 적용

84. EC2 콘솔 → **Auto Scaling** → **Auto Scaling Groups**로 이동합니다.
85. `starter-app-asg`를 선택하고 **Details** 탭의 [[Edit]]을 클릭합니다.
86. **Launch template** → **Version**: `Latest`로 변경합니다.
87. [[Update]]를 클릭합니다.
...
91. **2~3분 후** Amazon CloudWatch 알람 상태를 확인합니다:

- `TargetTracking-...-AlarmHigh` → **In alarm** 상태

92. **3~5분 후** ASG Activity를 확인합니다:
...
93. EC2 콘솔에서 인스턴스가 3개로 증가했는지 확인합니다.
94. Target Group에서 새 인스턴스가 등록되고 healthy 상태가 되는지 확인합니다.

### Scale In 확인

95. stress 명령이 종료된 후 (5분 후) CPU가 정상으로 돌아옵니다.
96. **약 15분 후** Scale In이 발생합니다:
...
> [!TIP]
> Step 7 전체를 완료했습니다. 도메인(Amazon Route 53) + HTTPS(ACM) + 로드밸런서(ALB) + 자동 확장(ASG) 구성을 통해  
> 고가용성 웹 서비스의 기본 아키텍처를 구축할 수 있게 되었습니다.
...
> | Amazon CloudWatch 알람    | 알람당 월별       | ASG 삭제 시 자동 삭제           |
...
>
> ```
> 삭제 순서: Tag Editor 확인 → ASG → Launch Template → ALB → Target Group → AWS AWS CloudFormation → Tag Editor 최종 확인
>
...
> (5) AWS CloudFormation Delete (VPC, Subnet, SG 일괄 삭제)
> ```
>
...
7. `starter-app-asg`를 선택합니다.
8. [[Delete]]를 클릭합니다.
9. 확인 입력란에 `delete`를 입력합니다.
10. [[Delete]]를 클릭합니다.
11. Amazon EC2 콘솔 → **Instances**에서 ASG가 관리하던 인스턴스가 `Shutting down` → `Terminated` 상태로 변경되는 것을 확인합니다 (약 1-2분).

> [!NOTE]
> ASG를 삭제하면 관리 중인 모든 Amazon EC2 인스턴스가 **자동으로 종료**됩니다. 별도로 인스턴스를 종료할 필요가 없습니다.
...
### 단계 4: ALB + Target Group 삭제

Step 7-2에서 생성한 ALB와 Target Group을 삭제합니다. (이미 삭제한 경우 건너뜁니다.)

**ALB 삭제:**

17. Amazon EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Load Balancers**를 클릭합니다.
18. ALB를 선택합니다 (예: `starter-alb` 또는 `asg-lab-alb`).
19. **Actions** → **Delete load balancer**를 클릭합니다.
20. 확인 입력란에 `confirm`을 입력합니다.
21. [[Delete]]를 클릭합니다.

**Target Group 삭제:**

22. Amazon EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
23. `starter-app-tg`를 선택합니다.
24. **Actions** → **Delete**를 클릭합니다.
25. 확인 팝업에서 [[Yes, delete]]를 클릭합니다.
...
27. **Stacks** 목록에서 `step7-3-asg-prereq` 스택을 선택합니다.
28. [[Delete]] 버튼을 클릭합니다.
29. 확인 팝업에서 [[Delete stack]]을 클릭합니다.
30. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 3-5분).
...
### 단계 6: Amazon CloudWatch 알람 삭제 확인

ASG의 Target Tracking 정책이 자동으로 생성한 Amazon CloudWatch 알람이 삭제되었는지 확인합니다.

31. 상단 검색창에 `CloudWatch`를 입력하고 선택합니다.
32. 왼쪽 메뉴에서 **Alarms** → **All alarms**를 클릭합니다.
33. `TargetTracking-starter-app-asg`로 시작하는 알람이 목록에서 사라졌는지 확인합니다.

> [!NOTE]
> ASG를 삭제하면 Target Tracking 정책이 생성한 Amazon CloudWatch 알람도 자동으로 삭제됩니다. 만약 알람이 남아있다면 선택 후 **Actions** → **Delete**로 수동 삭제하세요.
...
34. 상단 검색창에 `Resource Groups & Tag Editor`를 입력하고 선택합니다.
35. 왼쪽 메뉴에서 **Tag Editor**를 선택합니다.
36. 다음 조건으로 검색합니다:
...
37. [[Search resources]]를 클릭합니다.
38. 검색 결과가 없으면 모든 태그된 리소스가 정리된 것입니다.
