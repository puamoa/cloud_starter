...
>
> - `alb-lab-prereq.yaml` - AWS CloudFormation 템플릿 (태스크 0에서 VPC, 서브넷, IGW, EC2, Security Group 자동 생성)
...
3. 상단 검색창에 `EC2`를 입력하고 선택합니다.
4. 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
5. [[Create target group]]을 클릭합니다.

### Basic configuration

6. **Choose a target type**: **Instances**를 선택합니다.
7. **Target group name**: `starter-app-tg`
8. **Protocol**: HTTP
9. **Port**: `8080`
...
11. **VPC**: 실습용 VPC를 선택합니다 (예: `alb-lab-vpc`).
12. **Protocol version**: HTTP1

### Health checks

13. **Health check protocol**: HTTP
14. **Health check path**: `/actuator/health`
...
1. EC2 콘솔 → **Load Balancing** → **Target Groups**로 이동합니다.
2. `starter-app-tg`를 클릭합니다.
3. **Targets** 탭을 클릭합니다.
4. [[Register targets]]를 클릭합니다.

### Available instances에서 EC2 선택

5. 목록에서 등록할 EC2 인스턴스를 체크합니다.
6. **Ports for the selected instances**: `8080` (기본값 유지)
7. [[Include as pending below]]를 클릭합니다.

> [!NOTE]
> "Include as pending below"를 클릭하면 하단의 **Review targets** 영역에
> 선택한 인스턴스가 추가됩니다. 여러 인스턴스를 한 번에 등록할 수 있습니다.

8. 하단 **Review targets**에서 등록할 인스턴스를 확인합니다.
9. [[Register pending targets]]를 클릭합니다.

### Health Status 확인

10. Target Group 상세 → **Targets** 탭에서 등록된 인스턴스의 상태를 확인합니다.
...
5. **Outbound rules**는 기본값(All traffic, 0.0.0.0/0)을 유지합니다.
6. [[Create security group]]을 클릭합니다.
...
7. EC2 콘솔 → **Load Balancing** → **Load Balancers**로 이동합니다.
8. [[Create load balancer]]를 클릭합니다.
9. **Application Load Balancer** 섹션에서 [[Create]]를 클릭합니다.

### Basic configuration

10. **Load balancer name**: `starter-alb`
11. **Scheme**: **Internet-facing** (인터넷에서 접근 가능)
12. **IP address type**: IPv4
...
13. **VPC**: 실습용 VPC를 선택합니다.
14. **Mappings**: 최소 2개의 Availability Zone을 선택합니다.
...
15. 기본 Security Group을 제거하고, 앞서 생성한 `alb-sg`를 선택합니다.
...
17. 나머지 설정은 기본값을 유지합니다.
18. [[Create load balancer]]를 클릭합니다.

### ALB 생성 확인

19. Load Balancers 목록에서 `starter-alb`의 State를 확인합니다.
...
1. Load Balancers → `starter-alb` 클릭
2. **Description** 탭에서 **DNS name**을 복사합니다.
3. 브라우저에서 접속합니다:
...
5. **Targets** 탭에서 등록된 인스턴스의 **Health status**를 확인합니다.
...
1. EC2 콘솔 → **Network & Security** → **Security Groups**로 이동합니다.
2. EC2에 연결된 Security Group을 선택합니다 (예: `EC2SecurityGroup`).
3. **Inbound rules** 탭 → [[Edit inbound rules]]를 클릭합니다.
4. 기존 8080 포트 규칙을 **삭제**합니다 (Source: 0.0.0.0/0인 규칙).
5. [[Add rule]]을 클릭하고 새 규칙을 추가합니다:
...
6. SSH 규칙(22번 포트)은 유지합니다 (관리 목적).
7. [[Save rules]]를 클릭합니다.

### 변경 후 확인

8. 브라우저에서 **EC2 Public IP:8080**으로 직접 접속을 시도합니다.
   - ❌ 연결 시간 초과 (접근 차단됨)

9. 브라우저에서 **ALB DNS Name**으로 접속합니다.
...
2. 왼쪽 메뉴에서 **Load Balancing** → **Load Balancers**를 클릭합니다.
3. `starter-alb`를 선택합니다.
4. **Actions** → **Delete load balancer**를 클릭합니다.
5. 확인 입력란에 `confirm`을 입력합니다.
6. [[Delete]]를 클릭합니다.
7. 목록에서 ALB가 사라지는 것을 확인합니다.
...
1. EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
2. `starter-app-tg`를 선택합니다.
3. **Actions** → **Delete**를 클릭합니다.
4. 확인 팝업에서 [[Yes, delete]]를 클릭합니다.
...
1. EC2 콘솔 왼쪽 메뉴에서 **Network & Security** → **Security Groups**를 클릭합니다.
2. `alb-sg`를 선택합니다.
3. **Actions** → **Delete security groups**를 클릭합니다.
...
2. EC2에 연결된 Security Group을 선택합니다 (예: `EC2SecurityGroup`).
3. **Inbound rules** 탭 → [[Edit inbound rules]]를 클릭합니다.
4. Source가 `alb-sg`로 설정된 8080 포트 규칙을 삭제합니다.
5. [[Add rule]]을 클릭하고 새 규칙을 추가합니다:
   - **Type**: Custom TCP
   - **Port range**: 8080
   - **Source**: `0.0.0.0/0` (또는 본인 IP)
6. [[Save rules]]를 클릭합니다.
...
2. **Stacks** 목록에서 `alb-lab-prereq` 스택을 선택합니다.
3. [[Delete]] 버튼을 클릭합니다.
...
1. EC2 콘솔 → **Load Balancers**에서 `starter-alb`가 목록에서 사라졌는지 확인합니다.
2. EC2 콘솔 → **Target Groups**에서 `starter-app-tg`가 삭제되었는지 확인합니다.
3. EC2 콘솔 → **Security Groups**에서 `alb-sg`가 삭제되었는지 확인합니다.
4. CloudFormation 콘솔에서 `alb-lab-prereq` 스택이 목록에서 사라졌는지 확인합니다.
