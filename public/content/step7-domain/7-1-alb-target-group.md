...
7. [[Choose file]] 버튼을 클릭하고 다운로드한 `step7-1-alb-prereq.yaml` 파일을 선택합니다.
8. [[Next]] 버튼을 클릭합니다.

9. **Stack name**에 `step7-1-alb-prereq`를 입력합니다.
...
16. AWS CloudFormation 콘솔 → Stacks → `step7-1-alb-prereq` → **Outputs** 탭을 클릭합니다.
...
> Step 7-2에서 배울 **Auto Scaling Group**은 이 과정을 자동화합니다. — 인스턴스 생성 시 자동 등록, 종료 시 자동 해제
...
> [!NOTE]
> Amazon EC2를 더 이상 사용하지 않거나, 다음 단계에서 AWS CloudFormation 스택을 삭제할 경우 이 단계는 건너뛰어도 됩니다.
