...
| 세션                | 주제               | 핵심 리소스                     |
| ------------------- | ------------------ | ------------------------------- |
| **7-1 (이번 실습)** | ALB + Target Group | 트래픽 분산, Health Check       |
| 7-2                 | Auto Scaling Group | 자동 확장/축소, Launch Template |
| 7-3                 | Route 53 + ACM     | 도메인 연결, HTTPS 인증서       |
...
| Amazon EC2 인스턴스 | 1개 이상 (포트 8080 또는 80에서 HTTP 200 응답) | ☐    |
...
5. **Prepare template** 섹션:
   - **Template is ready** 선택 (기본값)
6. **Specify template** 섹션:
   - **Upload a template file** 선택
   - [[Choose file]]을 클릭하고 다운로드한 `step7-1-alb-prereq.yaml` 파일을 업로드합니다.
7. [[Next]]를 클릭합니다.

8. **Specify stack details** 페이지에서 다음을 설정합니다:
   - **Stack name**: `step7-1-alb-prereq`
   - **Required Settings** 그룹:
     - **ProjectName**: `alb-lab` (기본값) — 리소스 이름 접두사로 사용됩니다.
...
>
> 이전 차시에서 Spring Boot를 배포한 Amazon EC2가 있다면 8080을 선택합니다.  
> AWS CloudFormation으로 새로 만드는 경우 Nginx가 설치되므로 어떤 값이든 동작합니다.

9. [[Next]]를 클릭합니다.
10. **Configure stack options** 페이지:
    - 기본값을 유지합니다.
    - [[Next]]를 클릭합니다.
11. **Review and create** 페이지에서 설정 내용을 확인합니다.
12. [[Submit]]을 클릭합니다.

> [!WARNING]
> AWS CloudFormation 스택 생성에 2~3분이 소요됩니다.  
...
14. 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
    - 왼쪽 메뉴가 보이지 않으면 좌측 상단의 햄버거 버튼(☰)을 클릭하여 메뉴를 펼칩니다.
15. [[Create target group]]을 클릭합니다.

### Step 1: Specify group details

**Basic configuration** 섹션을 설정합니다:

16. **Choose a target type** — 타겟의 종류를 선택합니다:
    - **Instances** 선택
    - 다른 옵션(IP addresses, Lambda function, Application Load Balancer)은 이 실습에서 사용하지 않습니다.
17. **Target group name** — Target Group의 이름을 입력합니다:
    - `starter-app-tg`를 입력합니다.
18. **Protocol : Port** — ALB가 타겟에 요청을 전달할 때 사용하는 프로토콜과 포트입니다:
...
19. **IP address type**: `IPv4`를 선택합니다.
20. **VPC** — Target Group이 속할 VPC를 선택합니다:
    - 실습용 VPC를 선택합니다 (예: `alb-lab-vpc`).
    - 드롭다운에 여러 VPC가 표시되면, 이름 태그를 확인하여 올바른 VPC를 선택합니다.
21. **Protocol version**: `HTTP1`을 선택합니다 (기본값).

**Health checks** 섹션을 설정합니다:

22. **Health check protocol**: `HTTP`를 선택합니다.
23. **Health check path** — ALB가 타겟의 정상 여부를 확인하기 위해 요청을 보내는 경로입니다:
...
> | Nginx (CloudFormation 생성)   | `/`                | Nginx 기본 환영 페이지   |
>
> Health Check 경로는 **HTTP 200 응답**을 반환하는 엔드포인트여야 합니다.  
> 잘못된 경로를 입력하면 모든 타겟이 unhealthy로 표시됩니다.

24. **Advanced health check settings**를 펼칩니다. 각 항목을 다음과 같이 설정합니다:
...
26. 이 단계에서는 타겟을 등록하지 않고 바로 [[Create target group]]을 클릭합니다.
    - 타겟 등록은 다음 태스크에서 별도로 진행합니다.
...
27. EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Target Groups**를 클릭합니다.
28. `starter-app-tg`를 클릭하여 상세 페이지로 이동합니다.
29. **Targets** 탭을 클릭합니다.
30. [[Register targets]]를 클릭합니다.

### Available instances에서 EC2 선택

31. **Available instances** 목록에서 등록할 Amazon EC2 인스턴스를 체크합니다.
    - AWS CloudFormation으로 생성한 경우: `alb-lab-ec2`를 선택합니다.
    - 기존 EC2를 사용하는 경우: 해당 인스턴스를 선택합니다.
32. **Ports for the selected instances**: 애플리케이션 포트를 확인합니다.
    - 기본적으로 Target Group 생성 시 설정한 포트(8080 또는 80)가 표시됩니다.
    - 변경할 필요가 없으면 그대로 둡니다.
33. [[Include as pending below]]를 클릭합니다.
    - 하단의 **Review targets** 영역에 선택한 인스턴스가 추가됩니다.
    - 여러 인스턴스를 등록하려면 31~33번을 반복합니다.
34. 하단 **Review targets**에서 등록할 인스턴스와 포트가 올바른지 확인합니다.
35. [[Register pending targets]]를 클릭합니다.

### Health Status 확인

36. Target Group 상세 → **Targets** 탭에서 등록된 인스턴스의 상태를 확인합니다.
...
37. EC2 콘솔 왼쪽 메뉴에서 **Network & Security** → **Security Groups**를 클릭합니다.
38. [[Create security group]]을 클릭합니다.
39. **Basic details** 섹션을 설정합니다:
    - **Security group name**: `alb-sg`
    - **Description**: `Allow HTTP from internet to ALB`
    - **VPC**: 실습용 VPC를 선택합니다 (예: `alb-lab-vpc`).
      - ⚠️ 반드시 Target Group과 같은 VPC를 선택해야 합니다.
40. **Inbound rules** 섹션에서 [[Add rule]]을 2번 클릭하여 다음 규칙을 추가합니다:
...
44. EC2 콘솔 왼쪽 메뉴에서 **Load Balancing** → **Load Balancers**를 클릭합니다.
45. [[Create load balancer]]를 클릭합니다.
46. 로드밸런서 유형 선택 화면에서 **Application Load Balancer** 섹션의 [[Create]]를 클릭합니다.
    - Network Load Balancer, Gateway Load Balancer는 이 실습에서 사용하지 않습니다.

### Basic configuration

47. **Load balancer name**: `starter-alb`를 입력합니다.
    - 이름은 영문, 숫자, 하이픈만 사용할 수 있습니다.
48. **Scheme** — ALB의 접근 범위를 결정합니다:
    - **Internet-facing** 선택 (인터넷에서 접근 가능)
    - Internal은 VPC 내부에서만 접근 가능하므로 이 실습에서는 선택하지 않습니다.
49. **IP address type**: `IPv4`를 선택합니다 (기본값).
...
50. **VPC** — ALB가 배치될 VPC를 선택합니다:
    - 실습용 VPC를 선택합니다 (예: `alb-lab-vpc`).
51. **Mappings** — ALB를 배치할 AZ와 서브넷을 선택합니다:
    - ✅ `ap-northeast-2a` 체크 → 드롭다운에서 **Public Subnet 1** 선택 (예: `alb-lab-public-subnet-1`)
    - ✅ `ap-northeast-2c` 체크 → 드롭다운에서 **Public Subnet 2** 선택 (예: `alb-lab-public-subnet-2`)
...
52. 기본 Security Group(default)을 ✕ 버튼으로 제거합니다.
53. 드롭다운에서 앞서 생성한 `alb-sg`를 선택합니다.

### Listeners and routing

54. **Listener** 설정 — ALB가 수신하는 포트와 요청을 전달할 대상을 설정합니다:
    - **Protocol**: `HTTP`
    - **Port**: `80`
    - **Default action** → **Forward to**: 드롭다운에서 `starter-app-tg`를 선택합니다.

> [!TIP]
> Listener는 ALB가 수신하는 포트입니다.  
> 사용자는 ALB의 **80번 포트**로 접속하고, ALB는 Target Group의 EC2 **8080번(또는 80번) 포트**로 요청을 전달합니다.  
> 즉, 사용자는 포트 번호 없이 `http://alb-dns-name`으로 접속할 수 있습니다.

### Tags (선택)

55. 하단의 **Tags** 섹션을 펼치고 태그를 추가합니다:
    - **Key**: `CreatedBy`, **Value**: `admin-user`
    - **Key**: `Step`, **Value**: `step7`
    - **Key**: `Session`, **Value**: `7-1`
...
56. 우측의 **Summary** 패널에서 설정 내용을 확인합니다:
    - Scheme: Internet-facing
    - VPC: alb-lab-vpc
    - AZ: 2개 선택됨
    - Security Group: alb-sg
    - Listener: HTTP:80 → starter-app-tg
57. [[Create load balancer]]를 클릭합니다.

### ALB 생성 확인

58. Load Balancers 목록에서 `starter-alb`의 **State**를 확인합니다.
...
59. Load Balancers 목록에서 `starter-alb`를 클릭합니다.
60. **Description** 탭에서 **DNS name**을 복사합니다.
    - DNS name 옆의 복사 아이콘(📋)을 클릭하면 클립보드에 복사됩니다.
61. 브라우저 주소창에 붙여넣고 접속합니다:
...
- Nginx가 실행 중이라면 → "Welcome to nginx!" 기본 환영 페이지가 표시됩니다.
...
62. EC2 콘솔 → **Load Balancing** → **Target Groups** → `starter-app-tg`를 클릭합니다.
63. **Targets** 탭에서 등록된 인스턴스의 **Health status**가 `healthy`인지 확인합니다.
...
64. 터미널(또는 AWS CloudShell)에서 다음 명령어를 실행합니다:
...
> [!TIP]
> `<Target Group ARN>`은 Target Groups 목록에서 `starter-app-tg`를 선택하면  
> 상세 정보의 **ARN** 필드에 표시됩니다. 복사하여 사용하세요.
...
65. EC2 콘솔 왼쪽 메뉴에서 **Network & Security** → **Security Groups**를 클릭합니다.
66. EC2에 연결된 Security Group을 선택합니다 (예: `alb-lab-ec2-sg`).
    - 여러 SG가 있으면 **Name** 태그를 확인합니다.
67. **Inbound rules** 탭을 클릭합니다.
68. [[Edit inbound rules]]를 클릭합니다.
69. 기존 **애플리케이션 포트 규칙**(Source: `0.0.0.0/0`, Port: 8080 또는 80)을 찾아 우측 [[Delete]] 버튼으로 삭제합니다.
    - SSH(22번 포트) 규칙은 **삭제하지 않습니다**.
70. [[Add rule]]을 클릭하고 새 규칙을 추가합니다:
    - **Type**: `Custom TCP`
...
71. 최종 Inbound rules가 다음과 같은지 확인합니다:
...
72. [[Save rules]]를 클릭합니다.

### 변경 후 확인

73. 브라우저에서 **EC2 Public IP:포트**로 직접 접속을 시도합니다:
...
74. 브라우저에서 **ALB DNS Name**으로 접속합니다:
...
> [!TIP]
> EC2 Public IP는 EC2 콘솔 → Instances → 해당 인스턴스 선택 → **Details** 탭의  
> **Public IPv4 address** 필드에서 확인할 수 있습니다.  
> 또는 AWS CloudFormation 콘솔 → Stacks → `step7-1-alb-prereq` → **Outputs** 탭의 `EC2PublicIP`에서도 확인 가능합니다.
...
> ALB, Target Group, VPC, EC2, Security Group을 삭제하지 마세요. Step 7-2에서 재사용합니다.  
...
17. `alb-sg`를 선택합니다.
18. **Actions** 드롭다운 → **Delete security groups**를 클릭합니다.
19. 확인 팝업에서 [[Delete]]를 클릭합니다.

> [!NOTE]
> ALB 삭제 후에도 Security Group 삭제까지 몇 분 걸릴 수 있습니다.  
...
태스크 6에서 EC2 Security Group을 변경한 경우, 기존 Amazon EC2를 계속 사용하려면 원복합니다.

20. EC2에 연결된 Security Group을 선택합니다 (예: `alb-lab-ec2-sg`).
21. **Inbound rules** 탭 → [[Edit inbound rules]]를 클릭합니다.
22. Source가 `alb-sg`(sg-xxx)로 설정된 규칙을 [[Delete]] 버튼으로 삭제합니다.
23. [[Add rule]]을 클릭하고 새 규칙을 추가합니다:
    - **Type**: `Custom TCP`
    - **Port range**: `8080` (또는 `80`)
    - **Source**: `0.0.0.0/0` (또는 본인 IP)
24. [[Save rules]]를 클릭합니다.

> [!NOTE]
> Amazon EC2를 더 이상 사용하지 않거나, 다음 단계에서 AWS CloudFormation 스택을 삭제할 경우  
> 이 단계는 건너뛰어도 됩니다.
...
25. 상단 검색창에 `CloudFormation`을 입력하고 선택합니다.
26. **Stacks** 목록에서 `step7-1-alb-prereq` 스택을 선택합니다.
27. [[Delete]]를 클릭합니다.
28. 확인 팝업에서 [[Delete stack]]을 클릭합니다.
29. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 2~3분).
...
30. EC2 콘솔 → **Load Balancers**에서 `starter-alb`가 삭제되었는지 확인합니다.
31. EC2 콘솔 → **Target Groups**에서 `starter-app-tg`가 삭제되었는지 확인합니다.
32. EC2 콘솔 → **Security Groups**에서 `alb-sg`가 삭제되었는지 확인합니다.
33. Tag Editor에서 최종 확인합니다:
