...
AWS는 사용한 만큼 과금되는 종량제 모델입니다.  
2025년 7월 15일 이후 신규 가입자는 새로운 크레딧 기반 프리티어가 적용되며, 크레딧이 소진되면 과금이 시작됩니다.  
따라서 예산 알림을 먼저 설정하고, 프리티어의 정확한 범위를 이해하는 것이 중요합니다.

> [!WARNING]
> AWS 계정을 생성하면 프리티어가 자동 적용되지만, 크레딧이 소진되거나 유효 기간이 지나면 **즉시 과금**됩니다.  
> 이 실습을 가장 먼저 진행하여 비용 안전장치를 설정하세요.

> [!TIP]
> **유료 플랜으로 전환하기**: Step 0-1에서 무료 플랜으로 가입했다면,  
> AWS Management Console → **Account** → **Account plan** 에서 유료 플랜으로 전환할 수 있습니다.  
> 유료 플랜으로 전환하면 모든 AWS 서비스에 접근할 수 있고, 크레딧 소진 후에도 계정이 유지됩니다 (종량제 과금).  
> 이 가이드의 실습은 무료 플랜으로도 충분하지만, 더 다양한 서비스를 탐색하고 싶다면 전환을 고려하세요.
>
> ⚠️ 다만, 유료 플랜에서는 크레딧이 소진되면 사용한 만큼 비용이 청구됩니다. 반드시 이 실습에서 설정하는 Budget 알림을 먼저 구성한 뒤 전환하세요.

## 태스크 1: 새 AWS 프리티어 체계 이해 (2025.07.15 이후 신규 가입자)

2025년 7월 15일부터 AWS 프리티어가 크레딧 기반으로 변경되었습니다.  
가입 시 **무료 플랜(Free Plan)** 또는 **유료 플랜(Paid Plan)**을 선택합니다.
...
| **유효 기간**      | 6개월 또는 12개월                     | 6개월 또는 12개월     |
| **서비스 접근**    | 제한된 서비스                         | 150여 개 전체 서비스  |
| **크레딧 소진 후** | 요금 없음 (유료 전환 전까지)          | 사용량에 따라 과금    |
| **적합한 대상**    | 학습, 실험 목적                       | 실제 프로젝트 구축    |

> [!CONCEPT] 크레딧 기반 프리티어
> 기존에는 서비스별로 개별 한도가 있었지만, 새 체계에서는 가입 시 **$100 크레딧**이 기본 제공되고, AWS 서비스를 탐색하며 활동하면 추가로 최대 $100을 더 받을 수 있습니다.  
> 크레딧은 가입일로부터 **6개월 또는 12개월간** 유효합니다.
>
...
| 서비스                 | 비용 수준 | 주의 사항                                                     |
| ---------------------- | --------- | ------------------------------------------------------------- |
| Amazon EC2 t2.micro    | 저렴      | 사용하지 않을 때 Stop하면 EC2 비용 없음                       |
| Amazon RDS db.t3.micro | 보통      | 24시간 가동 시 EC2보다 비용 높음, 사용 후 Stop 또는 삭제 권장 |
| Amazon S3              | 매우 저렴 | 학습 수준에서는 거의 무시 가능                                |
| NAT Gateway            | 높음      | 시간당 과금으로 크레딧 소진이 빠름, 실습 후 즉시 삭제 필수    |
| Amazon EBS 볼륨        | 저렴      | EC2 Stop해도 계속 과금, 하지만 금액은 매우 적음               |

> [!TIP]
> 학습 목적이라면 EC2 + RDS를 동시에 사용해도 크레딧으로 충분히 커버됩니다.  
> 다만 **NAT Gateway**는 비용이 높으므로 실습 후 반드시 삭제하세요.  
> 사용하지 않는 리소스를 주기적으로 정리하는 습관이 중요합니다.
...
- **Parameter Group** (Amazon RDS 파라미터 그룹)
...
이 실습을 시작하기 전에 IAM 사용자(`admin-user`)로 AWS Management Console에 로그인되어 있는지 확인합니다.

> [!NOTE]
> Billing and Cost Management는 글로벌 서비스이므로 어떤 리전에서든 접근할 수 있습니다.
...
>
> - **Scope options**: `All AWS services (Recommended)` 선택 (모든 서비스 비용 추적)
> - **Advanced options / Aggregate costs by**: `Unblended costs` (기본값)
> - **Tags**: 설정하지 않음
>
> 학습 목적이라면 $10 정도가 적당합니다.  
> 무료 플랜 크레딧 내에서 사용하면 실제 비용은 $0에 가깝지만, 크레딧 소진이나 실수로 비용이 발생하는 경우를 대비합니다.
...
## 태스크 2.5: Zero Spend Budget 설정 (프리티어 이탈 감지)

이 태스크에서는 **Zero spend budget** 템플릿을 사용하여, 크레딧/프리티어 범위를 벗어나 실제 비용이 $0.01이라도 발생하면 즉시 알림을 받도록 설정합니다.

> [!CONCEPT] Zero Spend Budget
> Zero spend budget은 AWS가 제공하는 Budget 템플릿으로, 예산을 $0으로 설정합니다.  
> 크레딧이나 프리티어로 커버되지 않는 실제 과금이 발생하는 순간 즉시 알림을 보내므로, 의도치 않은 비용 발생을 가장 빠르게 감지할 수 있습니다.
...
> 태스크 2에서 만든 $10 예산과 함께 사용하면 이중 안전장치가 됩니다.

> [!TIP]
> AWS Budgets는 월 2개까지 무료입니다.  
> 태스크 2의 `Monthly-Cost-Budget`과 이 `Zero-Spend Budget`으로 정확히 2개이므로 추가 비용이 발생하지 않습니다.

✅ **태스크 완료**: Zero spend budget이 설정되었습니다. 프리티어/크레딧 범위를 벗어나는 즉시 알림을 받게 됩니다.

## 태스크 3: Cost Explorer 확인

이 태스크에서는 Cost Explorer에서 서비스별, 일별 비용을 시각적으로 확인하는 방법을 학습합니다.

> [!NOTE]
> Cost Explorer는 Billing and Cost Management 콘솔을 처음 방문할 때 **자동으로 활성화**됩니다.  
> 별도 활성화 작업이 필요 없으며, 활성화 후 데이터가 표시되기까지 최대 24시간이 소요될 수 있습니다.  
> Cost Anomaly Detection도 함께 자동 설정됩니다.

26. Billing and Cost Management 페이지에서 왼쪽 메뉴의 **Cost Explorer**를 선택합니다.

27. Cost Explorer 대시보드가 표시되면 다음 항목을 확인합니다:
...
> Budget 알림과 별개로 동작하며, 크레딧 잔액과 계정 플랜 만료 시기에 대한 알림도 주기적으로 받을 수 있습니다.
...
**사전 설정: CloudWatch Billing 메트릭 활성화**
...
> 활성화 후 약 15분이 지나야 Billing 메트릭 데이터가 나타납니다.

**CloudWatch 알람 생성**

41. 상단 검색창에 `CloudWatch`를 입력하고 선택합니다.
42. 리전을 **US East (N. Virginia) us-east-1**로 변경합니다.
...
> 이메일의 **Confirm subscription** 링크를 반드시 클릭해야 알림이 정상 동작합니다.
...
> [!TIP]
> 이후 실습을 진행하면서 **주 1회** 이상 Cost Explorer와 Free Tier 페이지를 확인하는 습관을 들이세요.  
> 크레딧 소진 속도를 파악하고, 불필요한 리소스를 빠르게 정리할 수 있습니다.
...
> [!NOTE]
> 이 설정들은 크레딧 소진과 비용 폭탄을 방지하는 핵심 안전장치입니다.  
> 모든 실습이 끝날 때까지 유지하세요.
...
> [!TIP]
> 이 가이드의 실습을 순서대로 진행하면 5개 활동(EC2, RDS, Lambda, Bedrock, Budgets)을 모두 완료하게 됩니다.  
...
> [!NOTE]
> 활동 완료 후 크레딧이 적립되기까지 시간이 걸릴 수 있습니다.  
> 적립된 크레딧은 Billing → Credits 페이지에서도 확인할 수 있습니다.
