...
awsServices:
  - Amazon EC2
learningObjectives:
  - EC2에 Node.js 22 LTS를 설치할 수 있습니다.
  - Vue 3 프로젝트를 생성하고 프로덕션 빌드할 수 있습니다.
  - rsync, scp, git clone으로 프로젝트를 EC2에 전송할 수 있습니다.
...
이 실습에서는 EC2 인스턴스에 Node.js와 Nginx를 설치하고, Vue 3 프로젝트를 빌드하여 배포합니다.  
SPA(Single Page Application) 라우팅을 위한 Nginx 설정도 함께 구성합니다.

> [!NOTE]
> 이 실습은 EC2 인스턴스가 필요합니다.  
...
| 태스크 1   | EC2 내부        | Node.js 설치, Swap 추가                        |
| 태스크 2   | 로컬 또는 EC2   | 프로젝트 업로드(git clone/rsync/scp) 또는 생성 |
| 태스크 3   | EC2 내부        | `npm install` + `npm run build`                |
| 태스크 4~5 | EC2 내부        | Nginx 설치 + 빌드 파일 배포                    |
...
이미 Vue 3 프로젝트가 있다면 **방법 A**에서 선택하여 EC2에 올립니다.  
새로 시작한다면 **방법 B**에서 EC2에서 직접 프로젝트를 생성합니다.

> [!CONCEPT] 왜 소스를 EC2에 올리나요? 빌드한 결과만 올리면 안 되나요?
>
> **결론부터: 빌드 결과(`dist/`)만 올려도 됩니다.** 실제 프로덕션에서는 그게 더 일반적입니다.
>
> 하지만 이 실습에서 소스를 EC2에 올려서 빌드하는 이유:
>
> - EC2에서 빌드 → 배포의 **전체 흐름을 체험**하기 위해
> - 나중에 CI/CD(Step 8)에서 이 과정을 자동화할 때 동일한 흐름이 GitHub Actions에서 실행됨
> - EC2 환경에서 빌드가 정상 동작하는지 확인 (OS 차이로 실패할 수 있음)
>
> **실무에서의 배포 흐름 비교:**
>
> | 방식                            | 흐름                                      | 사용 상황                 |
> | ------------------------------- | ----------------------------------------- | ------------------------- |
> | EC2에서 빌드                    | 소스 업로드 → EC2에서 npm install + build | 학습, 소규모              |
...
프로젝트가 이미 GitHub에 push되어 있다면, EC2에서 직접 clone합니다.  
...
> | `<Public-IP>`                | EC2 인스턴스의 Public IP | `3.35.123.456`          |
>
...
Windows에서는 `rsync`를 사용하기 어렵습니다. 대신 **로컬에서 먼저 빌드**하고, 결과물만 EC2에 전송합니다.

> [!NOTE]
> 이 방법을 사용하면 EC2에 Node.js가 설치되어 있지 않아도 됩니다.  
...
> [!WARNING]
>
> - `<Public-IP>`를 EC2의 실제 Public IP로 변경하세요
...
Vue 3 프로젝트가 없는 경우, EC2에서 직접 생성합니다.
...
방법 A-①(Git clone) 또는 A-②(rsync)로 소스를 업로드한 경우, EC2에서 빌드합니다.
...
>   1. **Nginx 미실행**: EC2에서 `sudo systemctl status nginx` 확인 → `active (running)`이 아니면 `sudo systemctl start nginx`
>   2. **브라우저가 HTTPS로 강제 전환**: 브라우저가 `http://`를 `https://`로 자동 변경하는 경우. EC2에 HTTPS(443) 설정이 없으면 연결 거부됨.
...
>      - EC2 내부에서 `curl http://localhost`가 정상 응답하면 Nginx는 문제 없고 브라우저 문제입니다
...
>
> 지금은 같은 EC2에 프론트엔드와 백엔드를 모두 배포할 예정이므로, Step 2-3에서 Nginx에 `/api → localhost:8080` 프록시를 추가하면 해결됩니다.
...
29. 404 문제를 확인합니다. EC2에서 다음을 실행하세요:
