...
이 실습에서는 EC2 인스턴스를 생성하고, SSH로 접속하여 MySQL 8.4 LTS를 직접 설치합니다.  
...
| 단계       | 실행 위치   | 내용                                    |
| ---------- | ----------- | --------------------------------------- |
| 태스크 0~1 | AWS 콘솔    | 선행 리소스(VPC) + EC2 인스턴스 생성    |
| 태스크 2~3 | 로컬 터미널 | SSH/SSM으로 EC2 접속                    |
| 태스크 4~6 | EC2 내부    | MySQL 설치, 보안 초기화, 사용자/DB 생성 |
...
EC2 생성 시 연결할 IAM Role을 먼저 만듭니다.  
이 Role은 태스크 3에서 SSM Session Manager로 접속할 때 사용됩니다.

> [!CONCEPT] IAM Role (인스턴스 프로필)
>
> IAM Role은 EC2 인스턴스에 AWS 서비스 사용 권한을 부여하는 방식입니다.  
> 키 페어가 "SSH 접속 열쇠"라면, IAM Role은 "AWS 서비스 이용 권한증"입니다.
>
> EC2에 Role을 연결하면 인스턴스 내부에서 별도 자격 증명(Access Key) 없이 AWS API를 호출할 수 있습니다.  
...
>
> **File systems** 항목은 EC2에 추가 파일 시스템을 마운트하는 옵션입니다:
>
> | 옵션             | 설명                                           | 사용 사례                          |
> | ---------------- | ---------------------------------------------- | ---------------------------------- |
> | `S3 Files - new` | Amazon S3를 파일 시스템처럼 마운트             | 대용량 데이터 공유, 정적 파일 접근 |
> | `EFS`            | Elastic File System (공유 NFS 스토리지)        | 여러 EC2에서 동시에 파일 공유      |
...
>
> SSH 접속에 사용한 키 페어(`.pem` 파일)는 EC2의 "물리적 열쇠"와 같습니다. 편리하지만 실무에서는 여러 위험이 있습니다.
>
> **키 페어의 장점:**
>
...
> | 유출 시 즉시 침해 가능 | 키 파일이 유출되면 누구나 EC2에 접속 가능                |
...
>
> EC2에 접속하는 대표적인 두 가지 방식입니다. 둘 다 알아두면 상황에 따라 적절한 방식을 선택할 수 있습니다.
>
...
> | ② SSM Agent 실행 중 | EC2 내부에서 SSM Agent가 실행 중이어야 함                                    | ✅ Amazon Linux 2023은 기본 설치·자동 실행         |
> | ③ 네트워크 접근     | EC2가 SSM 서비스 엔드포인트에 접근 가능해야 함                               | ✅ Public Subnet + IGW이므로 인터넷 통해 접근 가능 |
>
...
> - **Step 9 (3-Tier)**: Private Subnet의 EC2에 SSM Session Manager로 접속합니다.
>
...
> - **DBA/관리 계정** (`admin`): `ALL PRIVILEGES` (접속 IP를 VPC 내부로 제한)
>
...
> | **다른 EC2에서 접속**                 | 앱 서버(Web/WAS)와 DB 서버를 분리하는 구성 (Step 9에서 실습)    |
> | **CI/CD 파이프라인에서 마이그레이션** | 배포 시 자동으로 스키마 변경 실행                               |
> | **모니터링 도구 연동**                | Grafana, Datadog 등이 DB 메트릭을 수집                          |
>
> 이 실습에서는 로컬 PC의 MySQL Client(또는 GUI 도구)에서 EC2의 MySQL에 직접 접속할 수 있도록 설정합니다.
>
...
72. EC2 콘솔에서 `my-ec2-mysql` 인스턴스를 선택합니다.
...
>
> `<EC2-Public-IP>` 부분을 EC2 인스턴스의 실제 Public IPv4 address로 교체합니다.  
...
>
> ```bash
> # 1. EC2 내부에서 MySQL 접속 확인 (SSH 접속 상태에서)
...
>
> Amazon Linux 2023은 OS 레벨 방화벽(`iptables`, `firewalld`)이 기본 설치되어 있지 않으므로, EC2의 네트워크 접근 제어는 **Security Group**이 전부입니다.
...
- 위 실습에서 생성한 EC2 인스턴스가 `running` 상태이고, SSH로 접속 가능한 상태.
...
>
> SSH 외에 이 EC2에서 열려있는 MySQL(3306) 포트로도 동일한 테스트가 가능합니다.  
...
6. VPC 콘솔에서 다시 **Inbound rules** 탭 → [[Edit inbound rules]]를 클릭합니다.
7. Rule 50을 삭제합니다 (**Remove** 버튼 클릭).
8. [[Save changes]]를 클릭합니다.
9. 로컬 터미널에서 다시 SSH 접속을 시도합니다.
...
1. 상단 검색창에 `VPC`를 입력하고 VPC 서비스를 선택합니다.
2. 왼쪽 메뉴에서 **Network ACLs**를 선택합니다.
3. [[Create network ACL]] 버튼을 클릭합니다.
4. 다음과 같이 설정합니다:
   - **Name**: `test-nacl`
   - **VPC**: `my-vpc` 선택
   - **Tags**:
...
6. 생성된 `test-nacl`을 선택합니다.
7. **Subnet associations** 탭을 선택합니다.
8. [[Edit subnet associations]] 버튼을 클릭합니다.
9. EC2가 있는 `my-public-subnet-a`를 체크합니다.
10. [[Save changes]] 버튼을 클릭합니다.
11. 로컬 터미널에서 SSH 접속을 시도합니다:
...
12. `test-nacl`을 선택한 상태에서 **Inbound rules** 탭을 선택합니다.
13. [[Edit inbound rules]] 버튼을 클릭합니다.
14. [[Add new rule]] 버튼을 클릭하고 다음과 같이 설정합니다:
    - **Rule number**: `100`
    - **Type**: `SSH (22)`
    - **Source**: `0.0.0.0/0`
    - **Allow/Deny**: `Allow`
15. [[Save changes]] 버튼을 클릭합니다.
16. **Outbound rules** 탭을 선택합니다.
17. [[Edit outbound rules]] 버튼을 클릭합니다.
18. [[Add new rule]] 버튼을 클릭하고 다음과 같이 설정합니다:
    - **Rule number**: `100`
    - **Type**: `Custom TCP`
...
1. VPC 콘솔 → **Network ACLs**에서 `my-public-nacl`을 선택합니다.
2. **Subnet associations** 탭을 선택합니다.
...
1. VPC 콘솔 → **Network ACLs**에서 `test-nacl`을 선택합니다.
2. **Subnet associations** 탭을 선택합니다.
3. [[Edit subnet associations]] 버튼을 클릭합니다.
4. 모든 서브넷의 체크를 해제합니다.
5. [[Save changes]] 버튼을 클릭합니다.
6. 다시 `test-nacl`을 선택한 상태에서 **Actions** → [[Delete network ACL]]을 선택합니다.
7. 확인 팝업에서 `delete`를 입력하고 [[Delete]] 버튼을 클릭합니다.
...
- EC2 인스턴스(Amazon Linux 2023, t3.micro)를 생성하고 SSH와 SSM Session Manager 두 가지 방식으로 접속했습니다.
...
5. 이 실습에서 생성한 리소스(`my-ec2-mysql` 등)가 표시되는지 확인합니다.
...
11. 상단 검색창에 `CloudFormation`을 입력하고 선택합니다.
12. **Stacks** 목록에서 `ec2-lab-prereq` 스택을 선택합니다.
13. [[Delete]] 버튼을 클릭합니다.
14. 확인 팝업에서 [[Delete stack]]을 클릭합니다.
15. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 2-3분).
...
Step 1에서 수동으로 VPC를 생성하고 그것을 이 실습에서 사용한 경우, 아래 방법으로 삭제합니다.

> [!TIP]
> **가장 간단한 방법: VPC 삭제 (연결 리소스 자동 삭제)**
>
> EC2 인스턴스가 완전히 Terminated된 상태라면, VPC를 삭제하면 연결된 리소스(서브넷, Route Table, IGW, Security Group)가 함께 삭제됩니다.
>
> 1. VPC 콘솔 → **Your VPCs**에서 `my-vpc`를 선택합니다.
> 2. **Actions** → [[Delete VPC]]를 선택합니다.
> 3. 확인 팝업에서 `delete`를 입력하고 [[Delete]] 버튼을 클릭합니다.
>
> 만약 VPC 삭제가 실패한다면 ("has dependencies" 에러), EC2가 아직 Terminated되지 않았거나 다른 리소스가 남아있는 것입니다.  
...
16. **EC2 콘솔**: `my-ec2-mysql` 인스턴스가 `Terminated` 상태인지 확인합니다.
17. **CloudFormation 콘솔**: `ec2-lab-prereq` 스택이 목록에서 사라졌는지 확인합니다.
18. **VPC 콘솔**: `my-vpc`가 삭제되었는지 확인합니다.
19. **Tag Editor**: 다시 검색하여 관련 리소스가 남아있지 않은지 확인합니다.
