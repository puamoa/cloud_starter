...
awsServices:
  - Amazon EC2
learningObjectives:
  - Amazon EC2에 Amazon Corretto 17(Java)을 설치할 수 있습니다.
  - Spring Boot JAR 또는 Spring MVC WAR를 Amazon EC2에 배포할 수 있습니다.
...
이 실습에서는 Amazon EC2 인스턴스에 Java 17을 설치하고, Spring 애플리케이션을 배포합니다.  
**Spring Boot(JAR)** 또는 **Spring MVC(WAR + Tomcat)** 두 가지 방식을 모두 안내합니다.

> [!NOTE]
> 이 실습은 Amazon EC2 인스턴스가 필요합니다. Step 2-1에서 생성한 Amazon EC2를 사용하거나, 새로운 Amazon EC2 인스턴스를 생성합니다.  
...
| 태스크 1   | Amazon EC2 내부        | Java 17 설치, Swap 추가                    |
| 태스크 2   | **로컬 PC**     | Spring 프로젝트 빌드 (JAR 또는 WAR)        |
| 태스크 3   | **로컬 PC**     | SCP로 빌드 결과물을 Amazon EC2에 전송             |
| 태스크 4   | Amazon EC2 내부        | 애플리케이션 실행 + systemd 서비스 등록    |
| 태스크 5~6 | Amazon EC2 내부        | 로그 확인 + 자동 재시작 테스트             |
| 태스크 7   | Amazon EC2 내부        | Nginx 리버스 프록시 설정 (프론트엔드 연동) |
...
> **Spring Boot와의 핵심 차이:**
>
> | 항목 | Spring Boot (방법 A) | Spring MVC (방법 B) |
> | --- | --- | --- |
> | 서버 | 내장 Tomcat (별도 설치 불필요) | **외부 Tomcat 별도 설치** 필요 |
> | 설정 방식 | 자동 설정 (@SpringBootApplication) | 직접 설정 (Java Config 또는 XML) |
> | 빌드 결과물 | .jar (서버 포함) | .war (서버에 배포) |
> | 실행 | java -jar app.jar | Tomcat의 webapps/에 WAR 배치 |
>
> 두 방식 모두 application.properties로 DB 접속 정보, 환경별 설정 등을 관리할 수 있습니다.  
> build.gradle에 **id 'war'** 플러그인이 있고, WebApplicationInitializer나 web.xml로 서블릿을 등록하는 프로젝트라면 이 방법을 사용합니다.
...
16. **`gradlew` 파일 실행 권한 확인** (Mac/Linux):

```bash
chmod +x gradlew
```

<img src="/images/step2/2-3-step16-scp.png" alt="gradlew 권한 확인" class="guide-img-sm" />
...
17. **업로드 경로 수정** (Windows 경로 → Linux 경로):  
...
<img src="/images/step2/2-3-step17-scp-verify.png" alt="업로드 경로 수정" class="guide-img-sm" />

18. **DB 접속 정보 확인** (`application.properties`):
...
<img src="/images/step2/2-3-step18-db-config.png" alt="DB 접속 정보 확인" class="guide-img-sm" />

> [!NOTE]
> **`localhost:3306`이 맞는 이유:**  
> MySQL을 같은 Amazon EC2 인스턴스에 설치했으므로, 애플리케이션과 DB가 같은 서버에 있습니다.  
...
> [!WARNING]
> `application.properties`의 DB 접속 정보(DB명, 사용자명, 비밀번호)에 해당하는 **MySQL 사용자와 데이터베이스가 Amazon EC2에 존재해야** 합니다.  
> 아직 생성하지 않았다면 Amazon EC2에 SSH 접속 후 다음을 실행하세요:
>
> 📍 **실행 위치: EC2 (SSH 접속한 상태)**
>
> ```bash
...
>
> <img src="/images/step2/2-3-step18-db-create.png" alt="MySQL 사용자/DB 생성" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step19-gradle-build-war.png" alt="WAR 빌드" class="guide-img-sm" />
...
**IntelliJ IDEA에서 빌드하는 경우:**

> [!TIP]
> 터미널 명령이 익숙하지 않다면 IDE에서 직접 빌드할 수 있습니다.
>
> **방법 1: `war` 태스크 직접 사용 (권장)**
>
> 1. 오른쪽 **Gradle** 패널 열기 (View → Tool Windows → Gradle)
> 2. **backend** → **Tasks** → **war** 더블클릭
> 3. 빌드 시작 (테스트를 건너뛰고 WAR만 생성)
>
>       <img src="/images/step2/2-3-step19-intellij-war.png" alt="IntelliJ Gradle war 태스크 빌드" class="guide-img-sm" />
>
> **방법 2: `build` 태스크 + `-x test` 옵션**
>
> 1. 오른쪽 **Gradle** 패널 열기 (View → Tool Windows → Gradle)
> 2. **backend** → **Tasks** → **build** → **build** 우클릭 (더블클릭 아님)
> 3. **Modify Run Configuration...** 선택
>
>       <img src="/images/step2/2-3-step19-modify-run-config.png" alt="IntelliJ Modify Run Configuration 메뉴" class="guide-img-sm" />
>
> 4. **Run Configuration 설정 창이 열립니다:**
>    - **Run** 필드에 `build -x test` 입력 (기존 `build`를 지우고 입력)
>    - [[OK]] 클릭
>
>       <img src="/images/step2/2-3-step19-run-config-detail.png" alt="IntelliJ Run Configuration 설정 상세" class="guide-img-sm" />
>
> 5. 더블 클릭하여 빌드 시작
>
>       <img src="/images/step2/2-3-step19-build-execute.png" alt="IntelliJ 빌드 실행" class="guide-img-sm" />
>
> 빌드 완료 후 `build/libs/` 폴더에 WAR 파일이 생성됩니다.
>
> 
...
>
> - `build`: Gradle의 표준 빌드 태스크 (컴파일 → 테스트 실행 → JAR/WAR 생성)
> - `-x test`: `test` 태스크를 **제외(exclude)** 하라는 옵션
>
> **왜 `-x test`를 사용하는가?**
>
> 프로젝트에 테스트 코드가 있는데, 로컬 PC에서 다음과 같은 문제가 발생할 수 있습니다:
>
> | 상황 | 문제 |
> | --- | --- |
> | **MySQL이 로컬에 없음** | 테스트가 데이터베이스에 접속하려다 실패 |
> | **Spring 설정 누락** | 프로파일(dev/prod)이 없어서 빈 생성 실패 |
> | **외부 API 호출** | 테스트가 실제 API를 호출해서 느림 |
> | **테스트 환경 미설정** | `application-test.properties` 부재 |
>
> 이런 경우 빌드가 실패하거나 매우 오래 걸립니다.  
> `-x test`를 사용하면 테스트를 건너뛰고 **컴파일 + JAR/WAR 생성만** 수행하므로 빠르고 안정적입니다.
>
> **로컬에서 테스트 실행 시:**
>
> 테스트 환경을 제대로 설정한 경우, 옵션 없이 실행하면 테스트도 함께 실행됩니다:
>
> ```gradle
> ./gradlew build
> ```
...
20. 빌드된 파일을 SCP로 Amazon EC2에 전송합니다. 본인의 방법에 맞는 명령어를 실행하세요:
...
> | `<Public-IP>`                        | Amazon EC2의 Public IP            | `3.35.123.456`               |
>
...
> [!WARNING]
> 아래 명령어의 파일 경로와 파일명이 당신의 프로젝트와 일치하는지 확인하세요.
> - `build/libs/demo-0.0.1-SNAPSHOT.jar`: 실제 빌드된 JAR 파일명이 다를 수 있습니다
> - 확인 방법: `ls build/libs/` 명령으로 정확한 파일명을 확인한 후, 아래 명령어에서 `demo-0.0.1-SNAPSHOT.jar`을 실제 파일명으로 변경하세요
...
<img src="/images/step2/2-3-step20-scp-file-path.png" alt="JAR 파일명 확인" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step20-scp-war.png" alt="WAR 파일 SCP 전송" class="guide-img-sm" />
...
> - **Windows MobaXterm 사용자**: 왼쪽 파일 브라우저에서 Amazon EC2의 홈 디렉토리(`/home/ec2-user`)가 보입니다. 로컬의 JAR/WAR 파일을 드래그 앤 드롭으로 업로드할 수 있습니다.  
>   업로드 후 EC2 터미널에서 `mv <파일명> ~/app.jar`로 이름을 변경하세요.

21. Amazon EC2에 접속하여 파일이 전송되었는지 확인합니다:
...
<img src="/images/step2/2-3-step21-verify-scp.png" alt="파일 전송 확인" class="guide-img-sm" />
...
✅ **태스크 완료**: 파일이 Amazon EC2에 전송되었습니다.
...
22. 애플리케이션 디렉토리를 생성하고 JAR 파일을 이동합니다:
...
<img src="/images/step2/2-3-step22-setup-app-dir.png" alt="애플리케이션 디렉토리 설정" class="guide-img-sm" />

23. 직접 실행하여 정상 동작을 확인합니다:

```bash
java -jar /opt/app/app.jar
```

<img src="/images/step2/2-3-step23-run-jar.png" alt="Spring Boot JAR 실행" class="guide-img-sm" />
...
24. **새 터미널을 열어** Amazon EC2에 다시 접속하고 테스트합니다:

> [!TIP]
> `java -jar`를 실행한 터미널은 Spring이 점유하고 있어 다른 명령을 입력할 수 없습니다.  
> **새 터미널 창/탭을 열어** 같은 Amazon EC2에 SSH로 다시 접속하세요:
>
> ```bash
> ssh -i ~/Downloads/my-keypair.pem ec2-user@<Public-IP>
> ```
...
<img src="/images/step2/2-3-step24-curl-test.png" alt="Spring Boot API 테스트" class="guide-img-sm" />

25. 확인 후 `java -jar`를 실행한 터미널에서 **Ctrl+C**를 눌러 Spring 애플리케이션을 종료합니다.

<img src="/images/step2/2-3-step25-terminate-jar.png" alt="Spring Boot 애플리케이션 종료" class="guide-img-sm" />

26. **systemd 서비스로 등록합니다:**
...
<img src="/images/step2/2-3-step26-systemd-service.png" alt="systemd 서비스 등록" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step27-start-service.png" alt="systemd 서비스 시작" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step28-install-tomcat.png" alt="Tomcat 9 설치" class="guide-img-sm" />
...
29. Tomcat 실행 권한을 설정합니다:

```bash
chmod +x /opt/tomcat/bin/*.sh
```

<img src="/images/step2/2-3-step29-chmod-tomcat.png" alt="Tomcat 실행 권한 설정" class="guide-img-sm" />

30. Tomcat을 시작하여 정상 동작을 확인합니다:

```bash
/opt/tomcat/bin/startup.sh
```

<img src="/images/step2/2-3-step30-start-tomcat.png" alt="Tomcat 시작" class="guide-img-sm" />

31. 브라우저에서 `http://<Public-IP>:8080`으로 접속하여 Tomcat 기본 페이지가 표시되는지 확인합니다.

<img src="/images/step2/2-3-step31-tomcat-browser.png" alt="Tomcat 브라우저 접속 확인" class="guide-img-sm" />

> [!TIP]
> **페이지가 안 뜨는 경우 확인 순서:**
>
> 1. Amazon EC2 내부에서 먼저 확인: `curl http://localhost:8080` (정상이면 Tomcat은 OK)
> 2. Security Group에 8080 포트 열려 있는지 확인 (태스크 0 참조)
> 3. 브라우저가 `https://`로 자동 전환하는지 확인 → 시크릿 모드로 재시도

32. 확인 후 Tomcat을 중지합니다:

```bash
/opt/tomcat/bin/shutdown.sh
```

<img src="/images/step2/2-3-step32-shutdown-tomcat.png" alt="Tomcat 중지" class="guide-img-sm" />
...
> [!NOTE]
> **사전 확인**: Step 2-1에서 MySQL을 같은 Amazon EC2에 설치했다면, `localhost:3306`으로 접속됩니다.  
...
<img src="/images/step2/2-3-step33-scp-sql.png" alt="SQL 파일 SCP 전송" class="guide-img-sm" />
...
34. Amazon EC2에서 MySQL에 접속하여 SQL을 실행합니다:
...
<img src="/images/step2/2-3-step34-execute-sql.png" alt="SQL 실행" class="guide-img-sm" />
...
> ```
>
> <img src="/images/step2/2-3-step34-csv-import1.png" alt="CSV 데이터 import 1" class="guide-img-sm" />
> <img src="/images/step2/2-3-step34-csv-import2.png" alt="CSV 데이터 import 2" class="guide-img-sm" />

#### B-3. WAR 배포

35. 기존 기본 앱을 정리하고 WAR 파일을 배포합니다:
...
<img src="/images/step2/2-3-step35-deploy-war.png" alt="WAR 파일 배포" class="guide-img-sm" />
...
36. (옵션) 프로젝트에 파일 업로드 기능이 있는 경우, 업로드 디렉토리를 생성합니다:
...
<img src="/images/step2/2-3-step36-create-upload-dir.png" alt="업로드 디렉토리 생성" class="guide-img-sm" />

#### B-4. Tomcat systemd 서비스 등록

37. systemd 서비스 파일을 생성합니다:
...
<img src="/images/step2/2-3-step37-tomcat-service.png" alt="Tomcat systemd 서비스 파일 생성" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step38-start-service.png" alt="Tomcat systemd 서비스 시작" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step39-verify-log.png" alt="Tomcat 로그 확인" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step39-api-test.png" alt="API 테스트 결과" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step40-realtime-log.png" alt="Spring Boot 실시간 로그 확인" class="guide-img-sm" />

> [!TIP]
> `-f`는 실시간으로 새 로그가 추가될 때마다 출력합니다.  
> 이 상태에서 **다른 터미널 탭을 열거나 브라우저에서 `http://<Public-IP>:8080`에 접속**해 보세요.
>
> <img src="/images/step2/2-3-step42-tail-tip.png" alt="Tomcat 실시간 로그 감시 팁" class="guide-img-sm" />
>
> **요청 로그가 안 보이는 경우:**  
...
> ```
>
> <img src="/images/step2/2-3-step40-log-tip.png" alt="로그 레벨 설정" class="guide-img-sm" />
>
> 또는 존재하지 않는 경로(예: `http://<Public-IP>:8080/asdf`)에 접속하면 에러 로그가 출력됩니다.
>
> 확인이 끝나면 **Ctrl+C**로 빠져나옵니다.

41. 최근 로그를 확인합니다 (필요 시):
...
<img src="/images/step2/2-3-step44-kill-process-boot.png" alt="Spring Boot 프로세스 강제 종료" class="guide-img-sm" />
<img src="/images/step2/2-3-step44-kill-process-tomcat.png" alt="Tomcat 프로세스 강제 종료" class="guide-img-sm" />

45. 12초 후 자동 재시작되었는지 확인합니다:
...
<img src="/images/step2/2-3-step45-verify-restart-boot.png" alt="Spring Boot 자동 재시작 확인" class="guide-img-sm" />
...
<img src="/images/step2/2-3-step47-auto-restart.png" alt="Tomcat 자동 재시작 확인" class="guide-img-sm" />
...
49. 아래 내용을 **전체 교체**합니다 (기존 내용 삭제 후 붙여넣기):
...
<img src="/images/step2/2-3-step49-nginx-config.png" alt="Nginx 리버스 프록시 설정" class="guide-img-sm" />

50. 설정을 검증하고 Nginx를 재시작합니다:
...
<img src="/images/step2/2-3-step51-browser-test.png" alt="Nginx 리버스 프록시 브라우저 테스트" class="guide-img-sm" />

<img src="/images/step2/2-3-step51-api-response.png" alt="API 응답 확인" class="guide-img-sm" />
...
> **브라우저에서 접속이 안 되는 경우:**
>
> Amazon EC2 내부에서 Nginx 프록시 경유로 정상 동작하는지 확인하세요:
>
> ```bash
...
>
> Amazon EC2에 HTTPS 설정이 없으면 `ERR_CONNECTION_REFUSED`가 발생합니다.
>
...
> | **동작 위치** | 로컬 PC (Vite 개발 서버)           | Amazon EC2 서버 (Nginx)         |
...
- Amazon EC2에 Amazon Corretto 17(Java 17)을 설치했습니다.
- Spring Boot JAR 또는 Spring MVC WAR를 빌드하고 Amazon EC2에 전송했습니다.
...
Step 2-1 ~ 2-3을 통해 **하나의 Amazon EC2 인스턴스에 3개의 서버**를 동작시키는 구조를 완성했습니다.  
...
> 이후 Step에서는 새로운 인프라를 구성하므로, 여기서 만든 Amazon EC2를 유지할 필요가 없습니다.
...
4. [[Search resources]] 버튼을 클릭합니다.
    ...
    <img src="/images/step2/2-3-cleanup-step4-search.png" alt="Tag Editor 검색 결과" class="guide-img-sm" />
...
7. 상단 **Instance state** → **Terminate(delete) instance**를 클릭합니다.
    ...
    <img src="/images/step2/2-3-cleanup-step7-terminate.png" alt="EC2 인스턴스 Terminate 클릭" class="guide-img-sm" />
8. 확인 팝업이 표시됩니다:
   - **Termination protection**: `Disabled` (정상)
   - **Skip OS shutdown**: 체크하지 않음 (기본값 유지)

    <img src="/images/step2/2-3-cleanup-step8-confirm.png" alt="EC2 Terminate 확인 팝업" class="guide-img-sm" />
9. [[Terminate (delete)]] 버튼을 클릭합니다.
10. Instance state가 `Shutting down` → `Terminated`로 변경되는 것을 확인합니다.

    <img src="/images/step2/2-3-cleanup-step10-terminated.png" alt="EC2 인스턴스 Terminated 상태 확인" class="guide-img-sm" />
...
13. 검색창에 `my-ec2-ssm-role`을 입력하여 찾습니다.
    ...
    <img src="/images/step2/2-3-cleanup-step13-search-role.png" alt="IAM Role 검색" class="guide-img-sm" />
14. 해당 Role을 선택하고 [[Delete]] 버튼을 클릭합니다.
15. 확인을 위해 Role 이름(`my-ec2-ssm-role`)을 입력하고 [[Delete]] 클릭합니다.

    <img src="/images/step2/2-3-cleanup-step15-delete-role.png" alt="IAM Role 삭제 확인" class="guide-img-sm" />

#### 단계 4: CloudFormation 스택 삭제

Step 2-1 태스크 0에서 CloudFormation으로 Amazon VPC를 생성한 경우 스택을 삭제합니다.

> [!NOTE]
> Step 1에서 수동으로 Amazon VPC를 생성하고 그것을 이 실습에서 사용한 경우, CloudFormation 스택이 없으므로 이 단계를 건너뛰고 단계 5로 이동합니다.  
> 수동 생성한 Amazon VPC를 삭제하려면 [Step 1-1의 리소스 정리 섹션](/week/1/session/1)을 참고하세요.
...
19. 확인 팝업에서 [[Delete stack]]을 클릭합니다.
    ...
    <img src="/images/step2/2-3-cleanup-step19-delete-stack.png" alt="CloudFormation 스택 삭제 확인" class="guide-img-sm" />
20. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 2-3분).

    <img src="/images/step2/2-3-cleanup-step20-wait-complete.png" alt="CloudFormation 스택 삭제 완료" class="guide-img-sm" />
...
> | 원인                                      | 해결 방법                                                 |
> | ----------------------------------------- | --------------------------------------------------------- |
> | Amazon EC2가 아직 Terminated되지 않음            | Amazon EC2가 완전히 Terminated된 후(1-2분 대기) 스택 삭제 재시도 |
...
23. **Tag Editor**: Step 2 태그로 다시 검색하여 관련 리소스가 남아있지 않은지 확인합니다.
    <img src="/images/step2/2-3-cleanup-step23-tag-editor-check.png" alt="Tag Editor 검색 결과 확인" class="guide-img-sm" />
