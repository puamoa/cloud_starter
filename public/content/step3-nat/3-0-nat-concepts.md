...
> Amazon EC2에 연결된 Public IP, Elastic IP 모두 해당됩니다. 월 기준 약 $3.6/IP입니다.
...
| **예시**       | Amazon EC2가 패키지 다운로드       | 사용자가 웹서버 접속      |
...
| **NAT Instance** | Amazon EC2에 NAT 기능을 설정한 인스턴스. 비용 저렴하나 직접 관리 필요  |
...
| **연결 대상** | Amazon EC2 인스턴스 또는 NAT Gateway에 연결 가능                                                      |
...
| `10.0.11.20` | `10.0.0.0/16 → local` | 같은 Amazon VPC 내부이므로 local   |

> [!NOTE]
> `0.0.0.0/0`은 "그 외 모든 목적지"를 의미하는 기본 경로(Default Route)입니다.  
> Amazon VPC 내부 트래픽(10.0.0.0/16)은 항상 `local`로 처리되고, 그 외 트래픽만 NAT/IGW로 전달됩니다.  
> 따라서 NAT를 설정해도 Amazon VPC 내부 통신에는 영향을 주지 않습니다.
...
> NAT Instance는 일반 Amazon EC2 인스턴스에 3가지 핵심 설정을 적용하여 NAT 기능을 수행하게 합니다.  
...
> **Private Amazon EC2에서 외부로 나가는 패킷(아웃바운드) 기준으로**, filter 테이블의 FORWARD 체인이 먼저 패킷 통과를 판단(ACCEPT)한 뒤, nat 테이블의 POSTROUTING 체인에서 주소 변환(MASQUERADE)이 적용됩니다.  
...
> [!CONCEPT] Amazon EC2에 접속하는 3가지 방법
> AWS에서 Amazon EC2 인스턴스에 접속하는 방법은 3가지가 있습니다.  
...
> Amazon VPC 내부에 EIC Endpoint를 미리 생성해 두면, Public IP가 없고 22번 포트를 외부에 열지 않은 Private EC2에도 콘솔 브라우저로 접속할 수 있습니다.  
...
| **SSH Agent Forwarding**                | 로컬 키를 Bastion에 노출하지 않고 Private Amazon EC2에 접속하는 방법 |
| **ProxyJump (-J 옵션)**                 | Bastion을 경유하여 한 번의 명령으로 Private Amazon EC2에 직접 접속   |
| **AWS Systems Manager Session Manager** | Bastion 없이 Private Amazon EC2에 접속하는 AWS 관리형 서비스         |
...
> **SSH 점프 후 가장 흔한 실수:**
>
> - Bastion에서 실행할 명령을 Private Amazon EC2에서 실행 (또는 그 반대)
> - Private Amazon EC2에서 `exit`을 한 번 더 쳐서 Bastion까지 종료
> - 로컬인 줄 알고 `rm -rf`를 서버에서 실행
> - 패키지 설치를 Bastion에 해버림 (Private Amazon EC2에 해야 하는데)
...
> - 이 실습에서 Bastion을 사용하는 이유 → NAT 없는 상태에서 Private Amazon EC2에 접속하여 "인터넷 불가"를 확인하기 위함

---

## 11. NAT와 SSM Session Manager의 관계

> [!CONCEPT] Private Amazon EC2에서 SSM이 동작하려면
> AWS Systems Manager Session Manager는 EC2 내부의 **SSM Agent**가 AWS API 엔드포인트(`ssm.region.amazonaws.com`)에 접근하여 등록해야 동작합니다.  
> Private Subnet의 Amazon EC2는 인터넷에 접근할 수 없으므로, **NAT 또는 VPC Endpoint가 있어야 SSM 접속이 가능**합니다.
...
> **VPN**(Virtual Private Network)은 인터넷을 통해 암호화된 터널로 AWS Amazon VPC에 연결하는 방식이고, **Direct Connect**는 전용 물리 회선으로 AWS에 직접 연결하는 방식입니다. 둘 다 On-Premise와 AWS 간 하이브리드 연결에 사용됩니다.

### 주요 용어

| 용어                              | 설명                                                      |
| --------------------------------- | --------------------------------------------------------- |
| **Site-to-Site VPN**              | 인터넷을 통해 IPsec 암호화 터널로 AWS Amazon VPC에 연결하는 방식 |
...
| NAT Instance         | Amazon EC2에 직접 NAT 설정 (학습/비용 절약)                          |
