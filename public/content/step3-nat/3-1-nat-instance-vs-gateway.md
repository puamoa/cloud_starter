...
learningObjectives:
  ...
  - NAT 설정 후 Private Amazon EC2에서 SSM Session Manager 접속이 가능해지는 원리를 이해할 수 있습니다.
prerequisites:
  - AWS 계정 생성 완료
...
> Step 1에서 생성한 Amazon VPC가 있다면 그것을 사용합니다. 없다면 태스크 0의 AWS CloudFormation으로 생성합니다.

### 아키텍처 다이어그램

<img src="/images/step3/3-1-architecture.png" alt="Step 3-1 아키텍처 다이어그램" class="guide-img-lg" />
...
> (스택 내 Amazon EC2가 남아있으면 먼저 Terminate 후 스택 삭제)
>
...
| Public SG             | my-public-sg        | SSH(22) + Amazon VPC 내부 All Traffic  |
| Private SG            | my-private-sg       | Amazon VPC 내부에서 SSH(22)만          |
...
7. [[Choose file]] 버튼을 클릭하고 다운로드한 `step3-1-nat-prereq.yaml` 파일을 선택합니다.
    ...
    <img src="/images/step3/3-1-step07-upload-template.png" alt="CloudFormation 템플릿 업로드" class="guide-img-sm" />
...
10. **Parameters** 섹션에서 기본값을 확인합니다:
    ...
    <img src="/images/step3/3-1-step10-parameters.png" alt="CloudFormation Parameters 설정" class="guide-img-sm" />
...
13. 페이지 하단의 **Capabilities** 섹션에서 **I acknowledge that AWS CloudFormation might create IAM resources with custom names.** 체크박스를 선택합니다.
    ...
    <img src="/images/step3/3-1-step13-capabilities.png" alt="CloudFormation IAM Capabilities 체크" class="guide-img-sm" />

> [!NOTE]
> 이 템플릿은 Amazon EC2에 AWS Systems Manager (SSM) 접속 권한을 부여하는 IAM Role을 생성합니다.  
...
15. **Review and create** 페이지에서 설정을 확인합니다.
    ...
    <img src="/images/step3/3-1-step15-review.png" alt="CloudFormation Review and create" class="guide-img-sm" />
16. [[Submit]] 버튼을 클릭합니다.
17. 스택 상태가 `CREATE_COMPLETE`가 될 때까지 기다립니다 (약 1~2분).

    <img src="/images/step3/3-1-step17-create-complete.png" alt="CloudFormation CREATE_COMPLETE" class="guide-img-sm" />
...
> [!TIP]
> 이 Role은 Amazon EC2 인스턴스가 AWS Systems Manager에 접속할 수 있는 권한을 부여합니다.  
> Step 2에서 이미 생성했다면 동일한 Role을 재사용합니다. 여러 Amazon EC2에서 같은 Role을 공유할 수 있습니다.
...
| Public Subnet (`my-public-subnet-a`)   | Amazon VPC 콘솔 → Subnets           | 존재                    |
| Private Subnet (`my-private-subnet-a`) | Amazon VPC 콘솔 → Subnets           | 존재                    |
| Internet Gateway                       | Amazon VPC 콘솔 → Internet Gateways | Amazon VPC에 연결됨            |
| Private Route Table                    | Amazon VPC 콘솔 → Route Tables      | Private Subnet에 연결됨 |
| Security Group (Public 또는 EC2용)     | Amazon VPC 콘솔 → Security Groups   | SSH 허용                |
...
이 태스크에서는 Private Subnet에 Amazon EC2를 생성하고, 인터넷 접근이 불가능함을 직접 확인합니다.

### Bastion Host 생성 (Public Subnet)

Private Amazon EC2에 접속하려면 먼저 Public Subnet에 "점프 서버"(Bastion Host)가 필요합니다.
...
35. 다음과 같이 설정합니다:
    ...
    <img src="/images/step3/3-1-step35-bastion-settings.png" alt="Bastion EC2 태그 설정" class="guide-img-sm" />
    ...
    <img src="/images/step3/3-1-step35-instance-type.png" alt="Bastion EC2 Instance type 선택" class="guide-img-sm" />
    ...
    <img src="/images/step3/3-1-step35-network-settings.png" alt="Bastion EC2 Network settings" class="guide-img-sm" />

    > [!NOTE]
    > `my-public-sg`에는 SSH(22) 외부 접근 + Amazon VPC 내부 All Traffic이 허용되어 있습니다.  
    > Bastion은 외부에서 SSH 접속을 받고, 내부에서 Private Amazon EC2로 SSH를 보내는 역할이므로 이 SG가 적합합니다.

    - **Configure storage**: 기본값을 유지합니다 (`8 GiB`, `gp3`).

    - **Advanced details** 섹션을 펼칩니다 (클릭하여 확장):
        - **IAM instance profile**: 드롭다운에서 `my-ec2-ssm-role`을 선택
        - **User data**: 비워둡니다.
        - 나머지 설정은 기본값을 유지합니다.

    <img src="/images/step3/3-1-step35-advanced-details.png" alt="Bastion EC2 Advanced details" class="guide-img-sm" />

    > [!NOTE]
    > Bastion은 Public Subnet에 있어 IGW를 통해 인터넷에 접근 가능합니다.  
    > IAM Role을 연결하면 SSM Agent가 즉시 AWS에 등록되어 Session Manager로도 접속할 수 있습니다.  
    > 반면, 이후 생성할 Private Amazon EC2는 같은 Role을 연결해도 NAT 없이는 SSM이 동작하지 않습니다.  
    > 이 차이를 태스크 2에서 직접 확인합니다.

36. [[Launch instance]] 버튼을 클릭합니다.

    <img src="/images/step3/3-1-step36-launch-instance.png" alt="Bastion EC2 Launch instance" class="guide-img-sm" />

37. 인스턴스 상태가 `Running`이 될 때까지 기다립니다 (약 1~2분).
38. `my-bastion`의 **Public IPv4 address**를 메모합니다.

    <img src="/images/step3/3-1-step38-bastion-running.png" alt="Bastion EC2 Running 상태" class="guide-img-sm" />

### Private Amazon EC2 인스턴스 생성
...
40. 다음과 같이 설정합니다:
    ...
    <img src="/images/step3/3-1-step40-name-tags.png" alt="Private EC2 Name and tags 설정" class="guide-img-sm" />
    ...
    <img src="/images/step3/3-1-step40-network-settings.png" alt="Private EC2 Network settings" class="guide-img-sm" />
    ...
    > [!NOTE]
    > `my-private-sg`에는 Amazon VPC 내부(10.0.0.0/16)에서 SSH(22)만 허용되어 있습니다.  
...
43. `my-private-ec2`의 **Private IPv4 address**를 메모합니다 (예: `10.0.11.xxx`).
    ...
    <img src="/images/step3/3-1-step43-private-ec2-ip.png" alt="Private EC2 Private IP 확인" class="guide-img-sm" />

> [!NOTE]
> Private EC2에는 Public IP가 없으므로 외부에서 직접 접속할 수 없습니다.  
> Bastion Host를 경유해서만 접속할 수 있습니다.

### Bastion을 통해 Private Amazon EC2에 접속
...
<img src="/images/step3/3-1-step44-ssh-bastion.png" alt="Bastion SSH 접속" class="guide-img-sm" />
...
45. Bastion에서 Private Amazon EC2에 접속하기 위해 키 파일을 Bastion으로 전송합니다.  
...
<img src="/images/step3/3-1-step45-scp-key.png" alt="키 파일 SCP 전송" class="guide-img-sm" />
...
46. Bastion에서 Private Amazon EC2에 접속합니다:
...
<img src="/images/step3/3-1-step46-ssh-private.png" alt="Private EC2 SSH 접속" class="guide-img-sm" />

47. `Are you sure you want to continue connecting?` 메시지가 나오면 `yes`를 입력합니다.

> [!OUTPUT]
> Private Amazon EC2에 접속 성공 시 프롬프트가 표시됩니다:
>
> ```
...
> [!TIP]
> **Bastion과 Private Amazon EC2의 프롬프트가 비슷해 보이지만 IP가 다릅니다.**  
...
48. Private Amazon EC2에서 인터넷 접근을 테스트합니다:
...
<img src="/images/step3/3-1-step48-ping-ip.png" alt="Private EC2 ping 8.8.8.8 타임아웃" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step49-ping-domain.png" alt="Private EC2 ping google.com 실패" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step50-dnf-fail.png" alt="Private EC2 dnf check-update 실패" class="guide-img-sm" />
<img src="/images/step3/3-1-step50-dnf-fail2.png" alt="Private EC2 dnf check-update 실패 2" class="guide-img-sm" />
...
51. Private Amazon EC2에서 나갑니다 (Bastion으로 돌아감):
...
<img src="/images/step3/3-1-step52-bastion-ping.png" alt="Bastion ping google.com 성공" class="guide-img-sm" />
...
> 방금 Private Amazon EC2에서는 실패했던 동일한 명령이 Bastion에서는 성공합니다.  
...
56. **Session Manager** 탭을 선택합니다.
    ...
    <img src="/images/step3/3-1-step56-ssm-bastion.png" alt="Bastion Session Manager 활성화 확인" class="guide-img-sm" />
...
61. **Session Manager** 탭을 선택합니다.
    ...
    <img src="/images/step3/3-1-step61-ssm-private.png" alt="Private EC2 Session Manager 비활성화 확인" class="guide-img-sm" />

62. Connect 버튼이 **비활성화(회색)** 되어 있는지 확인합니다.

> [!OUTPUT]
> Private Amazon EC2의 SSM agent status:
>
...
> NAT를 설정한 후(태스크 2 이후) Private Amazon EC2의 SSM도 활성화되는 것을 확인합니다.
...
> NAT Instance는 일반 Amazon EC2 인스턴스에 NAT 기능을 설정한 것입니다.  
...
> **핵심 설정 3가지:**
>
> - **Source/Destination Check 비활성화** — Amazon EC2는 기본적으로 자신이 Source나 Destination이 아닌 트래픽을 폐기합니다.  
>   NAT Instance는 다른 인스턴스의 트래픽을 중계해야 하므로 이 체크를 비활성화해야 합니다.
> - **IP Forwarding 활성화** — 커널이 패킷을 다른 인터페이스로 전달할 수 있게 합니다.
> - **iptables MASQUERADE 설정** — Private Amazon EC2의 IP(10.0.11.x)를 NAT Instance의 Public IP로 변환하는 규칙을 추가합니다.
>
> ```
...
67. 다음과 같이 설정합니다:
    ...
    <img src="/images/step3/3-1-step67-nat-name-tags.png" alt="NAT Instance Name and tags 설정" class="guide-img-sm" />
    ...
    - **Network settings** → [[Edit]] 버튼을 클릭합니다:
        ...
        - 기본으로 추가된 SSH 규칙을 삭제하고, [[Add security group rule]] 버튼을 클릭하여 다음 규칙을 설정합니다:
            - Type: `All traffic` / Source: `0.0.0.0/0` / Description: `Allow all inbound`

    <img src="/images/step3/3-1-step67-nat-network-settings.png" alt="NAT Instance Network settings" class="guide-img-sm" />

    > [!NOTE]
    > **NAT Instance에 All traffic을 여는 이유:**  
    > NAT Instance는 Private Amazon EC2의 모든 종류의 트래픽(HTTP, HTTPS, DNS, ping 등)을 중계해야 합니다.  
    ...
    > Network settings에서 `Create security group`을 선택하면 별도로 Amazon VPC 콘솔에 가지 않아도 EC2 생성과 동시에 SG가 만들어집니다.  
    > 생성된 SG는 이후 EC2 콘솔 → Security Groups에서 확인·수정할 수 있습니다.

    > [!TIP]
    > NAT Instance를 **Public Subnet C** (AZ-c)에 배치합니다.  
    > Private Amazon EC2는 Private Subnet A (AZ-a)에 있으므로, 트래픽이 AZ를 넘어 이동하는 흐름을 확인할 수 있습니다:
    >
    > ```
    ...
    > ```
    >
    > Amazon VPC 내부에서는 서브넷과 AZ가 달라도 라우팅이 자유롭게 이루어집니다.
    >
...
73. **Networking** → **Change source/destination check**를 선택합니다.
    ...
    <img src="/images/step3/3-1-step73-source-dest-check.png" alt="Change source/destination check 메뉴" class="guide-img-sm" />

74. 팝업 창에서 **Stop** 체크박스를 선택합니다 (Source/destination checking을 중지).

    <img src="/images/step3/3-1-step74-stop-check.png" alt="Source/destination check Stop" class="guide-img-sm" />
...
>
> Amazon EC2는 기본적으로 자신이 보낸(Source) 패킷이거나 자신에게 온(Destination) 패킷만 처리합니다.  
> 그 외의 패킷은 폐기합니다.
>
> NAT Instance는 Private Amazon EC2가 보낸 패킷(Source: 10.0.11.x)을 받아서 인터넷으로 전달하는 "중계자"입니다.  
...
> ```
>
> 즉 Stop하면 Amazon EC2가 "우편배달부"처럼 자기 것이 아닌 패킷도 받아서 전달할 수 있게 됩니다.  
> NAT, VPN, 로드밸런서 등 트래픽을 중계하는 모든 Amazon EC2에서 이 설정을 비활성화해야 합니다.
...
76. EC2 Instance Connect로 NAT Instance에 접속합니다:
    ...
    > **EC2 Instance Connect**는 AWS 콘솔에서 브라우저 기반으로 Amazon EC2에 SSH 접속하는 방법입니다.  
    ...
    <img src="/images/step3/3-1-step76-instance-connect.png" alt="EC2 Instance Connect 접속" class="guide-img-sm" />
    <img src="/images/step3/3-1-step76-instance-connect2.png" alt="EC2 Instance Connect 접속 완료" class="guide-img-sm" />
    <img src="/images/step3/3-1-step76-instance-connect3.png" alt="EC2 Instance Connect 터미널" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step77-iptables-install.png" alt="iptables-services 설치" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step78-ip-forward.png" alt="IP Forwarding 활성화" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step79-netstat.png" alt="netstat -i 인터페이스 확인" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step79-ip-link.png" alt="ip link show 인터페이스 확인" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step80-iptables-nat.png" alt="iptables NAT 규칙 추가" class="guide-img-sm" />
...
> | 명령                                                         | 설명                                                               |
> | ------------------------------------------------------------ | ------------------------------------------------------------------ |
> | `-t nat -A POSTROUTING -o ens5 -s 10.0.0.0/16 -j MASQUERADE` | Amazon VPC 내부에서 온 패킷의 Source IP를 NAT Instance의 Public IP로 변환 |
> | `-A FORWARD ... RELATED,ESTABLISHED -j ACCEPT`               | 이미 연결된 세션의 응답 패킷 허용 (인터넷 → Private EC2 응답)      |
> | `-A FORWARD -s 10.0.0.0/16 -o ens5 -j ACCEPT`                | Amazon VPC 내부에서 인터넷으로 나가는 새 패킷 허용                        |
>
> **옵션 상세 설명:**
>
> | 옵션                                   | 의미                                                               |
...
<img src="/images/step3/3-1-step81-iptables-save.png" alt="iptables 규칙 영구 저장" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step82-verify-rules.png" alt="iptables 규칙 확인" class="guide-img-sm" />
...
87. 하단 **Routes** 탭을 선택합니다.
    ...
    <img src="/images/step3/3-1-step87-routes-tab.png" alt="Private Route Table Routes 탭" class="guide-img-sm" />
...
90. 새 경로를 다음과 같이 설정합니다:
    ...
    <img src="/images/step3/3-1-step90-add-route.png" alt="Route Table에 NAT Instance 경로 추가" class="guide-img-sm" />

> [!NOTE]
> `0.0.0.0/0`은 "모든 목적지"를 의미합니다. Amazon VPC 내부 트래픽(10.0.0.0/16)은 이미 `local` 경로로 처리되므로, 이 규칙은 VPC 외부(인터넷)로 나가는 트래픽에만 적용됩니다.

91. [[Save changes]] 버튼을 클릭합니다.

    <img src="/images/step3/3-1-step91-save-routes.png" alt="Route Table 저장 완료" class="guide-img-sm" />
...
93. Bastion에서 Private Amazon EC2에 접속합니다:
...
<img src="/images/step3/3-1-step94-nat-ping-test.png" alt="NAT Instance 통해 ping 성공" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step95-dnf-success.png" alt="dnf check-update 성공" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step96-checkip1.png" alt="curl checkip 결과" class="guide-img-sm" />
<img src="/images/step3/3-1-step96-checkip2.png" alt="NAT Instance Public IP 확인" class="guide-img-sm" />
...
> | --- | ----------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------- |
> | 1   | Route Table에 NAT 경로 존재   | Amazon VPC 콘솔 → Route Tables → `my-private-rt-a` → Routes 탭                | `0.0.0.0/0` → `eni-xxx (my-nat-instance)` 존재 |
...
NAT가 설정되었으므로 이제 Private Amazon EC2의 SSM Agent가 AWS에 등록될 수 있습니다.  
...
99. AWS 콘솔 → **EC2** 서비스 → **Instances**에서 `my-private-ec2`를 선택합니다.
    ...
    <img src="/images/step3/3-1-step43-private-ec2-ip.png" alt="Private EC2 선택" class="guide-img-sm" />

100. 상단 [[Connect]] 버튼을 클릭합니다.
101. **Session Manager** 탭을 선택합니다.

     <img src="/images/step3/3-1-step101-ssm-session.png" alt="Private EC2 SSM Session Manager 활성화" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step103-ssm-whoami.png" alt="SSM Session Manager whoami 및 ping 확인" class="guide-img-sm" />
...
> [!TIP]
> **핵심 포인트:**  
> NAT 설정 전에는 SSM 접속이 불가능했지만, NAT를 연결하자 Bastion 없이도 Private Amazon EC2에 접속할 수 있게 되었습니다.  
> 이것이 실무에서 NAT가 필수인 이유 중 하나입니다 — **Private Amazon EC2를 SSM으로 관리하려면 NAT(또는 VPC Endpoint)가 필요합니다.**
...
✅ **태스크 완료**: NAT Instance를 통해 Private Amazon EC2에서 인터넷 접근이 가능하고, SSM Session Manager로도 접속할 수 있습니다.
...
> [!CONCEPT] NAT Gateway
> NAT Gateway는 AWS 관리형 NAT 서비스입니다.  
> NAT Instance와 동일한 기능을 제공하지만, AWS가 가용성·대역폭·패치를 모두 관리합니다.
>
...
109. `0.0.0.0/0` → `eni-xxxxxxxx (my-nat-instance)` 경로 옆의 [[Remove]] 버튼(X 아이콘)을 클릭합니다.
     ...
     <img src="/images/step3/3-1-step109-remove-nat-route.png" alt="NAT Instance 경로 제거" class="guide-img-sm" />

110. [[Save changes]] 버튼을 클릭합니다.

     <img src="/images/step3/3-1-step110-save-changes.png" alt="Route Table 경로 제거 저장" class="guide-img-sm" />

> [!NOTE]
> NAT Instance 경로를 제거하면 Private Amazon EC2는 다시 인터넷에 접근할 수 없게 됩니다.  
> 이제 NAT Gateway를 생성하여 새 경로를 추가합니다.

### NAT Gateway 생성

111. Amazon VPC 콘솔 왼쪽 메뉴에서 **NAT gateways**를 선택합니다.

     <img src="/images/step3/3-1-step111-nat-gateways.png" alt="NAT gateways 메뉴" class="guide-img-sm" />
...
113. 다음과 같이 설정합니다:
     ...
     <img src="/images/step3/3-1-step113-create-nat-gw.png" alt="NAT Gateway 생성 설정" class="guide-img-sm" />
...
> | **Regional** (신규) | Amazon VPC 내 모든 AZ에 자동 확장/축소. Subnet 지정 불필요 | 멀티 AZ 고가용성을 간편하게 구성 |
>
...
> NAT Gateway를 삭제한 후 Amazon VPC 콘솔 → Elastic IPs에서 미사용 EIP를 직접 Release해야 합니다.  
...
115. [[Create NAT gateway]] 버튼을 클릭합니다.
    ...
    <img src="/images/step3/3-1-step115-nat-gw-created1.png" alt="NAT Gateway 생성 완료" class="guide-img-sm" />
    <img src="/images/step3/3-1-step115-nat-gw-created2.png" alt="NAT Gateway Available 상태" class="guide-img-sm" />
...
121. 새 경로를 다음과 같이 설정합니다:
     ...
     <img src="/images/step3/3-1-step121-nat-gw-route.png" alt="NAT Gateway 경로 추가" class="guide-img-sm" />

122. [[Save changes]] 버튼을 클릭합니다.

     <img src="/images/step3/3-1-step122-save-nat-gw-route.png" alt="NAT Gateway Route Table 저장" class="guide-img-sm" />
...
124. Bastion에서 Private Amazon EC2에 접속합니다:
...
<img src="/images/step3/3-1-step125-nat-gw-ping.png" alt="NAT Gateway ping 테스트 성공" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step126-nat-gw-checkip.png" alt="NAT Gateway curl checkip 확인" class="guide-img-sm" />
<img src="/images/step3/3-1-step126-nat-gw-checkip2.png" alt="NAT Gateway Elastic IP 비교" class="guide-img-sm" />
...
<img src="/images/step3/3-1-step127-nat-gw-dnf.png" alt="NAT Gateway dnf check-update 성공" class="guide-img-sm" />

> [!OUTPUT]
> 패키지 목록이 정상적으로 표시됩니다.

128. Private Amazon EC2에서 나갑니다:
...
✅ **태스크 완료**: NAT Gateway를 통해 Private Amazon EC2에서 인터넷 접근이 가능합니다.
...
> NAT를 연결해도 외부에서 Private Amazon EC2로 직접 접근하는 것은 **불가능**합니다.  
...
4. [[Search resources]] 버튼을 클릭합니다.
    ...
    <img src="/images/step3/3-1-cleanup-step04-tag-editor.png" alt="Tag Editor 검색 결과" class="guide-img-sm" />

5. 이 실습에서 생성한 리소스가 표시되는지 확인합니다.

> [!TIP]
> Tag Editor는 리소스를 찾는 용도로만 사용합니다. 실제 삭제는 다음 단계에서 수행합니다.  
> 검색 결과에 Amazon EC2 인스턴스, NAT Gateway, Elastic IP, VPC, Subnet, SG 등이 보여야 합니다.
...
10. `0.0.0.0/0` 경로 옆의 [[Remove]] 버튼(X 아이콘)을 클릭합니다.
    ...
    <img src="/images/step3/3-1-cleanup-step10-remove-route.png" alt="Route Table 경로 제거" class="guide-img-sm" />
...
12. Amazon VPC 콘솔 왼쪽 메뉴에서 **NAT gateways**를 선택합니다.
13. `my-nat-gateway`를 선택합니다.
14. 상단 **Actions** → **Delete NAT gateway**를 클릭합니다.

    <img src="/images/step3/3-1-cleanup-step14-delete-nat-gw.png" alt="NAT Gateway 삭제 메뉴" class="guide-img-sm" />

15. 확인 입력란에 `delete`를 입력합니다.

    <img src="/images/step3/3-1-cleanup-step15-confirm-delete.png" alt="NAT Gateway 삭제 확인" class="guide-img-sm" />

16. [[Delete]] 버튼을 클릭합니다.
17. 상태가 `Deleting` → `Deleted`로 변경될 때까지 기다립니다 (약 1~2분).

    <img src="/images/step3/3-1-cleanup-step17-deleted1.png" alt="NAT Gateway Deleting 상태" class="guide-img-sm" />
    <img src="/images/step3/3-1-cleanup-step17-deleted2.png" alt="NAT Gateway Deleted 상태" class="guide-img-sm" />
...
18. Amazon VPC 콘솔 왼쪽 메뉴에서 **Elastic IPs**를 선택합니다.
19. 미사용 EIP (Association 열이 비어있는 것)를 선택합니다.
20. 상단 **Actions** → **Release Elastic IP addresses**를 클릭합니다.

    <img src="/images/step3/3-1-cleanup-step20-release-eip.png" alt="Elastic IP Release" class="guide-img-sm" />

21. 확인 팝업에서 [[Release]] 버튼을 클릭합니다.

    <img src="/images/step3/3-1-cleanup-step21-release-confirm.png" alt="Elastic IP Release 확인" class="guide-img-sm" />
    <img src="/images/step3/3-1-cleanup-step21-released.png" alt="Elastic IP Release 완료" class="guide-img-sm" />
...
> Amazon VPC 콘솔 → NAT Gateways에서 상태가 `Deleted`인지 확인합니다.
...
25. 상단 **Instance state** → **Terminate instance**를 클릭합니다.
    ...
    <img src="/images/step3/3-1-cleanup-step25-terminate.png" alt="EC2 인스턴스 Terminate" class="guide-img-sm" />

26. 확인 팝업에서 [[Terminate]] 버튼을 클릭합니다.
27. 모든 인스턴스의 상태가 `Shutting down` → `Terminated`로 변경되는 것을 확인합니다.

<img src="/images/step3/3-1-cleanup-step27-terminated1.png" alt="EC2 인스턴스 Shutting down" class="guide-img-sm" />
<img src="/images/step3/3-1-cleanup-step27-terminated2.png" alt="EC2 인스턴스 Terminated" class="guide-img-sm" />
...
> [!NOTE]
> Step 1에서 생성한 Amazon VPC를 그대로 사용한 경우(태스크 0을 건너뛴 경우) 이 단계를 건너뛰세요.  
> 이후 Step에서도 같은 Amazon VPC를 사용할 예정이라면 삭제하지 마세요.
...
31. 확인 팝업에서 [[Delete stack]]을 클릭합니다.
    ...
    <img src="/images/step3/3-1-cleanup-step31-delete-stack.png" alt="CloudFormation Delete stack 확인" class="guide-img-sm" />

32. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`가 될 때까지 기다립니다 (약 2~3분).

    <img src="/images/step3/3-1-cleanup-step32-deleting1.png" alt="CloudFormation DELETE_IN_PROGRESS" class="guide-img-sm" />
    <img src="/images/step3/3-1-cleanup-step32-deleting2.png" alt="CloudFormation DELETE_COMPLETE" class="guide-img-sm" />
...
> | 원인                                    | 해결 방법                                                                                                 |
> | --------------------------------------- | --------------------------------------------------------------------------------------------------------- |
> | Amazon EC2 인스턴스가 아직 Terminated되지 않음 | EC2 콘솔에서 인스턴스가 `Terminated` 상태인지 확인 → 1~2분 대기 후 재시도                                 |
> | NAT Gateway가 아직 삭제되지 않음        | Amazon VPC 콘솔에서 NAT Gateway 상태 확인 → `Deleted` 확인 후 재시도                                             |
...
36. [[Search resources]] 버튼을 클릭합니다.
    ...
    <img src="/images/step3/3-1-cleanup-step36-tag-verify.png" alt="Tag Editor 최종 확인" class="guide-img-sm" />

37. 검색 결과가 비어있거나, `Terminated` 상태의 Amazon EC2만 남아있으면 정상입니다.
...
| 확인 항목           | 위치                    | 상태                                     |
| ------------------- | ----------------------- | ---------------------------------------- |
| Amazon EC2 인스턴스 3대    | EC2 콘솔 → Instances    | `Terminated`                             |
| NAT Gateway         | Amazon VPC 콘솔 → NAT gateways | `Deleted` (또는 목록에서 사라짐)         |
| Elastic IP          | Amazon VPC 콘솔 → Elastic IPs  | 미사용 EIP 없음 (Release 완료)           |
