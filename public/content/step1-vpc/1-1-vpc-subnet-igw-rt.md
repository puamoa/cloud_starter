...
> [!TIP]
> Private Subnet에도 동일한 태그를 넣어주세요. 나중에 Tag Editor에서 `Session: 1-1`로 검색하면 이 실습에서 만든 Public/Private 서브넷을 한번에 조회할 수 있습니다.
...
47. **Subnet associations** 탭 → [[Edit subnet associations]] → `my-private-subnet-a`를 체크 → [[Save associations]]

48. 다시 Route tables 목록으로 돌아가 [[Create route table]] 버튼을 클릭합니다.
49. 다음과 같이 설정합니다:
    ...
    - **Tags**:
      ...
      - `Step` = `step1`
      - `Session` = `1-1`
50. [[Create route table]] 버튼을 클릭합니다.
51. **Subnet associations** 탭 → [[Edit subnet associations]] → `my-private-subnet-c`를 체크 → [[Save associations]]
...
>
> Step 3에서 NAT Gateway를 추가할 때, AZ-a의 NAT GW는 `my-private-rt-a`에, AZ-c의 NAT GW는 `my-private-rt-c`에 연결합니다. 하나의 AZ에 장애가 발생해도 다른 AZ의 Private Subnet은 독립적으로 인터넷에 접근할 수 있습니다.
>
> **2. 용도별 경로 분리 (실무 핵심)**
>
...
>
> RT를 공유하면 이런 분리가 불가능합니다. DB Subnet에 NAT 경로가 생기면 DB에서 외부로 데이터가 유출될 수 있는 경로가 열리는 셈입니다.
>
> **3. 장애 격리**
>
> RT를 공유하면 잘못된 경로 하나가 모든 Private Subnet에 영향을 미칩니다. 분리하면 한쪽 RT에 문제가 생겨도 다른 서브넷은 정상 동작합니다.
>
> ```
> 실무 3-Tier 구성 예시:
>
...
52. 왼쪽 메뉴에서 **Your VPCs**를 선택하고 `my-vpc`를 클릭합니다.
53. 상세 페이지에서 **Resource map** 탭을 선택합니다.
54. VPC에 연결된 모든 리소스(서브넷, Route Table, IGW, 네트워크 연결)가 다이어그램으로 표시되는지 확인합니다.

> [!TIP]
> **Resource Map**은 VPC의 전체 네트워크 구성을 시각적으로 보여주는 기능입니다.
>
> - 서브넷이 어떤 Route Table에 연결되어 있는지 한눈에 파악 가능.
> - IGW, NAT Gateway 등의 연결 관계를 시각적으로 확인.
> - 리소스를 클릭하면 해당 리소스의 상세 페이지로 이동.
...
55. **Subnets**에서 상단 필터에 `my-vpc`를 입력하여 VPC로 필터링한 뒤, 4개의 서브넷이 모두 표시되는지 확인합니다.
56. **Internet gateways**에서 `my-igw`가 `my-vpc`에 Attached 상태인지 확인합니다.
57. **Route tables**에서 `my-public-rt`의 Routes에 `0.0.0.0/0 → igw` 경로가 있는지 확인합니다.
58. **Route tables**에서 `my-private-rt-a`와 `my-private-rt-c`의 Routes에 `10.0.0.0/16 → local`만 있는지 확인합니다.
...
>
> - **실수 가능성**: 클릭 한 번으로 잘못된 리소스를 삭제하거나, 설정을 빠뜨릴 수 있음.
> - **재현 불가**: 동일한 환경을 다시 만들려면 모든 단계를 기억하고 반복해야 함.
> - **감사 어려움**: 누가 언제 무엇을 변경했는지 추적이 어려움.
> - **휴먼 에러**: 서브넷 CIDR을 잘못 입력하거나, Route Table 연결을 빠뜨리는 등의 실수가 빈번.
>
> **실무에서는 다음과 같이 사용합니다:**
>
> | 방식                                | 사용 시점                            | 장점                            |
...
>
> - 여러 서비스에 흩어진 리소스를 태그 하나로 모아볼 수 있음.
> - 리소스를 클릭하면 해당 서비스 콘솔로 바로 이동 가능.
> - 삭제 기능은 없으므로, 확인 후 각 서비스 콘솔에서 직접 삭제해야 함.
...
> **VPC 삭제가 실패하는 주요 원인:**
>
> - VPC 내에 EC2 인스턴스가 실행 중 (Terminated가 아닌 상태)
> - NAT Gateway가 남아있음
> - Elastic IP가 할당되어 있음
> - VPN 연결이 존재함
> - 다른 서비스(RDS, ELB 등)가 서브넷을 사용 중
>
...
1. 왼쪽 메뉴에서 **Route tables**를 선택합니다.
2. `my-public-rt`를 선택합니다.
3. **Subnet associations** 탭을 선택합니다.
4. [[Edit subnet associations]]를 클릭합니다.
5. 체크된 서브넷을 모두 해제합니다.
6. [[Save associations]]를 클릭합니다.
7. 다시 `my-public-rt`를 선택한 상태에서 **Actions** → [[Delete route table]]을 선택합니다.
8. 확인 팝업에서 `delete`를 입력하고 [[Delete]]를 클릭합니다.
9. 동일하게 `my-private-rt-a`도 Subnet associations 해제 → 삭제합니다.
10. 동일하게 `my-private-rt-c`도 Subnet associations 해제 → 삭제합니다.
...
11. 왼쪽 메뉴에서 **Internet gateways**를 선택합니다.
12. `my-igw`를 선택합니다.
13. **Actions** → [[Detach from VPC]]를 선택합니다.
14. 확인 팝업에서 [[Detach internet gateway]]를 클릭합니다.
15. 상태가 `Detached`로 변경된 것을 확인합니다.
16. 다시 `my-igw`를 선택한 상태에서 **Actions** → [[Delete internet gateway]]를 선택합니다.
17. 확인 팝업에서 `delete`를 입력하고 [[Delete internet gateway]]를 클릭합니다.
...
18. 왼쪽 메뉴에서 **Subnets**를 선택합니다.
19. 상단 필터에서 `my-vpc`로 필터링합니다.
20. 4개 서브넷(`my-public-subnet-a`, `my-public-subnet-c`, `my-private-subnet-a`, `my-private-subnet-c`)을 모두 체크합니다.
21. **Actions** → [[Delete subnet]]을 선택합니다.
22. 확인 팝업에서 `delete`를 입력하고 [[Delete]]를 클릭합니다.

> [!NOTE]
> 서브넷에 EC2 인스턴스나 ENI(네트워크 인터페이스)가 남아있으면 삭제가 실패합니다. EC2 콘솔에서 해당 인스턴스를 Terminate한 뒤 다시 시도하세요.

**④ VPC 삭제:**

23. 왼쪽 메뉴에서 **Your VPCs**를 선택합니다.
24. `my-vpc`를 선택합니다.
25. **Actions** → [[Delete VPC]]를 선택합니다.
26. 확인 팝업에서 `delete`를 입력하고 [[Delete]]를 클릭합니다.
