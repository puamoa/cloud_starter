...
awsServices:
  ...
  - Route Table
learningObjectives:
  - Amazon VPC와 CIDR 블록의 개념을 이해하고 직접 생성할 수 있습니다.
  - Public Subnet과 Private Subnet의 차이를 설명할 수 있습니다.
  - Internet Gateway를 생성하고 Amazon VPC에 연결할 수 있습니다.
...
> [!NOTE]
> 이 실습은 AWS 네트워크의 첫 번째 단계입니다. 선행 조건 없이 처음부터 Amazon VPC를 생성합니다.

<img src="/images/architecture/step1-1-vpc-architecture.png" alt="Step 1-1 VPC 아키텍처 구성도" class="guide-img-lg" />

## 태스크 1: VPC 생성

> [!CONCEPT] VPC (Virtual Private Cloud)
> Amazon VPC는 AWS 클라우드 내에서 논리적으로 격리된 가상 네트워크입니다.  
...
> [!OUTPUT]
> Amazon VPC가 생성되면 상세 페이지로 이동합니다.
...
> [!CONCEPT] Subnet (서브넷)
> 서브넷은 Amazon VPC의 IP 주소 범위를 더 작은 단위로 나눈 것입니다.
>
...
>
> 이 설정을 활성화하면 이 서브넷에 Amazon EC2 인스턴스를 생성할 때 **자동으로 공인 IP가 할당**됩니다.  
...
> Internet Gateway는 Amazon VPC와 인터넷 간의 통신을 가능하게 하는 게이트웨이입니다.  
...
> [!NOTE]
> IGW가 생성되면 상태가 `Detached`입니다. Amazon VPC에 연결해야 사용할 수 있습니다.
...
> 이제 이 Amazon VPC의 리소스가 인터넷과 통신할 수 있는 경로가 열렸습니다 (Route Table 설정 후).

✅ **태스크 완료**: Internet Gateway가 생성되고 Amazon VPC에 연결되었습니다.

> [!TIP]
> IGW를 Amazon VPC에 연결했지만, 아직 인터넷 통신은 되지 않습니다.  
...
>
> Amazon VPC를 생성하면 기본 Route Table(Main)이 자동 생성됩니다. 이것은 Private용으로 사용하고, Public용은 별도로 생성합니다.
...
> [!NOTE]
> 서브넷을 명시적으로 Route Table에 연결하지 않으면 Amazon VPC의 Main Route Table이 적용됩니다.  
...
>   → DB 서버: Amazon VPC 내부 통신만 가능, 외부 유출 경로 원천 차단
> ```
>
...
<img src="/images/architecture/step1-1-vpc-architecture.png" alt="Step 1-1 최종 VPC 아키텍처" class="guide-img-lg" />
...
56. Amazon VPC에 연결된 모든 리소스(서브넷, Route Table, IGW, 네트워크 연결)가 다이어그램으로 표시되는지 확인합니다.

> [!TIP]
> **Resource Map**은 Amazon VPC의 전체 네트워크 구성을 시각적으로 보여주는 기능입니다.
>
...
✅ **태스크 완료**: 전체 Amazon VPC 네트워크 구성이 완료되었습니다.
...
- Internet Gateway를 생성하고 Amazon VPC에 연결했습니다.
...
Amazon VPC 내에 EC2, RDS, NAT Gateway 등 다른 리소스가 없는 경우, Amazon VPC를 삭제하면 연결된 리소스가 함께 삭제됩니다.
...
>
> 단, Amazon VPC 내에 Amazon EC2 인스턴스, NAT Gateway, VPN, Elastic IP 등이 남아있으면 삭제가 실패합니다.  
...
Amazon VPC 삭제가 실패하는 경우, 의존 관계가 있는 리소스를 **생성의 역순**으로 개별 삭제해야 합니다.

> [!NOTE]
> **VPC 삭제가 실패하는 주요 원인:**
>
> - Amazon VPC 내에 Amazon EC2 인스턴스가 실행 중입니다. (Terminated가 아닌 상태)
> - NAT Gateway가 남아있습니다.
...
IGW를 삭제하려면 먼저 Amazon VPC에서 분리(Detach)해야 합니다.
...
> [!WARNING]
> IGW는 반드시 **Detach → Delete** 순서로 진행해야 합니다. Amazon VPC에 연결된 상태에서는 "The internet gateway is in use" 오류가 발생합니다.
...
> [!NOTE]
> 서브넷에 Amazon EC2 인스턴스나 ENI(네트워크 인터페이스)가 남아있으면 삭제가 실패합니다.  
