...
이 실습에서는 Amazon EC2 인스턴스와 RDS 인스턴스에 적용할 Security Group을 생성합니다.  
...
> Security Group은 생성만으로는 동작하지 않으며, 이후 Step 2에서 Amazon EC2 인스턴스, Step 4에서 RDS 인스턴스를 생성할 때 해당 SG를 연결해야 실제로 적용됩니다.
...
>
> 이번 실습부터는 Amazon VPC 환경을 "VPC and more" 마법사로 빠르게 구성한 뒤, 본론인 Security Group 설정에 집중합니다.
...
> 토글(▶)을 클릭하면 펼쳐지며, 여기서 추가한 태그는 Amazon VPC와 함께 생성되는 모든 리소스(서브넷, IGW, Route Table 등)에 자동으로 적용됩니다.  
...
> 2. **Public IP 확인**: Amazon EC2 인스턴스에 Public IP가 할당되어 있는지 확인합니다. (Public Subnet + Auto-assign Public IP 활성화 필요)
> 3. **Route Table 확인**: 서브넷의 Route Table에 `0.0.0.0/0 → IGW` 경로가 있는지 확인합니다.
> 4. **인스턴스 상태 확인**: Amazon EC2 인스턴스가 `running` 상태인지 확인합니다.
...
> [!NOTE]
> Security Group은 Amazon VPC와 달리 Name tag 전용 입력 필드가 없습니다.  
...
>
> - Amazon EC2 인스턴스의 IP가 변경되어도 규칙을 수정할 필요 없음
...
>
> **예시 1: Amazon EC2가 외부 API를 호출하는 경우**
>
> ```
> EC2 → 외부 API (아웃바운드)
...
>   → Amazon EC2의 SG 아웃바운드: All traffic 허용 → 통과 ✓
>   → RDS의 SG 인바운드: 3306 from my-ec2-sg 허용 → 통과 ✓
>   → 양쪽 SG가 이 연결을 기억함
>
> RDS → EC2 (쿼리 결과 응답)
>   → RDS의 SG: Stateful이므로 응답 자동 허용 ✓
>   → Amazon EC2의 SG: Stateful이므로 응답 자동 허용 ✓
> ```
>
...
Security Group과 Amazon VPC는 이후 실습(Step 2: EC2, Step 3: NAT, Step 4: RDS)에서 계속 사용합니다.  
**삭제하지 않고 그대로 유지합니다.**

> [!TIP]
> Security Group과 Amazon VPC는 항상 무료이므로 유지해도 비용이 발생하지 않습니다.  
...
> [!NOTE]
> Amazon EC2 인스턴스가 이 Security Group을 사용 중이면 삭제가 실패합니다.  
> 해당 인스턴스를 먼저 Terminate하거나, 인스턴스의 Security Group을 다른 것으로 변경한 뒤 삭제하세요.

**단계 4: VPC 삭제 (선택)**

VPC와 서브넷, IGW, Route Table까지 모두 정리하려면 Amazon VPC를 삭제합니다.
