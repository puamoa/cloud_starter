...
awsServices:
  - Amazon S3
learningObjectives:
  - Amazon S3 버킷을 생성하고 주요 설정 옵션을 이해할 수 있습니다.
  - Block Public Access, 버전 관리, 암호화 설정을 구성할 수 있습니다.
  - 버킷 정책과 ACL의 차이를 설명할 수 있습니다.
  - Amazon S3의 다양한 사용 시나리오를 설명할 수 있습니다.
...
파일 업로드/다운로드를 테스트하고, 버킷 정책과 수명 주기 규칙을 설정하여 실무에서 Amazon S3를 활용하는 방법을 학습합니다.
...
> - **객체(Object)**: Amazon S3에 저장되는 개별 파일. 최대 5TB까지 저장 가능합니다.
...
- **정적 웹 호스팅**: 별도 서버 없이 HTML/CSS/JS 파일을 Amazon S3에서 직접 서빙 가능 (CloudFront 연동 시 CDN + HTTPS)
- **버전 관리**: 객체의 모든 변경 이력을 자동 보존하여 실수로 덮어쓰거나 삭제해도 복원 가능

✅ **태스크 완료** — Amazon S3의 핵심 개념(오브젝트 스토리지, 버킷, 객체, 키)을 이해했습니다.

---

## 태스크 2: Amazon S3 버킷 생성

### 버킷 이름 규칙

Amazon S3 버킷에는 두 가지 네임스페이스 방식이 있습니다:
...
5. **General configuration** 섹션을 설정합니다. 각 필드를 아래와 같이 입력합니다:
   ...
   - **Bucket name**: `{닉네임}-starter-app` (예: `hong-starter-app`, `mylab-starter-app`)
    ...
    <img src="/images/step5/5-1-step5-general-config.png" alt="General configuration 설정" class="guide-img-sm" />
...
7. **Block Public Access settings for this bucket** 섹션:
   - ✅ **Block all public access** 체크 (기본값 유지)
    ...
    <img src="/images/step5/5-1-step7-block-public-access.png" alt="Block Public Access 설정" class="guide-img-sm" />
...
9. **Tags** 섹션 (선택 사항이지만 실습 관리를 위해 권장):
   ...
   - **Key**: `Session`, **Value**: `5-1`
    ...
    <img src="/images/step5/5-1-step9-tags.png" alt="Tags 설정" class="guide-img-sm" />
...
10. **Default encryption** 섹션:
    ...
    <img src="/images/step5/5-1-step10-encryption.png" alt="Default encryption 설정" class="guide-img-sm" />

11. [[Create bucket]]을 클릭합니다.

    <img src="/images/step5/5-1-step11-create-bucket.png" alt="Create bucket 완료" class="guide-img-sm" />
...
✅ **태스크 완료** — Amazon S3 버킷을 생성하고 기본 설정을 확인했습니다.
...
Amazon S3는 기본적으로 모든 새 객체에 서버 측 암호화를 적용합니다.

12. S3 콘솔에서 생성한 버킷을 클릭합니다.
13. **Properties** 탭을 선택합니다.

    <img src="/images/step5/5-1-step13-properties-tab.png" alt="Properties 탭 선택" class="guide-img-sm" />
...
>
> 위 두 개는 서로 다른 Origin이므로, 브라우저가 프론트엔드에서 Amazon S3로 직접 요청을 보내면 차단합니다.
>
> **왜 차단하는가? ** 악의적인 웹사이트가 사용자 브라우저를 이용해 다른 서버의 데이터를 무단으로 가져가는 것을 방지하기 위한 보안 정책(Same-Origin Policy)입니다.
>
> **해결 방법: ** Amazon S3 버킷에 "이 도메인에서 오는 요청은 허용한다"고 CORS 설정을 추가하면, 브라우저가 해당 요청을 통과시킵니다.
>
> ```
...
브라우저에서 Amazon S3에 직접 파일을 업로드하려면 CORS 설정이 필요합니다.

15. **Permissions** 탭을 선택합니다.

    <img src="/images/step5/5-1-step15-permissions-tab.png" alt="Permissions 탭 선택" class="guide-img-sm" />

16. 하단의 **Cross-origin resource sharing (CORS)** 섹션에서 [[Edit]]을 클릭합니다.

    <img src="/images/step5/5-1-step16-cors-edit.png" alt="CORS 섹션 Edit 클릭" class="guide-img-sm" />
...
<img src="/images/step5/5-1-step17-cors-json.png" alt="CORS JSON 입력" class="guide-img-sm" />
...
18. [[Save changes]]를 클릭합니다.
    ...
    <img src="/images/step5/5-1-step18-cors-saved.png" alt="CORS Save changes" class="guide-img-sm" />
...
콘솔에서 직접 파일을 업로드하고 다운로드하여 Amazon S3의 기본 동작을 확인합니다.
...
<img src="/images/step5/5-1-step19-test-file.png" alt="테스트 파일 준비" class="guide-img-sm" />

### 파일 업로드

19. **Objects** 탭에서 [[Upload]]를 클릭합니다.

    <img src="/images/step5/5-1-step19-upload-btn.png" alt="Upload 버튼 클릭" class="guide-img-sm" />

20. [[Add files]]를 클릭하고 방금 만든 `test.txt`를 선택합니다.
    - 또는 파일을 Upload 영역으로 **드래그 앤 드롭**하면 바로 추가됩니다.

    <img src="/images/step5/5-1-step20-add-files.png" alt="Add files로 파일 선택" class="guide-img-sm" />
...
22. 우측 상단의 [[Close]]를 클릭하여 버킷 Objects 목록으로 돌아갑니다.
    ...
    <img src="/images/step5/5-1-step22-close-upload.png" alt="Close 버튼으로 돌아가기" class="guide-img-sm" />

### 폴더 생성

23. [[Create folder]]를 클릭합니다.

    <img src="/images/step5/5-1-step23-create-folder.png" alt="Create folder 클릭" class="guide-img-sm" />

24. Folder name에 `public`을 입력합니다.

    <img src="/images/step5/5-1-step24-folder-name.png" alt="Folder name 입력" class="guide-img-sm" />
...
>
> Amazon S3에서 "폴더"는 실제로 `/`로 끝나는 0바이트 객체입니다. 폴더 안의 파일 암호화는 파일 업로드 시점의 버킷 기본 설정에 의해 결정됩니다.
...
27. **Properties** 탭의 각 섹션을 확인합니다:
    ...
    <img src="/images/step5/5-1-step27-object-properties.png" alt="Object Properties 확인" class="guide-img-sm" />
...
31. **Object URL**을 브라우저 주소창에 붙여넣기합니다.
    ...
    <img src="/images/step5/5-1-step31-access-denied.png" alt="AccessDenied 에러" class="guide-img-sm" />
...
32. [[Open]]을 클릭하면 임시 Presigned URL로 파일이 열립니다 (로그인된 사용자만 가능한 임시 접근).
    ...
    <img src="/images/step5/5-1-step32-presigned-url.png" alt="Open으로 Presigned URL 접근" class="guide-img-sm" />

> [!WARNING]
> **한글 파일이 깨져 보이는 경우:**
> [[Open]]으로 텍스트 파일을 열면 한글이 `�`로 깨져 보일 수 있습니다.  
> 이는 Amazon S3가 파일의 Content-Type을 `text/plain`으로 서빙할 때 인코딩(charset)을 지정하지 않아 브라우저가 UTF-8이 아닌 다른 인코딩으로 해석하기 때문입니다.  
...
Amazon S3에 업로드한 파일을 브라우저에서 URL만으로 직접 열 수 있도록 설정합니다.
...
<img src="/images/step5/5-1-step33-html-file.png" alt="HTML 테스트 파일 준비" class="guide-img-sm" />
...
34. **Block public access (bucket settings)** 섹션에서 [[Edit]]을 클릭합니다.
    ...
    <img src="/images/step5/5-1-step34-edit-block-public.png" alt="Block Public Access Edit 클릭" class="guide-img-sm" />

35. ☐ **Block all public access** 체크를 해제합니다.

    <img src="/images/step5/5-1-step35-uncheck-block.png" alt="Block all public access 해제" class="guide-img-sm" />
...
37. 확인 입력란에 `confirm`을 입력하고 [[Confirm]]을 클릭합니다.
    ...
    <img src="/images/step5/5-1-step37-confirm-block.png" alt="confirm 입력 후 Confirm" class="guide-img-sm" />

### 버킷 정책 설정 (public/ 경로만 공개)

38. **Bucket policy** 섹션에서 [[Edit]]을 클릭합니다.

    <img src="/images/step5/5-1-step38-bucket-policy-edit.png" alt="Bucket policy Edit 클릭" class="guide-img-sm" />
...
<img src="/images/step5/5-1-step39-bucket-policy-json.png" alt="버킷 정책 JSON 입력" class="guide-img-sm" />
...
40. [[Save changes]]를 클릭합니다.
    ...
    <img src="/images/step5/5-1-step40-policy-saved.png" alt="Bucket policy Save changes" class="guide-img-sm" />

### HTML 파일 업로드 및 브라우저 접근 테스트

41. **Objects** 탭 → `public/` 폴더로 이동합니다.

    <img src="/images/step5/5-1-step41-public-folder.png" alt="public 폴더로 이동" class="guide-img-sm" />

42. 준비한 `index.html`을 업로드합니다 (드래그 앤 드롭 또는 Add files).

    <img src="/images/step5/5-1-step42-upload-html.png" alt="index.html 업로드" class="guide-img-sm" />

43. [[Upload]]를 클릭합니다.

    <img src="/images/step5/5-1-step43-upload-complete.png" alt="Upload 완료" class="guide-img-sm" />
44. 업로드된 `index.html`을 클릭하고 **Object URL**을 복사합니다.
45. 브라우저 주소창에 Object URL을 붙여넣기합니다.

    <img src="/images/step5/5-1-step45-public-html-browser.png" alt="브라우저에서 퍼블릭 HTML 확인" class="guide-img-sm" />

> [!OUTPUT]
> 브라우저에 "🎉 Amazon S3에서 직접 열린 페이지입니다!" 라는 HTML 페이지가 표시됩니다.
> Amazon S3에 저장된 파일을 URL만으로 누구나 열 수 있는 상태입니다.

> [!TIP]
> 지금은 단순한 HTML 파일 하나를 열어본 것입니다.  
> 이 방식을 확장하면 CSS, JavaScript, 이미지를 포함한 **완전한 정적 웹사이트**를 Amazon S3에서 호스팅할 수 있습니다.  
...
46. 버킷 루트의 `test.txt`의 Object URL을 브라우저에서 열어봅니다.
    ...
    <img src="/images/step5/5-1-step31-access-denied.png" alt="비공개 파일 AccessDenied" class="guide-img-sm" />
...
<img src="/images/step5/5-1-step47-overwrite-file.png" alt="덮어쓰기용 파일 준비" class="guide-img-sm" />

47. `public/` 폴더에 수정된 `index.html`을 다시 업로드합니다.

    <img src="/images/step5/5-1-step47-upload-v2.png" alt="수정된 파일 업로드" class="guide-img-sm" />
48. [[Upload]]를 클릭합니다.

    <img src="/images/step5/5-1-step48-upload-click.png" alt="Upload 클릭" class="guide-img-sm" />

49. 브라우저에서 같은 Object URL을 새로고침합니다.

    <img src="/images/step5/5-1-step49-browser-v2.png" alt="브라우저에서 Version 2 확인" class="guide-img-sm" />

> [!OUTPUT]
> "📝 수정된 페이지 (Version 2)" 가 표시됩니다. 이전 내용("🎉 Amazon S3에서 직접 열린 페이지입니다!")은 사라졌습니다.

50. S3 콘솔에서 `public/index.html`을 클릭하고 **Versions** 탭을 확인합니다.

    <img src="/images/step5/5-1-step50-no-versions.png" alt="Versions 탭 - No versions" class="guide-img-sm" />
...
51. S3 콘솔에서 버킷 이름을 클릭하여 버킷 루트로 돌아간 뒤, **Properties** 탭을 선택합니다.
    ...
    <img src="/images/step5/5-1-step51-properties-tab.png" alt="Properties 탭 이동" class="guide-img-sm" />

52. **Bucket Versioning** 섹션에서 [[Edit]]을 클릭합니다.
53. **Enable**을 선택합니다.

    <img src="/images/step5/5-1-step53-enable-versioning.png" alt="Versioning Enable 선택" class="guide-img-sm" />

54. [[Save changes]]를 클릭합니다.

    <img src="/images/step5/5-1-step54-save-versioning.png" alt="Versioning Save changes" class="guide-img-sm" />
...
<img src="/images/step5/5-1-step55-v3-file.png" alt="Version 3 파일 준비" class="guide-img-sm" />

55. `public/` 폴더에 수정된 `index.html`을 다시 업로드합니다.

    <img src="/images/step5/5-1-step55-upload-v3.png" alt="Version 3 업로드" class="guide-img-sm" />

56. [[Upload]]를 클릭합니다.

    <img src="/images/step5/5-1-step56-upload-click.png" alt="Upload 클릭" class="guide-img-sm" />

57. 브라우저에서 같은 Object URL을 새로고침합니다.

    <img src="/images/step5/5-1-step57-browser-v3.png" alt="브라우저에서 Version 3 확인" class="guide-img-sm" />
...
58. S3 콘솔에서 `public/index.html`을 클릭하고 **Versions** 탭을 선택합니다.
    ...
    <img src="/images/step5/5-1-step58-versions-tab.png" alt="Versions 탭 - 두 개의 버전" class="guide-img-sm" />
59. 이제 **두 개의 버전**이 표시됩니다 — 버전 관리 활성화 이후의 변경 이력이 보존됩니다.
60. 이전 버전(Version 2)을 클릭하여 다운로드합니다.

    <img src="/images/step5/5-1-step60-download-prev.png" alt="이전 버전 다운로드" class="guide-img-sm" />

61. 다운로드한 파일을 열어 "📝 수정된 페이지 (Version 2)" 내용이 보존되어 있는지 확인합니다.

    <img src="/images/step5/5-1-step61-verify-prev.png" alt="이전 버전 내용 확인" class="guide-img-sm" />
...
62. S3 콘솔에서 버킷 → **Management** 탭을 선택합니다.
    ...
    <img src="/images/step5/5-1-step62-management-tab.png" alt="Management 탭 선택" class="guide-img-sm" />
...
64. 규칙을 설정합니다:
    ...
    <img src="/images/step5/5-1-step64-lifecycle-settings.png" alt="Lifecycle rule 설정" class="guide-img-sm" />

65. **Lifecycle rule actions** 섹션에서 다음을 체크합니다:
    - ✅ **Transition current versions of objects between storage classes**
    - ✅ **Expire current versions of objects**

    <img src="/images/step5/5-1-step65-lifecycle-actions.png" alt="Lifecycle rule actions 체크" class="guide-img-sm" />
...
66. **Transition current versions of objects between storage classes** 섹션:
    ...
    <img src="/images/step5/5-1-step66-transition-settings.png" alt="Transition 설정" class="guide-img-sm" />
...
68. [[Create rule]]을 클릭합니다.
    ...
    <img src="/images/step5/5-1-step68-create-rule.png" alt="Create rule 클릭" class="guide-img-sm" />
...
Amazon S3는 다양한 용도로 활용됩니다. 실무에서 가장 많이 사용되는 시나리오를 정리합니다.
...
- 대용량 데이터를 Amazon S3에 저장하고 Athena로 분석
...
- AWS CloudTrail API 호출 기록을 Amazon S3에 자동 저장 (누가 언제 무엇을 했는지 감사 추적)
...
✅ **태스크 완료** — Amazon S3의 다양한 사용 시나리오와 각 시나리오별 권장 설정을 이해했습니다.
...
- Amazon S3 버킷을 생성하고 암호화, CORS 등 핵심 설정을 구성했습니다.
...
- Amazon S3의 다양한 실무 시나리오(파일 저장, 정적 호스팅, 백업, 감사 로그 등)를 이해했습니다.

---

## 실습에서 다루지 않은 주요 기능

아래 기능들은 Amazon S3를 실무에서 운영할 때 자주 사용되지만, 별도의 심화 학습이 필요한 주제들입니다.
...
- 해외에서 서울 리전 Amazon S3로 대용량 파일을 업로드할 때 유용
...
1. S3 콘솔 → 버킷 → **Permissions** 탭으로 이동합니다.
    ...
    <img src="/images/step5/5-1-cleanup1-permissions-tab.png" alt="Permissions 탭으로 이동" class="guide-img-sm" />

2. **Bucket policy** 섹션에서 [[Edit]]을 클릭합니다.
3. 정책 내용을 모두 삭제합니다 (빈 상태).

    <img src="/images/step5/5-1-cleanup3-delete-policy.png" alt="정책 내용 삭제" class="guide-img-sm" />

4. [[Save changes]]를 클릭합니다.

    <img src="/images/step5/5-1-cleanup4-save-policy.png" alt="Save changes 클릭" class="guide-img-sm" />
...
8. 확인 입력란에 `confirm`을 입력하고 [[Confirm]]을 클릭합니다.
    ...
    <img src="/images/step5/5-1-cleanup8-confirm-block.png" alt="confirm 입력 후 Confirm" class="guide-img-sm" />

9. **Management** 탭 → **Lifecycle rules** 섹션에서 `move-to-ia-and-delete` 규칙을 선택합니다.

    <img src="/images/step5/5-1-cleanup9-lifecycle-select.png" alt="Lifecycle rule 선택" class="guide-img-sm" />

10. [[Delete]]를 클릭합니다.
11. 확인 팝업에서 [[Delete]]를 클릭합니다.

    <img src="/images/step5/5-1-cleanup11-delete-btn.png" alt="Delete 클릭" class="guide-img-sm" />

    <img src="/images/step5/5-1-cleanup11-confirm-delete.png" alt="삭제 확인" class="guide-img-sm" />
...
15. [[Search resources]] 버튼을 클릭합니다.
    ...
    <img src="/images/step5/5-1-cleanup15-tag-editor.png" alt="Tag Editor 검색 결과" class="guide-img-sm" />

16. 이 실습에서 생성한 Amazon S3 버킷이 표시되는지 확인합니다.
...
29. 버킷 이름 왼쪽의 라디오 버튼을 선택합니다.
    ...
    <img src="/images/step5/5-1-cleanup29-select-bucket.png" alt="버킷 선택" class="guide-img-sm" />

30. [[Empty]]를 클릭합니다.
31. 확인 입력란에 `permanently delete`를 입력합니다.

    <img src="/images/step5/5-1-cleanup31-permanently-delete.png" alt="permanently delete 입력" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-1-cleanup-object2.png" alt="개별 객체 삭제 1" class="guide-img-sm" />
>
> <img src="/images/step5/5-1-cleanup-object3.png" alt="개별 객체 삭제 2" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-1-cleanup-cli2.png" alt="CloudShell CLI 삭제 1" class="guide-img-sm" />
>
> <img src="/images/step5/5-1-cleanup-cli3.png" alt="CloudShell CLI 삭제 2" class="guide-img-sm" />
...
36. 버킷 이름을 입력하여 확인합니다.
    ...
    <img src="/images/step5/5-1-cleanup36-bucket-name-confirm.png" alt="버킷 이름 입력하여 삭제 확인" class="guide-img-sm" />
...
39. **Tag Editor**에서 `Session: 5-1`로 다시 검색하여 관련 리소스가 남아있지 않은지 확인합니다.
    ...
    <img src="/images/step5/5-1-cleanup39-tag-verify.png" alt="Tag Editor 검색 결과 확인" class="guide-img-sm" />

✅ **옵션 B 완료**: 모든 리소스가 정리되었습니다.

> [!TIP]
> **CloudFormation이 생성한 Amazon S3 버킷 (`cf-templates-...`):**
>
> 이전 실습에서 CloudFormation을 사용한 경우, Amazon S3 버킷 목록에 `cf-templates-{랜덤문자열}-{리전}` 형태의 버킷이 남아있을 수 있습니다.  
