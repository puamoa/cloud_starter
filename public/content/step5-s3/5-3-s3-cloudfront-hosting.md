...
> | 접속 시 S3 버킷 목록 XML | REST API 엔드포인트를 Origin으로 사용 | Origin을 웹사이트 엔드포인트(`Other`)로 변경 |
...
> 실제로 S3에 `/about` 파일은 없지만(404), CloudFront가 `index.html`을 200으로 대신 응답 → SPA 라우터가 URL을 처리합니다.
...
캐시 무효화를 체험하려면 먼저 S3의 파일을 변경해야 합니다.
...
> 이 명령어는 S3가 아닌 CloudFront 서비스 호출이므로 `s3-dev-user`의 정책에 포함되지 않습니다.  
> 다음 중 하나로 실행하세요:
>
...
> - CloudFront만 삭제하고 S3 버킷은 유지해도 됩니다 (단계 4~5 건너뜀).
> - S3 정적 호스팅 비용은 저장된 파일 크기에 비례하며, 수 MB 수준이면 월 1원 미만입니다.
...
### 단계 4: S3 버킷 비우기 (Empty)

> [!NOTE]
> S3 버킷을 이후 실습에서 재사용하려면 이 단계와 단계 5를 건너뛰세요.
...
| S3 버킷 | S3 콘솔 → Buckets | 목록에서 사라짐 (또는 유지 선택) |
