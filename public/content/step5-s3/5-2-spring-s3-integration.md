...
learningObjectives:
  ...
  - MultipartFile을 Amazon S3에 업로드하는 서비스를 구현할 수 있습니다.
  - Presigned URL을 생성하여 클라이언트 직접 업로드를 구현할 수 있습니다.
  - Amazon S3 객체를 삭제하고 목록을 조회할 수 있습니다.
  - IAM Role을 Amazon EC2에 연결하여 Access Key 없이 Amazon S3에 접근할 수 있습니다.
prerequisites:
  - AWS 계정 생성 완료
  - Amazon S3 버킷 생성 완료 (Step 5-1 참조)
  - Spring Boot 또는 Spring MVC 프로젝트 (로컬)
estimatedCost: 크레딧 내 사용 가능 (비용 매우 저렴)
---

이 실습에서는 Spring 애플리케이션에서 AWS SDK v2를 사용하여 Amazon S3에 파일을 업로드, 다운로드, 조회하는 기능을 구현합니다.  
먼저 **로컬 환경에서 개발·테스트**한 후, CloudFormation으로 Amazon EC2를 생성하고 **IAM Role을 직접 만들어 연결**하여 배포합니다.
...
| **Presigned URL**                     | 임시 서명이 포함된 URL — AWS 자격 증명 없이 Amazon S3 객체에 접근 가능        |
| **IAM Role**                          | Amazon EC2에 부여하는 권한 — Access Key 없이 AWS 서비스에 접근                |
...
> [!TIP]
> Amazon S3 버킷 이름을 메모해 두세요. 이후 태스크에서 `application.yml` 설정과 IAM 정책에 사용됩니다.  
...
> [!NOTE]
> Amazon S3 버킷의 상세 개념(버전 관리, 수명 주기, 스토리지 클래스 등)을 학습하려면 [Step 5-1](/week/5/session/1)을 참고하세요.
...
6. **General configuration** 섹션을 설정합니다:
   ...
   <img src="/images/step5/5-2-step6-cors-edit.png" alt="General configuration 설정" class="guide-img-sm" />
...
8. **Block Public Access settings for this bucket** 섹션:
   ...
   <img src="/images/step5/5-2-step8-block-public-access.png" alt="Block Public Access 해제" class="guide-img-sm" />
...
10. **Tags** 섹션에서 [[Add new tag]]를 클릭하여 다음 태그를 추가합니다:
    ...
    <img src="/images/step5/5-2-step10-tags.png" alt="Tags 설정" class="guide-img-sm" />
...
12. [[Create bucket]]을 클릭합니다.
    ...
    <img src="/images/step5/5-2-step12-create-bucket.png" alt="Create bucket 완료" class="guide-img-sm" />
...
16. **Block all public access** 체크를 **해제**합니다.
    ...
    <img src="/images/step5/5-2-step16-cors-saved.png" alt="Block Public Access 해제" class="guide-img-sm" />

17. [[Save changes]]를 클릭합니다.
18. 확인 필드에 `confirm`을 입력하고 [[Confirm]]을 클릭합니다.

    <img src="/images/step5/5-2-step18-confirm-block.png" alt="confirm 입력" class="guide-img-sm" />

    <img src="/images/step5/5-2-step18-saved.png" alt="Block Public Access 해제 완료" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step20-bucket-policy.png" alt="버킷 정책 JSON 입력" class="guide-img-sm" />
...
21. [[Save changes]]를 클릭합니다.
    ...
    <img src="/images/step5/5-2-step21-policy-saved.png" alt="버킷 정책 저장 완료" class="guide-img-sm" />
...
✅ **태스크 완료**: Amazon S3 버킷과 공개 설정이 준비되었습니다.

---

## 태스크 1: 로컬 개발용 IAM 사용자 + Access Key 설정

Spring SDK가 로컬에서 Amazon S3에 접근하려면 **자격 증명(Access Key)**이 필요합니다.  
...
>
> 즉 `~/.aws/credentials`에 키를 설정해두면 **코드 변경 없이** Spring 앱이 Amazon S3에 접근합니다.  
> Amazon EC2에 배포할 때는 IAM Role을 연결하면 키 자체가 불필요하고, 동일한 코드가 그대로 동작합니다.
...
23. 왼쪽 메뉴에서 **IAM Users**를 선택합니다.
    ...
    <img src="/images/step5/5-2-step23-iam-users.png" alt="IAM Users 메뉴 선택" class="guide-img-sm" />

24. [[Create user]]를 클릭합니다.
25. **User name**: `s3-dev-user`를 입력합니다.

    <img src="/images/step5/5-2-step25-user-name.png" alt="User name 입력" class="guide-img-sm" />

26. **Provide user access to the AWS Management Console** 체크를 **하지 않습니다** (콘솔 접근 불필요).
27. [[Next]]를 클릭합니다.
28. **Attach policies directly**를 선택합니다.

    <img src="/images/step5/5-2-step28-attach-policies.png" alt="Attach policies directly 선택" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step31-create-user.png" alt="IAM 정책 JSON 입력" class="guide-img-sm" />
...
34. **Description**: `Allow PutObject, GetObject, DeleteObject, ListBucket on specific S3 bucket`를 입력합니다.
    ...
    <img src="/images/step5/5-2-step34-policy-name.png" alt="Policy name과 Description 입력" class="guide-img-sm" />
...
36. [[Create policy]]를 클릭합니다.
    ...
    <img src="/images/step5/5-2-step36-policy-created.png" alt="Create policy 완료" class="guide-img-sm" />
...
40. `S3AppBucketPolicy` 체크박스를 선택합니다.
    ...
    <img src="/images/step5/5-2-step40-attach-policy.png" alt="S3AppBucketPolicy 선택" class="guide-img-sm" />

41. [[Next]]를 클릭합니다.

    <img src="/images/step5/5-2-step41-create-user-complete.png" alt="사용자 생성 확인" class="guide-img-sm" />
...
44. **Security credentials** 탭을 선택합니다.
    ...
    <img src="/images/step5/5-2-step44-security-credentials.png" alt="Security credentials 탭" class="guide-img-sm" />

45. **Access keys** 섹션에서 [[Create access key]]를 클릭합니다.
46. Use case에서 `Local code`를 선택합니다.

    <img src="/images/step5/5-2-step46-use-case.png" alt="Use case - Local code 선택" class="guide-img-sm" />

47. 하단 확인 체크박스를 선택하고 [[Next]]를 클릭합니다.
48. [[Create access key]]를 클릭합니다.

    <img src="/images/step5/5-2-step48-access-key.png" alt="Access key 생성 완료" class="guide-img-sm" />

49. **Access key**와 **Secret access key**를 안전한 곳에 메모합니다.

    <img src="/images/step5/5-2-step49-access-key-show.png" alt="Access key 확인" class="guide-img-sm" />

> [!WARNING]
> Secret access key는 이 화면에서만 볼 수 있습니다. 페이지를 닫으면 다시 확인할 수 없으므로 반드시 메모하세요.  
> [[Download .csv file]]로 다운로드해 두는 것을 권장합니다.
>
> <img src="/images/step5/5-2-step49-secret-warning.png" alt="Secret key 다운로드 권장" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step50-cli-configure.png" alt="AWS CLI 버전 확인" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-step50-cli-install1.png" alt="AWS CLI 설치 확인 1" class="guide-img-sm" />
>
> <img src="/images/step5/5-2-step50-cli-install2.png" alt="AWS CLI 설치 확인 2" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step51-configure-profile.png" alt="aws configure --profile s3-dev 실행" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-step52-credentials-file.png" alt="credentials 파일 확인" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step53-verify-credentials.png" alt="aws configure list 결과" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step54-s3-ls1.png" alt="sts get-caller-identity 결과" class="guide-img-sm" />

<img src="/images/step5/5-2-step54-s3-ls2.png" alt="자격 증명 확인 완료" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step55-s3-ls-test.png" alt="S3 접근 테스트 결과" class="guide-img-sm" />
...
> Spring Boot 프로젝트에는 보통 이미 있지만, 없으면 `dependencies { ... }` 바로 위에 새로 만들면 됩니다. 58. `dependencyManagement` 블록 안에 AWS SDK BOM을 추가합니다:
...
<img src="/images/step5/5-2-step58-gradle-dependency.png" alt="build.gradle BOM 추가" class="guide-img-sm" />
...
> ```
>
> <img src="/images/step5/5-2-step62-properties-tip.png" alt="application.properties 설정 예시" class="guide-img-sm" />
...
> [!WARNING]
> `bucket` 값을 **본인의 실제 Amazon S3 버킷 이름**으로 변경하세요.  
> 버킷 이름이 틀리면 런타임에 `NoSuchBucket` 에러가 발생합니다.

> [!NOTE]
> Amazon EC2에서 IAM Role을 사용하면 `access-key`, `secret-key` 설정이 **필요 없습니다**.  
...
<img src="/images/step5/5-2-step-mvc-dependency.png" alt="MVC 프로젝트 의존성 추가" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step-mvc-properties.png" alt="MVC application.properties 설정" class="guide-img-sm" />
...
> 중요한 건 `@Configuration` 클래스가 컴포넌트 스캔 범위 안에 있으면 됩니다. 65. **New** → **Java Class**를 선택합니다. 66. 클래스 이름에 `S3Config`를 입력하고 `Enter`를 누릅니다.

    <img src="/images/step5/5-2-step66-s3config-class.png" alt="S3Config 클래스 생성" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-step67-config-warning.png" alt="패키지 경로 변경 안내" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step67-s3config-code.png" alt="S3Config 코드 완성" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-task2-warning.png" alt="ComponentScan 패키지 추가 확인" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-step67-package-warning.png" alt="패키지 경로 변경 안내" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step71-s3service-code.png" alt="S3Service 코드 완성" class="guide-img-sm" />
...
>
> <img src="/images/step5/5-2-task3-tip.png" alt="MVC 프로젝트 적용 팁" class="guide-img-sm" />
>
...
> private void upload(Long bno, List<MultipartFile> files) {
>     for (MultipartFile part : files) {
>         // s3Service.upload()는 Amazon S3 버킷에 파일을 업로드하고
...
<img src="/images/step5/5-2-step75-controller-code.png" alt="S3Controller 코드 완성" class="guide-img-sm" />
...
> 이 URL을 가진 사람은 AWS 자격 증명 없이도 지정된 시간 동안 Amazon S3 객체에 접근할 수 있습니다.
>
> **왜 필요한가?**
>
...
> 실제 파일은 클라이언트가 Amazon S3로 직접 보내므로 서버 부하가 없습니다.
>
> ```
> [서버 경유 방식 — 태스크 4]
...
<img src="/images/step5/5-2-step78-bootrun.png" alt="PresignedUrlService 코드 완성" class="guide-img-sm" />
...
클라이언트는 이 API로 URL을 받아서 Amazon S3에 직접 업로드/다운로드합니다.
...
<img src="/images/step5/5-2-step81-upload-test.png" alt="PresignedUrlService 필드 추가" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step82-list-test.png" alt="Presigned URL 엔드포인트 추가 완료" class="guide-img-sm" />
...
84. IntelliJ에서 **Run Configuration**을 엽니다:
    ...
    <img src="/images/step5/5-2-step84-bootrun1.png" alt="Run Configuration 열기" class="guide-img-sm" />

    <img src="/images/step5/5-2-step84-bootrun2.png" alt="Run Configuration 설정" class="guide-img-sm" />
...
85. 환경변수를 추가합니다:
    ...
    <img src="/images/step5/5-2-step85-upload1.png" alt="환경변수 추가 1" class="guide-img-sm" />

    <img src="/images/step5/5-2-step85-upload2.png" alt="환경변수 추가 2" class="guide-img-sm" />
...
86. 앱을 실행합니다 (Boot: 메인 클래스 Run / MVC: ▶️ Run 버튼).
    ...
    <img src="/images/step5/5-2-step86-list.png" alt="앱 실행" class="guide-img-sm" />

87. 앱이 정상 시작되었는지 확인합니다.

    <img src="/images/step5/5-2-step87-download.png" alt="앱 시작 확인" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step88-cat-test.png" alt="cat으로 테스트 파일 확인" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step89-upload-mac.png" alt="파일 업로드 테스트 (Mac)" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step90-file-list.png" alt="파일 목록 조회 결과" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step91-presigned-url.png" alt="Presigned Upload URL 발급 결과" class="guide-img-sm" />
...
92. **발급받은 Presigned URL로 파일을 직접 업로드합니다.**

> [!NOTE]
> **이 단계가 핵심입니다.** 앞서 발급받은 `uploadUrl`은 "이 URL로 보내면 Amazon S3에 저장해줄게"라는 임시 허가증입니다.  
> 이제 그 URL에 실제 파일 데이터를 PUT 요청으로 전송해야 Amazon S3에 파일이 올라갑니다.
>
...
<img src="/images/step5/5-2-step92-presigned-upload.png" alt="Presigned URL로 PUT 업로드" class="guide-img-sm" />
...
업로드 확인 — 파일 목록을 조회하여 Amazon S3에 올라갔는지 확인합니다:

```bash
curl http://localhost:8080/api/files?prefix=test/
```

<img src="/images/step5/5-2-step92-upload-verify.png" alt="Presigned URL 업로드 확인" class="guide-img-sm" />
...
93. **Presigned Download URL을 발급받아 파일을 다운로드합니다.**
...
>
> <img src="/images/step5/5-2-step93-download.png" alt="Presigned Download URL 발급" class="guide-img-sm" />
...
> ```
>
> <img src="/images/step5/5-2-step93-jq-tip.png" alt="jq로 Presigned Download 확인" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step94-file-delete.png" alt="파일 삭제 테스트" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step94-delete-verify.png" alt="삭제 확인 - 빈 배열" class="guide-img-sm" />
...
> 파일이 백엔드 서버를 거치지 않고 **브라우저에서 Amazon S3로 직접** 올라갑니다.
>
...
로컬에서 개발·테스트를 완료했으므로, 이제 Amazon EC2를 생성하여 배포합니다.  
CloudFormation으로 Amazon EC2를 생성하면서 UserData로 Java 17, MySQL, Tomcat 등 필요한 패키지를 자동 설치합니다.
...
| 상황                                                                      | 사용할 파일                 |
| ------------------------------------------------------------------------- | --------------------------- |
| 이전 실습(Step 2-1, 3 등)에서 Amazon VPC를 이미 생성함 또는 기본 VPC 사용 | `step5-2-ec2.yaml`          |
| Amazon VPC를 만든 적 없거나, 깨끗한 환경에서 시작하고 싶음                | `step5-2-ec2-with-vpc.yaml` |

### CloudFormation 스택 생성

📍 **실행 위치: 로컬 PC (브라우저 — AWS 콘솔)**

96. AWS Management Console 상단 검색창에 `CloudFormation`을 입력하고 **CloudFormation** 서비스를 선택합니다.
97. [[Create stack]] → **With new resources (standard)**를 선택합니다.
98. **Prepare template** 섹션에서 **Upload a template file**을 선택합니다.
99. [[Choose file]]을 클릭하고 본인 상황에 맞는 yaml 파일을 선택합니다.
    - 기존 VPC 사용: `step5-2-ec2.yaml`
    - VPC 새로 생성: `step5-2-ec2-with-vpc.yaml`

    <img src="/images/step5/5-2-step99-choose-file.png" alt="템플릿 파일 선택" class="guide-img-sm" />

    <img src="/images/step5/5-2-step99-choose-file2.png" alt="템플릿 업로드 완료" class="guide-img-sm" />

100.  [[Next]]를 클릭합니다.
101.  **Stack name**: `s3-app-ec2`를 입력합니다.
      <img src="/images/step5/5-2-step101-stack-name1.png" alt="Stack name 입력" class="guide-img-sm" />

      <img src="/images/step5/5-2-step101-stack-name2.png" alt="Stack 설정 확인" class="guide-img-sm" />

102.  **Parameters**를 설정합니다:
...
> [!NOTE]
>
> - **KeyPairName**: Amazon EC2에 SSH 접속할 때 사용하는 키 페어. EC2 콘솔 → Key Pairs에서 확인. 없으면 먼저 생성하세요.
> - **VpcId / SubnetId** (`step5-2-ec2.yaml`만): 기존 Amazon VPC와 **Public Subnet**을 선택합니다.
...
> 단, **기존 Amazon VPC와 이름이 같아도 별개의 리소스**입니다 (CloudFormation 스택 단위로 관리).

103. [[Next]]를 클릭합니다 (Configure stack options 페이지 — 기본값 유지).

     <img src="/images/step5/5-2-step103-next.png" alt="Next 클릭" class="guide-img-sm" />

104. 한 번 더 [[Next]]를 클릭합니다 (Review 페이지가 표시됨).
105. 설정 내용을 확인한 후 [[Submit]]을 클릭합니다.

     <img src="/images/step5/5-2-step105-submit.png" alt="Submit 클릭" class="guide-img-sm" />

106. 스택 목록에서 `s3-app-ec2` 스택의 **Status**가 `CREATE_COMPLETE`가 될 때까지 기다립니다 (약 3~5분).
...
107. 스택 이름(`s3-app-ec2`)을 클릭하여 상세 페이지로 이동합니다.

     <img src="/images/step5/5-2-step107-outputs.png" alt="스택 상세 페이지" class="guide-img-sm" />

108. 상단 탭에서 **Outputs** 탭을 클릭합니다.
109. **PublicIP** 값을 메모합니다. 이후 SSH 접속, SCP 전송, API 테스트에서 모두 이 IP를 사용합니다.
...
110. Amazon EC2에 SSH로 접속합니다:

     <img src="/images/step5/5-2-step110-ssh-connect.png" alt="SSH 접속" class="guide-img-sm" />
...
111. UserData 설치가 정상 완료되었는지 확인합니다:
...
112. 각 서비스가 정상 실행 중인지 확인합니다:

     <img src="/images/step5/5-2-step112-services-check.png" alt="서비스 상태 확인" class="guide-img-sm" />
...
✅ **태스크 완료**: Amazon EC2가 생성되고 Java 17 + MySQL 8.4 + Tomcat 9 + Swap이 설치·실행 확인되었습니다.

---

## 태스크 8: IAM Policy + Role 수동 생성 → Amazon EC2에 연결

EC2에서 Amazon S3에 접근하려면 **IAM Role**이 필요합니다.  
로컬에서는 `~/.aws/credentials`를 사용했지만, Amazon EC2에서는 Role을 연결하면 Access Key 없이 자동으로 자격 증명을 획득합니다.

> [!CONCEPT] 왜 Amazon EC2에서 Access Key 대신 IAM Role을 사용하는가?
>
...
113. 상단 검색창에 `IAM`을 입력하고 **IAM** 서비스를 선택합니다.
114. 왼쪽 메뉴에서 **Roles**를 선택합니다.

     <img src="/images/step5/5-2-step114-iam-roles.png" alt="IAM Roles 메뉴" class="guide-img-sm" />

115. [[Create role]]을 클릭합니다.
116. **Trusted entity type**: `AWS service`를 선택합니다.

     <img src="/images/step5/5-2-step116-trusted-entity.png" alt="Trusted entity type 선택" class="guide-img-sm" />

117. **Service or use case** 드롭다운에서 `EC2`를 선택합니다.
118. 아래 **Use case** 라디오 목록에서 첫 번째 `EC2` ("Allows EC2 instances to call AWS services on your behalf.")를 선택합니다.
119. [[Next]]를 클릭합니다.
120. **Permissions policies** 검색창에 `S3AppBucketPolicy`를 입력합니다.

     <img src="/images/step5/5-2-step120-permissions.png" alt="Permissions policies 검색" class="guide-img-sm" />

121. `S3AppBucketPolicy` 체크박스를 선택합니다.
122. [[Next]]를 클릭합니다.
123. **Role name**: `ec2-s3-access-role`을 입력합니다.

     <img src="/images/step5/5-2-step123-role-name.png" alt="Role name 입력" class="guide-img-sm" />

124. **Tags** 섹션에서 [[Add new tag]]를 클릭하여 다음 태그를 추가합니다:
     - `CreatedBy` = `admin-user`
     - `Step` = `step5`
     - `Session` = `5-2`
125. [[Create role]]을 클릭합니다.

     <img src="/images/step5/5-2-step125-create-role.png" alt="Create role 완료" class="guide-img-sm" />

> [!OUTPUT]
> "Role ec2-s3-access-role created" 메시지가 표시됩니다.

### Amazon EC2에 IAM Role 연결

📍 **실행 위치: 로컬 PC (브라우저 — AWS 콘솔)**

126. 상단 검색창에 `EC2`를 입력하고 **EC2** 서비스를 선택합니다.
127. 왼쪽 메뉴에서 **Instances**를 선택합니다.
128. 태스크 7에서 생성한 Amazon EC2 인스턴스의 체크박스를 선택합니다.
129. 상단 **Actions** → **Security** → **Modify IAM role**을 클릭합니다.

     <img src="/images/step5/5-2-step129-modify-iam.png" alt="Modify IAM role 클릭" class="guide-img-sm" />

130. **IAM role** 드롭다운에서 `ec2-s3-access-role`을 선택합니다.

     <img src="/images/step5/5-2-step130-select-role.png" alt="IAM role 선택" class="guide-img-sm" />

131. [[Update IAM role]]을 클릭합니다.

     <img src="/images/step5/5-2-step131-update-role.png" alt="Update IAM role 완료" class="guide-img-sm" />

> [!OUTPUT]
> "Successfully attached ec2-s3-access-role to instance" 메시지가 표시됩니다.

### Amazon EC2에서 Role 동작 확인

📍 **실행 위치: EC2** (SSH 접속한 상태)

132. Amazon EC2에 SSH로 접속합니다 (태스크 7에서 이미 접속 중이면 건너뛰세요):

```bash
ssh -i ~/Downloads/my-keypair.pem ec2-user@{EC2-Public-IP}
```

133. IAM Role이 연결되었는지 확인합니다:

```bash
aws sts get-caller-identity
```

<img src="/images/step5/5-2-step133-iam-verify.png" alt="IAM Role 연결 확인" class="guide-img-sm" />
...
134. Amazon S3 버킷에 접근 가능한지 테스트합니다:

```bash
aws s3 ls s3://{닉네임}-starter-app/
```

<img src="/images/step5/5-2-step134-s3-access.png" alt="S3 접근 테스트 성공" class="guide-img-sm" />

> [!OUTPUT]
> 에러 없이 빈 결과 또는 기존 파일 목록이 표시되면 성공입니다.  
> `AccessDenied` 에러가 나면 IAM Policy의 Resource ARN에서 버킷 이름이 맞는지 확인하세요.

✅ **태스크 완료**: IAM Policy → Role 생성 → EC2 연결이 완료되었습니다. Access Key 없이 Amazon S3에 접근할 수 있습니다.
...
> **기존 MVC 프로젝트** (게시판 등)를 배포하는 경우, Amazon EC2의 MySQL에 사용자와 스키마를 생성해야 합니다.

135. Amazon EC2에 SSH로 접속한 상태에서 MySQL에 root로 접속합니다:
...
136. 애플리케이션용 사용자와 데이터베이스를 생성합니다:
...
<img src="/images/step5/5-2-step136-db-setup.png" alt="DB 사용자 및 데이터베이스 생성" class="guide-img-sm" />

> [!TIP]
> 사용자명, 비밀번호, 데이터베이스명은 본인 프로젝트의 `application.properties` (또는 `root-context.xml`)에 설정된 값과 일치시키세요.

137. (선택) 로컬에서 SQL 파일을 Amazon EC2로 전송하고 실행합니다:
...
<img src="/images/step5/5-2-step137-scp-sql.png" alt="SCP로 SQL 파일 전송" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step137-sql-import.png" alt="SQL 파일 실행 완료" class="guide-img-sm" />

> [!NOTE]
> SQL 파일은 본인 프로젝트에 있는 파일을 사용하세요.  
> 파일이 여러 개라면 테이블 생성 순서(FK 의존 관계)를 고려하여 실행합니다.

<img src="/images/step5/5-2-step137-additional1.png" alt="SQL 실행 결과 1" class="guide-img-sm" />

<img src="/images/step5/5-2-step137-additional2.png" alt="SQL 실행 결과 2" class="guide-img-sm" />

### (선택) 기존 이미지 파일을 Amazon S3에 업로드

> [!NOTE]
> 프로젝트에서 이미지를 로컬 디스크에 저장하고 있었다면, Amazon S3로 전환하기 위해 기존 파일을 업로드해야 합니다.  
...
<img src="/images/step5/5-2-step137-s3-sync.png" alt="S3 sync 이미지 업로드" class="guide-img-sm" />

<img src="/images/step5/5-2-step137-s3-sync2.png" alt="S3 sync 결과 1" class="guide-img-sm" />

<img src="/images/step5/5-2-step137-s3-sync3.png" alt="S3 sync 결과 2" class="guide-img-sm" />

> [!TIP]
> ExFAT 외장 드라이브에서 작업하면 `._` 메타데이터 파일이 자동 생성됩니다.  
> 이 파일들이 Amazon S3에 올라가지 않도록 `--exclude` 옵션을 추가하세요:
>
> ```bash
...
138. IntelliJ 터미널에서 빌드합니다 (프로젝트 루트에서 바로 실행 가능):
...
<img src="/images/step5/5-2-step138-gradle-build.png" alt="Gradle 빌드 결과" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step139-ls-build.png" alt="빌드 결과물 확인" class="guide-img-sm" />
...
<img src="/images/step5/5-2-step140-scp-war.png" alt="SCP로 WAR 전송" class="guide-img-sm" />
...
141. Amazon EC2에 SSH로 접속합니다 (이미 접속 중이면 건너뛰세요):
...
<img src="/images/step5/5-2-step142-deploy-war.png" alt="Tomcat WAR 배포" class="guide-img-sm" />

143. 앱이 정상 시작되었는지 로그를 확인합니다:
...
### Amazon EC2에서 API 테스트

📍 **실행 위치: EC2 + 로컬 PC** (두 터미널 사용)

> [!NOTE]
> 앞서 `tail -f`로 로그를 보고 있다면 `Ctrl+C`로 종료한 후 테스트합니다.  
> 로그를 실시간으로 확인하면서 테스트하려면 **새 터미널**을 열어 Amazon EC2에 SSH 접속하세요:
>
> ```bash
> ssh -i ~/Downloads/my-keypair.pem ec2-user@{EC2-Public-IP}
> ```

144. **EC2 내부**에서 업로드를 테스트합니다 (SSH 접속 상태에서):
...
<img src="/images/step5/5-2-step144-ec2-upload.png" alt="EC2에서 S3 업로드 테스트" class="guide-img-sm" />
...
> ```
>
> `key`와 `url`이 반환되면 Amazon EC2에서 S3 업로드가 성공한 것입니다.  
> IAM Role 덕분에 Access Key 없이도 동작합니다.

145. **본인 PC**(로컬 터미널)에서 외부 접근을 테스트합니다:
...
<img src="/images/step5/5-2-step145-external-test.png" alt="외부에서 EC2 API 테스트" class="guide-img-sm" />
...
> ```
>
> Amazon EC2에서 업로드한 파일이 목록에 표시되면 외부 접근도 정상입니다.

> [!TROUBLESHOOTING]
> | 증상 | 원인 | 해결 |
> |------|------|------|
> | 연결 시간 초과 | Security Group에 8080 미허용 | EC2 콘솔 → Security Group → Inbound rules → 8080 포트 추가 |
> | `Connection refused` | 앱이 실행 중이 아님 | Amazon EC2에서 로그 확인 |
...
✅ **태스크 완료**: Amazon EC2에 배포하고 IAM Role을 통해 Amazon S3에 접근하는 것을 확인했습니다.

---

## 🎯 셀프 미션 1: Amazon EC2에서 게시판 CRUD 테스트 (curl)

> [!NOTE]
> Amazon EC2에 배포한 앱이 정상 동작하는지, curl로 게시판 API CRUD를 직접 테스트해 봅니다.
...
> [!NOTE]
> 이 미션은 선택 사항입니다. 백엔드 배포가 정상 동작하는 것을 확인한 후, 프론트엔드도 Amazon EC2에 올려서 Amazon S3에서 이미지가 정상 로드되는지 확인해 봅니다.

### 목표

- 프론트엔드(Vue) 빌드 결과물(`dist/`)을 Amazon EC2에 배포
...
<img src="/images/step5/5-2-mission2-guide1.png" alt="셀프 미션 2 결과 1" class="guide-img-sm" />

<img src="/images/step5/5-2-mission2-guide2.png" alt="셀프 미션 2 결과 2" class="guide-img-sm" />
...
| 게시글 첨부파일 S3 전환          | 기존 로컬 저장 방식을 Amazon S3로 교체 (board 첨부파일)            |
| 이미지 갤러리 API                | Amazon S3의 특정 prefix 아래 파일 목록을 조회하여 갤러리 반환      |
| Presigned URL 기반 대용량 업로드 | 프론트에서 Presigned URL로 직접 Amazon S3에 업로드하는 플로우 구현 |
...
- 로컬 환경에서 AWS CLI 자격 증명으로 Amazon S3에 접근하여 개발·테스트했습니다.
- Spring 프로젝트에 AWS SDK v2를 설정하고 S3 업로드/다운로드/삭제 API를 구현했습니다.
- Presigned URL을 활용한 클라이언트 직접 업로드 방식을 구현했습니다.
- CloudFormation으로 Amazon EC2를 생성하고 UserData로 Java를 자동 설치했습니다.
- IAM Policy(최소 권한) → IAM Role → EC2 연결을 수동으로 수행하여 Access Key 없이 Amazon S3에 접근했습니다.
...
<img src="/images/step5/5-2-cleanup5-tag-editor.png" alt="Tag Editor 검색 결과" class="guide-img-sm" />
...
8. 터미널을 열고 Amazon EC2에 SSH로 접속합니다:
...
14. 태스크 7에서 생성한 Amazon EC2 인스턴스의 체크박스를 선택합니다.
...
20. 우측 상단 [[Delete stack]] 버튼을 클릭합니다.
21. 확인 팝업이 표시됩니다. 스택 이름 `s3-app-ec2`를 입력하고 [[Delete stack]]을 클릭합니다.

    <img src="/images/step5/5-2-cleanup21-delete-stack.png" alt="Delete stack 확인" class="guide-img-sm" />

22. 스택 상태가 `DELETE_IN_PROGRESS` → `DELETE_COMPLETE`로 변경될 때까지 기다립니다 (약 2~3분).
    - 자동 새로고침이 안 되면 🔄 버튼을 클릭하세요.
      <img src="/images/step5/5-2-cleanup22-delete-complete.png" alt="스택 삭제 완료" class="guide-img-sm" />
...
> 이후 실습에서 Amazon S3를 다시 사용할 예정이라면 **이 단계를 건너뛰어도 됩니다**.
...
> [!OUTPUT]
> 결과가 비어있거나 Amazon S3 버킷만 표시되면 모든 리소스가 정상 정리된 것입니다.  
...
| 확인 항목           | 확인 방법                                 | 정상 상태                              |
| ------------------- | ----------------------------------------- | -------------------------------------- |
| Amazon EC2 인스턴스 | EC2 콘솔 → Instances                      | `Terminated`                           |
...
| Tag Editor          | `Session=5-2` 검색                        | 결과 없음 (또는 Amazon S3 버킷만)      |
