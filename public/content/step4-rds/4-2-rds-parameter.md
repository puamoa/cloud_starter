...
learningObjectives:
  ...
  - Spring Boot HikariCP 커넥션 풀 설정과 Amazon RDS 설정을 연계할 수 있습니다.
prerequisites:
  - AWS 계정 생성 완료
  - Amazon RDS MySQL 인스턴스 실행 중 (Step 4-1 참조)
...
>
> 실습이 끝나면 불필요한 Amazon RDS 인스턴스를 정리하세요. (Step 4-1 리소스 정리 참조)
...
> - **버전별 관리**: MySQL 8.4용 등 엔진 버전별로 별도 관리됨
...
| 공유 리소스  | 여러 Amazon RDS 인스턴스가 동일한 기본 그룹을 참조할 수 있음  |
...
>
> - **DB Parameter Group**: 단일 Amazon RDS 인스턴스용 (이 실습에서 사용)
...
> [!CONCEPT] Amazon RDS 시간대 (time_zone)
> Amazon RDS MySQL의 기본 시간대는 **UTC**입니다. 한국 서비스에서 `NOW()` 함수를 호출하면 한국 시간보다 9시간 느린 시간이 반환됩니다.
>
> **시간대 설정 전략:**
>
> | 전략              | 적용 대상        | 장점                    | 단점                     |
...
> **예시 (학습 환경):**
>
> - Amazon EC2 서버 1대 × HikariCP 풀 크기 10 = 10
...
> - db.t3.micro(1GB)에서 max_connections=200이면 → 200 × 10MB = 2GB → 메모리 부족으로 Amazon RDS 크래시 가능
...
>
> - Parameter Group의 family가 Amazon RDS 엔진 버전과 일치하는지 확인하세요.
...
> | Modify 버튼이 비활성화            | Amazon RDS 상태가 Available이 아님  | Modifying/Rebooting 완료까지 대기                   |
...
✅ **태스크 완료**: Parameter Group이 Amazon RDS에 적용되고 재부팅이 완료되었습니다.
...
> 접속이 안 되는 경우 확인 사항:
>
> - Amazon EC2의 Security Group이 Amazon RDS의 Security Group에서 인바운드 허용되어 있는지 (3306 포트)
> - Amazon RDS가 `Available` 상태인지 (Rebooting 중이면 접속 불가)
> - 비밀번호가 정확한지
...
> | 증상                                    | 원인                         | 해결 방법                                               |
> | --------------------------------------- | ---------------------------- | ------------------------------------------------------- |
> | time_zone이 여전히 UTC                  | Reboot 미실행 또는 PG 미적용 | Amazon RDS Configuration 탭에서 PG 확인 후 Reboot              |
...
>     │   ├── Connection 1 ──→ Amazon RDS
>     │   ├── Connection 2 ──→ Amazon RDS
>     │   ├── ...
>     │   └── Connection 10 ──→ Amazon RDS
...
> Amazon RDS의 `max_connections`는 전체 연결 한도이고, HikariCP의 `maximum-pool-size`는 서버 1대가 사용하는 연결 수입니다.
>
> ```
...
> | `serverTimezone`    | `Asia/Seoul` | JDBC 드라이버의 시간대 (Amazon RDS 설정과 일치) |
