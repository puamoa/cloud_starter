...
이 실습에서는 RDS를 배포하기 위한 DB Subnet Group을 생성하고, RDS MySQL 인스턴스를 Private Subnet에 배치합니다.
EC2에서 MySQL Client를 설치하여 RDS에 접속하고 정상 동작을 확인합니다.

> [!WARNING]
> **비용 주의**: RDS Free Tier(db.t3.micro/db.t4g.micro, 750시간/월)를 초과하면 비용이 발생합니다.
>
> | 리소스                      | 비용 (서울 리전)    | 비고                       |
> | --------------------------- | ------------------- | -------------------------- |
> | RDS db.t3.micro (Free Tier) | 750시간/월 무료     | 초과 시 ~$0.026/시간       |
> | RDS 스토리지 20GB (gp2)     | Free Tier 내 무료   | 20GB 초과 시 ~$0.131/GB·월 |
> | EC2 t2.micro                | 크레딧 내 사용 가능 | 월 ~$10                    |
>
> 실습이 끝나면 **RDS 인스턴스 + DB Subnet Group + EC2**를 반드시 삭제하세요.

> [!NOTE]
> 이 실습은 VPC, Private Subnet 2개(서로 다른 AZ), Security Group이 필요합니다.
...
> RDS를 생성할 때 반드시 DB Subnet Group을 지정해야 하며, **최소 2개 이상의 서로 다른 AZ(Availability Zone)에 있는 서브넷**을 포함해야 합니다.
>
> **왜 2개 AZ가 필요한가?**
>
> RDS는 고가용성(Multi-AZ) 배포를 지원합니다. Single-AZ로 생성하더라도, AWS는 향후 Multi-AZ 전환이나 장애 복구를 위해 최소 2개 AZ를 요구합니다.
>
> ```
...
>
> **운영 환경에서는 반드시 Private Subnet에 RDS를 배치합니다.**
> 이 실습에서도 Private Subnet에 RDS를 배치하고, 같은 VPC 내의 EC2에서만 접속합니다.

✅ **태스크 완료**: DB Subnet Group의 개념과 2개 AZ가 필요한 이유를 이해했습니다.

## 태스크 2: DB Subnet Group 생성

이 태스크에서는 RDS 인스턴스를 배치할 DB Subnet Group을 AWS 콘솔에서 생성합니다.
...
> [!WARNING]
> **Public Subnet을 선택하지 마세요.** DB Subnet Group에 Public Subnet을 포함하면 RDS가 인터넷에 노출될 수 있습니다.
...
이 태스크에서는 Free Tier 템플릿을 사용하여 RDS MySQL 인스턴스를 생성합니다.

24. RDS 콘솔 왼쪽 메뉴에서 **Databases**를 선택합니다.
...
28. **Engine version**: 최신 MySQL 8.0.x 버전을 선택합니다 (기본값 유지).
...
> [!NOTE]
> Free Tier 템플릿을 선택하면 Multi-AZ, 성능 관련 옵션이 자동으로 제한됩니다.
> 이는 비용을 최소화하면서 학습하기에 적합합니다.
>
...
> [!WARNING]
> Master password는 반드시 기억해 두세요. 이후 EC2에서 RDS에 접속할 때 필요합니다.
...
> [!NOTE]
> Free Tier 템플릿을 선택했다면 자동으로 `db.t3.micro`가 선택됩니다.
> `db.t3.micro`: vCPU 2개, RAM 1GB — 학습/개발 용도에 적합합니다.
...
> `Yes`로 설정하면 RDS에 Public IP가 할당되어 인터넷에서 직접 접근이 가능해집니다.
...
> [!WARNING]
> **Deletion protection**이 체크되어 있으면 실습 후 RDS를 삭제할 수 없습니다.
> 학습 환경에서는 반드시 체크 해제하세요.

> [!TIP]
> **Initial database name**에 `mydb`를 입력하면 RDS 생성 시 자동으로 데이터베이스가 만들어집니다.
...
>
> **RDS 생성에는 약 5~10분이 소요됩니다.** 상태가 `Available`이 될 때까지 기다립니다.
...
> ```
>
> 이 Endpoint가 EC2에서 RDS에 접속할 때 사용하는 호스트 주소입니다.
> Port는 `3306` (MySQL 기본 포트)입니다.

✅ **태스크 완료**: RDS MySQL 인스턴스가 생성되었습니다.

## 태스크 4: EC2 생성 + MySQL Client 설치 + RDS 접속 테스트

> [!CONCEPT] EC2에서 RDS 접속 구조
> RDS는 Private Subnet에 있으므로 인터넷에서 직접 접속할 수 없습니다.
> 같은 VPC 내의 EC2 인스턴스에서 MySQL Client를 사용하여 접속합니다.
>
> ```
> 로컬 PC
>     ↓ SSH (Port 22)
> EC2 (Public Subnet, my-public-subnet-a)
>     ↓ MySQL 프로토콜 (Port 3306)
> RDS MySQL (Private Subnet, my-private-subnet-a 또는 b)
>
> Security Group 흐름:
...
> | 증상                                        | 확인 사항                    | 해결 방법                                                                                |
> | ------------------------------------------- | ---------------------------- | ---------------------------------------------------------------------------------------- |
> | `ERROR 2003: Can't connect to MySQL server` | Security Group 인바운드 규칙 | `my-rds-sg`에서 MySQL(3306) 포트가 VPC CIDR(10.0.0.0/16) 또는 EC2 SG에서 허용되는지 확인 |
...
✅ **태스크 완료**: EC2에서 RDS MySQL에 성공적으로 접속하고 데이터를 조작했습니다.
...
- RDS MySQL 인스턴스를 Free Tier 템플릿으로 생성하고 Private Subnet에 배치했습니다.
- EC2에서 MySQL Client를 설치하고 RDS에 접속하여 데이터를 조작했습니다.
...
> [!WARNING]
> **RDS 인스턴스는 실행 중이면 시간당 과금됩니다.** Free Tier 750시간/월을 초과하면 비용이 발생합니다.
> 실습이 끝나면 즉시 삭제하세요.
>
> | 리소스                              | 방치 시 일일 비용 | 방치 시 월 비용 |
> | ----------------------------------- | ----------------- | --------------- |
> | RDS db.t3.micro (Free Tier 초과 시) | ~$0.62/일         | ~$18.7/월       |
> | RDS 스토리지 20GB                   | ~$0.09/일         | ~$2.6/월        |
> | EC2 t2.micro                        | ~$0.35/일         | ~$10/월         |
...
>
> ```
> ① RDS 인스턴스 삭제 (5~10분 대기)
> → ② DB Subnet Group 삭제
> → ③ EC2 인스턴스 종료
> → ④ CloudFormation 스택 삭제
> ```
>
> RDS가 DB Subnet Group을 사용 중이므로, RDS를 먼저 삭제해야 DB Subnet Group을 삭제할 수 있습니다.
...
> [!WARNING]
> RDS 삭제에는 시간이 걸립니다. 상태가 목록에서 사라질 때까지 기다린 후 다음 단계를 진행하세요.
...
>
> RDS 인스턴스가 아직 삭제 중입니다. Databases 목록에서 `my-rds-mysql`이 완전히 사라질 때까지 기다린 후 다시 시도하세요 (약 5~10분).
...
> | 증상                     | 원인                             | 해결 방법                                                          |
> | ------------------------ | -------------------------------- | ------------------------------------------------------------------ |
> | Security Group 삭제 실패 | 다른 리소스가 SG를 참조 중       | EC2가 Terminated 상태인지 확인 → 1~2분 대기 후 재시도              |
