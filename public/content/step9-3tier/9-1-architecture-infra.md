...
📥 **[step9-3tier-infra.yaml 다운로드](/files/step9/step9-3tier-infra.yaml)**
...
1. AWS Console에서 **CloudFormation** 서비스로 이동합니다.
2. [[Create stack]] → **With new resources (standard)**를 클릭합니다.
3. **Prepare template**: `Template is ready` 선택
4. **Template source**: `Upload a template file` 선택
5. [[Choose file]]을 클릭하여 다운로드한 `step9-3tier-infra.yaml`을 업로드합니다.
6. [[Next]]를 클릭합니다.

### 3-2. 스택 파라미터 설정

7. **Stack name**: `my-3tier-infra`
8. 파라미터를 입력합니다:
...
10. **Configure stack options** 페이지에서 기본값 유지 → [[Next]]
11. **Review** 페이지에서 설정을 확인합니다.
12. 하단의 **Capabilities** 체크박스를 확인합니다:
    - ✅ `I acknowledge that AWS CloudFormation might create IAM resources.`
13. [[Submit]]을 클릭합니다.

### 3-4. 스택 생성 대기

14. **Events** 탭에서 리소스 생성 진행 상황을 확인합니다.
15. 전체 생성에 약 **10~15분** 소요됩니다 (RDS가 가장 오래 걸림).
16. Status가 `CREATE_COMPLETE`가 될 때까지 기다립니다.
...
1. CloudFormation → **Stacks** → `my-3tier-infra` 클릭
2. **Outputs** 탭을 클릭합니다.
3. 다음 값들을 메모합니다:
...
4. AWS Console → **VPC** 서비스로 이동합니다.
5. 왼쪽 메뉴에서 **Your VPCs**를 클릭합니다.
6. `my-3tier-app-vpc`가 생성되었는지 확인합니다.
7. **Subnets**에서 4개의 서브넷을 확인합니다:
...
8. AWS Console → **RDS** 서비스로 이동합니다.
9. **Databases**에서 `my-3tier-app-db`를 클릭합니다.
10. **Connectivity & security** 탭에서 Endpoint를 확인합니다.
11. Status가 `Available`인지 확인합니다.

### 4-4. S3 버킷 확인

12. AWS Console → **S3** 서비스로 이동합니다.
13. `my-3tier-app-frontend-{AccountId}` 버킷이 생성되었는지 확인합니다.
14. **Properties** 탭 → **Static website hosting**이 활성화되었는지 확인합니다.

### 4-5. ALB 확인

15. AWS Console → **EC2** → **Load Balancers**로 이동합니다.
16. `my-3tier-app-alb`가 생성되었는지 확인합니다.
17. **DNS name**을 복사합니다 (Step 9-3에서 사용).
18. **Target Groups**에서 `my-3tier-app-tg`를 확인합니다 (아직 등록된 타겟 없음).

### 4-6. Security Groups 확인

19. **EC2** → **Security Groups**에서 3개의 SG를 확인합니다:
...
| 리소스            | 예상 비용     | 비고                  |
| ----------------- | ------------- | --------------------- |
| NAT Gateway       | ~$0.045/시간  | 가장 비용 높음        |
| RDS (db.t3.micro) | ~$0.017/시간  | 프리티어 해당 시 무료 |
| ALB               | ~$0.0225/시간 | 고정 비용             |
| S3                | 거의 무료     | 저장량 기반           |
